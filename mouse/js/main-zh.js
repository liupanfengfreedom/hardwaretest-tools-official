...
function startCPSTest(duration) {
    ...
    buttons.forEach(btn => {
        if (btn.textContent.includes(duration + '秒')) {
...
function endCPSTest() {
    ...
    addLog(`CPS测试完成: ${cps.toFixed(1)} CPS (${cpsTestClicks}次点击, ${actualSeconds.toFixed(1)}秒)`);
