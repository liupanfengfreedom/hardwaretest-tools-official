...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'Kamera İzin Talebi';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'Web Kamerası Test Aracının performans parametrelerini tespit edebilmesi için kamera cihazınıza erişmesi gerekiyor. Lütfen erişime izin verip vermediğinizi onaylayın.';
        ...
        noticeText.textContent = 'Garanti ediyoruz: Tüm veriler yalnızca yerel olarak işlenir, herhangi bir sunucuya yüklenmez.';
        ...
        allowButton.textContent = 'Erişime İzin Ver';
        ...
        denyButton.textContent = 'Erişimi Reddet';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'Kullanıcı onayı bekleniyor...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'Kullanıcı erişim iznini reddetti';
            updateStatus('error');
        ...
        statusText.textContent = 'İzin isteniyor...';
        ...
        console.error("Kamera aracının başlatılması başarısız oldu:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'Kamera algılanmadı';
            videoSelect.appendChild(option);
        ...
        console.error("Cihazlar numaralandırılırken hata oluştu:", error);
        const option = document.createElement('option');
        option.text = 'Cihaz numaralandırması başarısız oldu';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'Kamera video akışı');
        
        updateStatus('active');
        statusText.textContent = 'Normal çalışıyor';
        ...
        console.error('Belirli bir akışı başlatırken hata oluştu:', error);
        updateStatus('error');
        statusText.textContent = 'Başlatma başarısız oldu: ' + (error.name || 'Bilinmiyor');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'Bilinmiyor device';
    deviceLabel.setAttribute('title', track.label || 'Bilinmiyor device');
...
function handleError(error) {
    ...
    let errorMessage = 'Bilinmiyor error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'Kullanıcı kamera iznini reddetti';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'Kamera cihazı bulunamadı';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'Kamera başka bir uygulama tarafından kullanılıyor';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'Kamera parametresi gereksinimleri karşılanamıyor';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'HTTPS veya localhost ortamı gerektirir';
    } else {
    ...
    if (window.confirm('Kamera erişimi başarısız oldu: ' + errorMessage + '\n\nYardım belgelerini görüntüle?')) {
...
videoSelect.onchange = () => {
    statusText.textContent = 'Değiştiriliyor...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'Ayna Çevirme' : 'Aynalamayı İptal Et';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Tarayıcınız kamera erişimini desteklemiyor. Lütfen Chrome, Firefox, Edge veya Safari gibi modern tarayıcıları kullanın.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('HTTPS olmayan ortamda kamera işlevselliği sınırlı olabilir');
    }
...
window.addEventListener('error', function(e) {
    console.error('Kamera aracı genel hatası:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('İşlenmeyen Sözün reddedilmesi:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'Bilinmiyor');
