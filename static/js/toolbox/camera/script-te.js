...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'కెమెరా అనుమతి అభ్యర్థన';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'పనితీరు పారామితులను గుర్తించడానికి వెబ్‌క్యామ్ పరీక్ష సాధనానికి మీ కెమెరా పరికరానికి యాక్సెస్ అవసరం. దయచేసి మీరు యాక్సెస్‌ని అనుమతిస్తే నిర్ధారించాలా?';
        ...
        noticeText.textContent = 'మేము హామీ ఇస్తున్నాము: మొత్తం డేటా స్థానికంగా మాత్రమే ప్రాసెస్ చేయబడుతుంది, ఏ సర్వర్‌లకు అప్‌లోడ్ చేయబడదు.';
        ...
        allowButton.textContent = 'యాక్సెస్‌ని అనుమతించండి';
        ...
        denyButton.textContent = 'యాక్సెస్ నిరాకరించండి';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'వినియోగదారు నిర్ధారణ కోసం వేచి ఉంది...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'వినియోగదారు యాక్సెస్ అనుమతిని తిరస్కరించారు';
            updateStatus('error');
        ...
        statusText.textContent = 'అనుమతిని అభ్యర్థిస్తోంది...';
        ...
        console.error("కెమెరా సాధనం ప్రారంభించడం విఫలమైంది:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'కెమెరా ఏదీ కనుగొనబడలేదు';
            videoSelect.appendChild(option);
        ...
        console.error("పరికరాలను లెక్కించడంలో లోపం:", error);
        const option = document.createElement('option');
        option.text = 'పరికర గణన విఫలమైంది';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'కెమెరా వీడియో స్ట్రీమ్');
        
        updateStatus('active');
        statusText.textContent = 'సాధారణంగా నడుస్తోంది';
        ...
        console.error('నిర్దిష్ట ప్రసారాన్ని ప్రారంభించడంలో లోపం:', error);
        updateStatus('error');
        statusText.textContent = 'ప్రారంభం విఫలమైంది: ' + (error.name || 'తెలియదు');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'తెలియదు device';
    deviceLabel.setAttribute('title', track.label || 'తెలియదు device');
...
function handleError(error) {
    console.error("కెమెరా లోపం:", error);
    updateStatus('error');
    
    let errorMessage = 'తెలియదు error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'వినియోగదారు కెమెరా అనుమతిని తిరస్కరించారు';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'కెమెరా పరికరం కనుగొనబడలేదు';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'కెమెరా మరొక అప్లికేషన్ ద్వారా ఉపయోగించబడుతోంది';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'కెమెరా పారామీటర్ అవసరాలను తీర్చలేదు';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'HTTPS లేదా స్థానిక హోస్ట్ పర్యావరణం అవసరం';
    } else {
        errorMessage = error.message || error.name;
    }
    
    statusText.textContent = 'లోపం: ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('కెమెరా యాక్సెస్ విఫలమైంది: ' + errorMessage + '\n\nసహాయ డాక్యుమెంటేషన్‌ని చూడాలా?')) {
...
function cleanup() {
    ...
    statusText.textContent = 'ఆగిపోయింది';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'మారుతోంది...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'మిర్రర్ ఫ్లిప్' : 'మిర్రర్‌ని రద్దు చేయండి';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('మీ బ్రౌజర్ కెమెరా యాక్సెస్‌కు మద్దతు ఇవ్వదు. దయచేసి Chrome, Firefox, Edge లేదా Safari వంటి ఆధునిక బ్రౌజర్‌లను ఉపయోగించండి.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('HTTPS కాని వాతావరణంలో కెమెరా కార్యాచరణ పరిమితం కావచ్చు');
    }
...
window.addEventListener('error', function(e) {
    console.error('కెమెరా సాధనం గ్లోబల్ ఎర్రర్:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('నిర్వహించని వాగ్దాన తిరస్కరణ:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'తెలియదు');
