...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'Solicitação de permissão de câmera';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'A ferramenta de teste de webcam precisa de acesso ao seu dispositivo de câmera para detectar parâmetros de desempenho. Por favor, confirme se você permite o acesso?';
        ...
        noticeText.textContent = 'Nós garantimos: Todos os dados são processados ​​apenas localmente e não são carregados em nenhum servidor.';
        ...
        denyButton.textContent = 'Negar acesso';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'Aguardando confirmação do usuário...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'Permissão de acesso negada ao usuário';
            updateStatus('error');
        ...
        console.error("Falha na inicialização da ferramenta da câmera:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        const option = document.createElement('option');
        option.text = 'Falha na enumeração do dispositivo';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'Fluxo de vídeo da câmera');
        
        updateStatus('active');
        statusText.textContent = 'Funcionando normalmente';
        ...
        console.error('Erro ao iniciar stream específico:', error);
        updateStatus('error');
        statusText.textContent = 'Falha no início: ' + (error.name || 'Desconhecido');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'Desconhecido device';
    deviceLabel.setAttribute('title', track.label || 'Desconhecido device');
...
function handleError(error) {
    ...
    let errorMessage = 'Desconhecido error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'Usuário negou permissão de câmera';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'Nenhum dispositivo de câmera encontrado';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'A câmera está sendo usada por outro aplicativo';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'Não é possível atender aos requisitos de parâmetros da câmera';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'Requer ambiente HTTPS ou localhost';
    } else {
    ...
    if (window.confirm('Falha no acesso à câmera: ' + errorMessage + '\n\nVer documentação de ajuda?')) {
...
function cleanup() {
    ...
    statusText.textContent = 'Parou';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'Trocando...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'Virar espelho' : 'Cancelar espelho';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Seu navegador não suporta acesso à câmera. Use navegadores modernos como Chrome, Firefox, Edge ou Safari.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('A funcionalidade da câmera pode ser limitada em ambientes não HTTPS');
    }
...
window.addEventListener('error', function(e) {
    console.error('Erro global da ferramenta Câmera:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('Rejeição de promessa não tratada:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'Desconhecido');
