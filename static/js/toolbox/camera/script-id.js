...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'Permintaan Izin Kamera';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'Alat Uji Webcam memerlukan akses ke perangkat kamera Anda untuk mendeteksi parameter kinerja. Harap konfirmasi apakah Anda mengizinkan akses?';
        ...
        noticeText.textContent = 'Kami menjamin: Semua data hanya diproses secara lokal, tidak diunggah ke server mana pun.';
        ...
        allowButton.textContent = 'Izinkan Akses';
        ...
        denyButton.textContent = 'Tolak Akses';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'Menunggu konfirmasi pengguna...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'Pengguna menolak izin akses';
            updateStatus('error');
        ...
        statusText.textContent = 'Meminta izin...';
        ...
        console.error("Inisialisasi alat kamera gagal:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'Tidak ada kamera yang terdeteksi';
            videoSelect.appendChild(option);
        ...
        console.error("Kesalahan saat menghitung perangkat:", error);
        const option = document.createElement('option');
        option.text = 'Pencacahan perangkat gagal';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'Aliran video kamera');
        
        updateStatus('active');
        statusText.textContent = 'Berjalan normal';
        ...
        console.error('Kesalahan saat memulai streaming tertentu:', error);
        updateStatus('error');
        statusText.textContent = 'Mulai gagal: ' + (error.name || 'Tidak dikenal');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'Tidak dikenal device';
    deviceLabel.setAttribute('title', track.label || 'Tidak dikenal device');
...
function handleError(error) {
    ...
    let errorMessage = 'Tidak dikenal error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'Pengguna menolak izin kamera';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'Tidak ada perangkat kamera yang ditemukan';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'Kamera sedang digunakan oleh aplikasi lain';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'Tidak dapat memenuhi persyaratan parameter kamera';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'Membutuhkan lingkungan HTTPS atau localhost';
    } else {
        errorMessage = error.message || error.name;
    }
    
    statusText.textContent = 'Kesalahan: ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('Akses kamera gagal: ' + errorMessage + '\n\nLihat dokumentasi bantuan?')) {
...
videoSelect.onchange = () => {
    statusText.textContent = 'Beralih...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'Cermin Balik' : 'Batalkan Cermin';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Browser Anda tidak mendukung akses kamera. Silakan gunakan browser modern seperti Chrome, Firefox, Edge, atau Safari.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('Fungsionalitas kamera mungkin terbatas di lingkungan non-HTTPS');
    }
...
window.addEventListener('error', function(e) {
    console.error('Kesalahan global alat kamera:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('Penolakan Janji yang Tidak Tertangani:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'Tidak dikenal');
