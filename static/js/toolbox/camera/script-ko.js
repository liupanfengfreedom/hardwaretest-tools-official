...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        description.textContent = '웹캠 테스트 도구는 성능 매개변수를 감지하기 위해 카메라 장치에 액세스해야 합니다. 액세스를 허용하는지 확인해 주세요.';
        ...
        noticeText.textContent = '우리는 다음을 보장합니다: 모든 데이터는 로컬에서만 처리되며 서버에 업로드되지 않습니다.';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = '사용자 확인을 기다리는 중...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = '사용자가 액세스 권한을 거부했습니다.';
            updateStatus('error');
            
            // Delay redirect to let user see status change
            setTimeout(() => {
                window.location.href = '/ko/도구상자/';
            }, 1000);
            return;
        }
        
        // User agreed, continue with original process
        statusText.textContent = '권한을 요청하는 중...';
...
async function startStream() {
    ...
    try {
        ...
        updateStatus('active');
        statusText.textContent = '정상적으로 실행 중';
        ...
        updateStatus('error');
        statusText.textContent = '시작 실패: ' + (error.name || '알려지지 않은');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || '알 수 없는 장치';
    ...
    capTable.innerHTML = capHtml || `<tr><td colspan="2">자세한 드라이버 정보를 가져올 수 없습니다.</td></tr>`;
}
...
function handleError(error) {
    ...
    if (window.confirm('카메라 액세스 실패: ' + errorMessage + '\\n\\n도움말 문서를 보시겠습니까?')) {
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? '거울 뒤집기' : '미러 취소';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('귀하의 브라우저는 카메라 액세스를 지원하지 않습니다. Chrome, Firefox, Edge, Safari 등 최신 브라우저를 사용해 주세요.');
        return;
    }
