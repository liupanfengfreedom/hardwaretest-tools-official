...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        description.textContent = 'Webcam test tool need permission to use your camera so e fit check performance details. You gree make e use am?';
        ...
        noticeText.textContent = 'We promise say all data stay for your device only, e no go upload go any server.';
        ...
        denyButton.textContent = 'No gree access';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'Dey ask for permission...';
        ...
        console.error("Camera tool no fit start well:", error);
        handleError(error);
    }
}
...
function handleError(error) {
    ...
    if (error.name === 'NotAllowedError') {
        errorMessage = 'User deny camera permission';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'No camera device show';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'Another app dey use di camera now now';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'No fit meet di camera parameter requirement';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'E need HTTPS or localhost environment';
    } else {
...
videoSelect.onchange = () => {
    statusText.textContent = 'Dey switch...';
    updateStatus('loading');
...
window.addEventListener('load', () => {
    ...
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('Camera fit get limit for non-HTTPS environment');
    }
...
window.addEventListener('error', function(e) {
    console.error('Global camera tool error:', e.error);
