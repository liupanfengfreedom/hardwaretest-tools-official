...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        description.textContent = 'ウェブカメラ テスト ツールは、パフォーマンス パラメーターを検出するためにカメラ デバイスにアクセスする必要があります。アクセスを許可するか確認してください?';
        ...
        noticeText.textContent = '当社は保証します: すべてのデータはローカルでのみ処理され、サーバーにはアップロードされません。';
        ...
        allowButton.textContent = 'アクセスを許可する';
        ...
        denyButton.textContent = 'アクセスを拒否する';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'ユーザーの確認を待っています...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'ユーザーがアクセス許可を拒否されました';
            updateStatus('error');
            
            // Delay redirect to let user see status change
            setTimeout(() => {
                window.location.href = '/ja/ツールボックス/';
            }, 1000);
            return;
        }
        
        // User agreed, continue with original process
        statusText.textContent = '許可を要求中...';
...
async function startStream() {
    ...
    try {
        ...
        updateStatus('active');
        statusText.textContent = '正常に動作しています';
        ...
        updateStatus('error');
        statusText.textContent = '開始に失敗しました: ' + (error.name || '未知');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || '不明なデバイス';
    ...
    capTable.innerHTML = capHtml || `<tr><td colspan="2">詳細なドライバー情報を取得できません</td></tr>`;
}
...
function handleError(error) {
    ...
    statusText.textContent = 'エラー： ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('カメラへのアクセスに失敗しました: ' + errorMessage + '\\n\\nヘルプドキュメントを表示しますか?')) {
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'ミラーフリップ' : 'ミラーをキャンセル';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('お使いのブラウザはカメラへのアクセスをサポートしていません。 Chrome、Firefox、Edge、Safari などの最新のブラウザを使用してください。');
        return;
    }
