...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'कॅमेरा परवानगी विनंती';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'कार्यप्रदर्शन मापदंड शोधण्यासाठी वेबकॅम चाचणी साधनाला तुमच्या कॅमेरा डिव्हाइसमध्ये प्रवेश आवश्यक आहे. कृपया तुम्ही प्रवेशाची परवानगी दिल्यास पुष्टी करा?';
        ...
        noticeText.textContent = 'आम्ही हमी देतो: सर्व डेटावर केवळ स्थानिक पातळीवर प्रक्रिया केली जाते, कोणत्याही सर्व्हरवर अपलोड केली जात नाही.';
        ...
        allowButton.textContent = 'प्रवेशास अनुमती द्या';
        ...
        denyButton.textContent = 'प्रवेश नाकार';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'वापरकर्त्याच्या पुष्टीकरणाची प्रतीक्षा करत आहे...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'वापरकर्त्याने प्रवेश परवानगी नाकारली';
            updateStatus('error');
        ...
        statusText.textContent = 'परवानगीची विनंती करत आहे...';
        ...
        console.error("कॅमेरा टूल इनिशिएलायझेशन अयशस्वी:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'कोणताही कॅमेरा आढळला नाही';
            videoSelect.appendChild(option);
        ...
        console.error("डिव्हाइसेसची गणना करताना त्रुटी:", error);
        const option = document.createElement('option');
        option.text = 'डिव्हाइस गणना अयशस्वी';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'कॅमेरा व्हिडिओ प्रवाह');
        
        updateStatus('active');
        statusText.textContent = 'सामान्यपणे चालत आहे';
        ...
        console.error('विशिष्ट प्रवाह सुरू करताना त्रुटी:', error);
        updateStatus('error');
        statusText.textContent = 'प्रारंभ अयशस्वी: ' + (error.name || 'अज्ञात');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'अज्ञात device';
    deviceLabel.setAttribute('title', track.label || 'अज्ञात device');
...
function handleError(error) {
    console.error("कॅमेरा त्रुटी:", error);
    updateStatus('error');
    
    let errorMessage = 'अज्ञात error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'वापरकर्त्याने कॅमेरा परवानगी नाकारली';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'कॅमेरा डिव्हाइस आढळले नाही';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'कॅमेरा दुसऱ्या अनुप्रयोगाद्वारे वापरला जात आहे';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'कॅमेरा पॅरामीटर आवश्यकता पूर्ण करू शकत नाही';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'HTTPS किंवा लोकलहोस्ट वातावरण आवश्यक आहे';
    } else {
        errorMessage = error.message || error.name;
    }
    
    statusText.textContent = 'त्रुटी: ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('कॅमेरा प्रवेश अयशस्वी: ' + errorMessage + '\n\nमदत दस्तऐवजीकरण पहायचे?')) {
...
function cleanup() {
    ...
    statusText.textContent = 'थांबला';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'स्विच करत आहे...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'मिरर फ्लिप' : 'मिरर रद्द करा';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('तुमचा ब्राउझर कॅमेरा प्रवेशास समर्थन देत नाही. कृपया Chrome, Firefox, Edge किंवा Safari सारखे आधुनिक ब्राउझर वापरा.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('नॉन-HTTPS वातावरणात कॅमेरा कार्यक्षमता मर्यादित असू शकते');
    }
...
window.addEventListener('error', function(e) {
    console.error('कॅमेरा टूल ग्लोबल एरर:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('न हाताळलेले वचन नाकारणे:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'अज्ञात');
