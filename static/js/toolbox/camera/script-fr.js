...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'Demande d'autorisation de caméra';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'Webcam Test Tool doit accéder à votre appareil photo pour détecter les paramètres de performances. Veuillez confirmer si vous autorisez l'accès ?';
        ...
        noticeText.textContent = 'Nous garantissons : toutes les données sont traitées localement uniquement et ne sont téléchargées sur aucun serveur.';
        ...
        allowButton.textContent = 'Autoriser l'accès';
        ...
        denyButton.textContent = 'Refuser l'accès';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'En attente de confirmation de l'utilisateur...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'L'utilisateur a refusé l'autorisation d'accès';
            updateStatus('error');
        ...
        statusText.textContent = 'Demander l'autorisation...';
        ...
        console.error("L'initialisation de l'outil Caméra a échoué :", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'Aucune caméra détectée';
            videoSelect.appendChild(option);
        ...
        const option = document.createElement('option');
        option.text = 'L'énumération des appareils a échoué';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'Flux vidéo de la caméra');
        
        updateStatus('active');
        statusText.textContent = 'Fonctionne normalement';
        ...
        console.error('Erreur lors du démarrage d'un flux spécifique :', error);
        updateStatus('error');
        statusText.textContent = 'Échec du démarrage : ' + (error.name || 'Inconnu');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'Inconnu device';
    deviceLabel.setAttribute('title', track.label || 'Inconnu device');
...
function handleError(error) {
    ...
    let errorMessage = 'Inconnu error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'L'utilisateur a refusé l'autorisation de la caméra';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'Aucun appareil photo trouvé';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'La caméra est utilisée par une autre application';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'Ne peut pas répondre aux exigences des paramètres de la caméra';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'Nécessite un environnement HTTPS ou localhost';
    } else {
    ...
    if (window.confirm('L'accès à la caméra a échoué : ' + errorMessage + '\n\nAfficher la documentation d'aide ?')) {
...
videoSelect.onchange = () => {
    statusText.textContent = 'Commutation...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'Retournement du miroir' : 'Annuler le miroir';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Votre navigateur ne prend pas en charge l'accès à la caméra. Veuillez utiliser des navigateurs modernes comme Chrome, Firefox, Edge ou Safari.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('La fonctionnalité de la caméra peut être limitée dans un environnement non HTTPS');
    }
...
window.addEventListener('error', function(e) {
    console.error('Erreur globale de l'outil Appareil photo :', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('Rejet de la promesse non gérée :', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'Inconnu');
