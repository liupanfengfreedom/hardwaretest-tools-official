...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'کیمرے کی اجازت کی درخواست';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'کارکردگی کے پیرامیٹرز کا پتہ لگانے کے لیے ویب کیم ٹیسٹ ٹول کو آپ کے کیمرہ ڈیوائس تک رسائی کی ضرورت ہے۔ براہ کرم تصدیق کریں کہ کیا آپ رسائی کی اجازت دیتے ہیں؟';
        ...
        noticeText.textContent = 'ہم ضمانت دیتے ہیں: تمام ڈیٹا کو صرف مقامی طور پر پروسیس کیا جاتا ہے، کسی سرور پر اپ لوڈ نہیں کیا جاتا ہے۔';
        ...
        allowButton.textContent = 'رسائی کی اجازت دیں۔';
        ...
        denyButton.textContent = 'رسائی سے انکار کریں۔';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'صارف کی تصدیق کا انتظار ہے...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'صارف نے رسائی کی اجازت سے انکار کر دیا۔';
            updateStatus('error');
        ...
        statusText.textContent = 'اجازت طلب کی جا رہی ہے...';
        ...
        console.error("کیمرے کے آلے کی شروعات ناکام ہوگئی:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'کسی کیمرے کا پتہ نہیں چلا';
            videoSelect.appendChild(option);
        ...
        console.error("آلات کی گنتی میں خرابی:", error);
        const option = document.createElement('option');
        option.text = 'ڈیوائس کی گنتی ناکام ہو گئی۔';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'کیمرہ ویڈیو سٹریم');
        
        updateStatus('active');
        statusText.textContent = 'عام طور پر چل رہا ہے۔';
        ...
        console.error('مخصوص سلسلہ شروع کرنے میں خرابی:', error);
        updateStatus('error');
        statusText.textContent = 'شروع ناکام: ' + (error.name || 'نامعلوم');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'نامعلوم device';
    deviceLabel.setAttribute('title', track.label || 'نامعلوم device');
...
function handleError(error) {
    console.error("کیمرے کی خرابی:", error);
    updateStatus('error');
    
    let errorMessage = 'نامعلوم error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'صارف نے کیمرے کی اجازت سے انکار کر دیا۔';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'کوئی کیمرہ آلہ نہیں ملا';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'کیمرہ ایک اور ایپلیکیشن کے ذریعے استعمال کیا جا رہا ہے۔';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'کیمرے کے پیرامیٹر کی ضروریات کو پورا نہیں کر سکتا';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'HTTPS یا لوکل ہوسٹ ماحول کی ضرورت ہے۔';
    } else {
        errorMessage = error.message || error.name;
    }
    
    statusText.textContent = 'خرابی: ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('کیمرے تک رسائی ناکام: ' + errorMessage + '\n\nمدد کی دستاویزات دیکھیں؟')) {
...
function cleanup() {
    ...
    statusText.textContent = 'رک گیا۔';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'سوئچ کر رہا ہے...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'آئینہ پلٹائیں۔' : 'آئینہ منسوخ کریں۔';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('آپ کا براؤزر کیمرے تک رسائی کی حمایت نہیں کرتا ہے۔ براہ کرم کروم، فائر فاکس، ایج، یا سفاری جیسے جدید براؤزر استعمال کریں۔');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('غیر HTTPS ماحول میں کیمرے کی فعالیت محدود ہو سکتی ہے۔');
    }
...
window.addEventListener('error', function(e) {
    console.error('کیمرے کے آلے کی عالمی خرابی:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('غیر سنبھالا ہوا وعدہ مسترد:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'نامعلوم');
