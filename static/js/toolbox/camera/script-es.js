...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'Solicitud de permiso de cámara';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'Webcam Test Tool necesita acceso a su dispositivo de cámara para detectar parámetros de rendimiento. ¿Por favor confirma si permites el acceso?';
        ...
        noticeText.textContent = 'Garantizamos: Todos los datos se procesan únicamente localmente y no se cargan en ningún servidor.';
        ...
        allowButton.textContent = 'Permitir acceso';
        ...
        denyButton.textContent = 'Denegar acceso';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'Esperando confirmación del usuario...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'Permiso de acceso denegado al usuario';
            updateStatus('error');
        ...
        statusText.textContent = 'Solicitando permiso...';
        ...
        console.error("Error al inicializar la herramienta de la cámara:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'No se detectó ninguna cámara';
            videoSelect.appendChild(option);
        ...
        const option = document.createElement('option');
        option.text = 'Error en la enumeración del dispositivo';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'Transmisión de vídeo de la cámara');
        
        updateStatus('active');
        statusText.textContent = 'Corriendo normalmente';
        ...
        console.error('Error al iniciar una secuencia específica:', error);
        updateStatus('error');
        statusText.textContent = 'Inicio fallido: ' + (error.name || 'Desconocido');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'Desconocido device';
    deviceLabel.setAttribute('title', track.label || 'Desconocido device');
...
function handleError(error) {
    ...
    let errorMessage = 'Desconocido error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'Usuario denegado permiso de cámara';
        ...
        errorMessage = 'No se pueden cumplir los requisitos de los parámetros de la cámara';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'Requiere HTTPS o entorno localhost';
    } else {
    ...
    if (window.confirm('Error de acceso a la cámara: ' + errorMessage + '\n\n¿Ver documentación de ayuda?')) {
...
function cleanup() {
    ...
    statusText.textContent = 'Interrumpido';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'Traspuesta...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'Voltear espejo' : 'Cancelar espejo';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Su navegador no admite el acceso a la cámara. Utilice navegadores modernos como Chrome, Firefox, Edge o Safari.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('La funcionalidad de la cámara puede estar limitada en entornos que no son HTTPS');
    }
...
window.addEventListener('error', function(e) {
    console.error('Error global de la herramienta de cámara:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('Rechazo de promesa no controlada:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'Desconocido');
