...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'Anfrage zur Kameragenehmigung';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'Das Webcam Test Tool benötigt Zugriff auf Ihr Kameragerät, um Leistungsparameter zu erkennen. Bitte bestätigen Sie, ob Sie den Zugriff zulassen?';
        ...
        noticeText.textContent = 'Wir garantieren: Alle Daten werden nur lokal verarbeitet und nicht auf irgendwelche Server hochgeladen.';
        ...
        allowButton.textContent = 'Zugriff zulassen';
        ...
        denyButton.textContent = 'Zugriff verweigern';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'Warten auf Benutzerbestätigung...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'Dem Benutzer wurde die Zugriffsberechtigung verweigert';
            updateStatus('error');
        ...
        statusText.textContent = 'Bitte um Erlaubnis...';
        ...
        console.error("Die Initialisierung des Kameratools ist fehlgeschlagen:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'Keine Kamera erkannt';
            videoSelect.appendChild(option);
        ...
        const option = document.createElement('option');
        option.text = 'Geräteaufzählung ist fehlgeschlagen';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'Kamera-Videostream');
        
        updateStatus('active');
        statusText.textContent = 'Läuft normal';
        ...
        console.error('Fehler beim Starten eines bestimmten Streams:', error);
        updateStatus('error');
        statusText.textContent = 'Start fehlgeschlagen: ' + (error.name || 'Unbekannt');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'Unbekannt device';
    deviceLabel.setAttribute('title', track.label || 'Unbekannt device');
...
function handleError(error) {
    ...
    let errorMessage = 'Unbekannt error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'Der Benutzer hat die Kameraberechtigung verweigert';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'Kein Kameragerät gefunden';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'Die Kamera wird von einer anderen Anwendung verwendet';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'Die Anforderungen an die Kameraparameter können nicht erfüllt werden';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'Erfordert eine HTTPS- oder Localhost-Umgebung';
    } else {
        errorMessage = error.message || error.name;
    }
    
    statusText.textContent = 'Fehler: ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('Kamerazugriff fehlgeschlagen: ' + errorMessage + '\n\nHilfedokumentation ansehen?')) {
...
function cleanup() {
    ...
    statusText.textContent = 'Angehalten';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'Wechsel...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'Spiegelumschlag' : 'Spiegeln abbrechen';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Ihr Browser unterstützt keinen Kamerazugriff. Bitte verwenden Sie moderne Browser wie Chrome, Firefox, Edge oder Safari.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('Die Kamerafunktionalität kann in einer Nicht-HTTPS-Umgebung eingeschränkt sein');
    }
...
window.addEventListener('error', function(e) {
    console.error('Globaler Fehler des Kamera-Tools:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('Ablehnung eines nicht behandelten Versprechens:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'Unbekannt');
