...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'طلب إذن الكاميرا';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'تحتاج أداة اختبار كاميرا الويب إلى الوصول إلى جهاز الكاميرا الخاص بك لاكتشاف معلمات الأداء. يرجى التأكيد إذا كنت تسمح بالوصول؟';
        ...
        noticeText.textContent = 'نحن نضمن: تتم معالجة جميع البيانات محليًا فقط، ولا يتم تحميلها على أي خوادم.';
        ...
        allowButton.textContent = 'السماح بالوصول';
        ...
        denyButton.textContent = 'رفض الوصول';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'في انتظار تأكيد المستخدم...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'رفض المستخدم إذن الوصول';
            updateStatus('error');
        ...
        statusText.textContent = 'جارٍ طلب الإذن...';
        ...
        console.error("فشلت تهيئة أداة الكاميرا:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'لم يتم اكتشاف أي كاميرا';
            videoSelect.appendChild(option);
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'دفق فيديو الكاميرا');
        
        updateStatus('active');
        statusText.textContent = 'تشغيل بشكل طبيعي';
        ...
        console.error('خطأ في بدء بث معين:', error);
        updateStatus('error');
        statusText.textContent = 'فشل البدء: ' + (error.name || 'مجهول');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'مجهول device';
    deviceLabel.setAttribute('title', track.label || 'مجهول device');
...
function handleError(error) {
    ...
    let errorMessage = 'مجهول error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'رفض المستخدم إذن الكاميرا';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'لم يتم العثور على جهاز الكاميرا';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'الكاميرا قيد الاستخدام بواسطة تطبيق آخر';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'لا يمكن تلبية متطلبات معلمات الكاميرا';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'يتطلب HTTPS أو بيئة المضيف المحلي';
    } else {
    ...
    if (window.confirm('فشل الوصول إلى الكاميرا: ' + errorMessage + '\n\nهل تريد عرض وثائق المساعدة؟')) {
...
function cleanup() {
    ...
    statusText.textContent = 'توقف';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'التبديل...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'مرآة الوجه' : 'إلغاء المرآة';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('متصفحك لا يدعم الوصول إلى الكاميرا. الرجاء استخدام المتصفحات الحديثة مثل Chrome أو Firefox أو Edge أو Safari.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('قد تكون وظائف الكاميرا محدودة في بيئة غير HTTPS');
    }
...
window.addEventListener('error', function(e) {
    console.error('خطأ عالمي في أداة الكاميرا:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('رفض الوعد غير المعالج:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'مجهول');
