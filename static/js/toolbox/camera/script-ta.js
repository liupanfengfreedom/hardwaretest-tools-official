...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'கேமரா அனுமதி கோரிக்கை';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'செயல்திறன் அளவுருக்களைக் கண்டறிய வெப்கேம் சோதனைக் கருவிக்கு உங்கள் கேமரா சாதனத்திற்கான அணுகல் தேவை. நீங்கள் அணுகலை அனுமதித்தால் உறுதிப்படுத்தவா?';
        ...
        noticeText.textContent = 'நாங்கள் உத்தரவாதம் அளிக்கிறோம்: எல்லா தரவும் உள்நாட்டில் மட்டுமே செயலாக்கப்படும், எந்த சேவையகத்திலும் பதிவேற்றப்படவில்லை.';
        ...
        allowButton.textContent = 'அணுகலை அனுமதிக்கவும்';
        ...
        denyButton.textContent = 'அணுகலை மறுக்கவும்';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'பயனர் உறுதிப்படுத்தலுக்காகக் காத்திருக்கிறது...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'பயனர் அணுகல் அனுமதி மறுக்கப்பட்டது';
            updateStatus('error');
        ...
        statusText.textContent = 'அனுமதி கோருகிறது...';
        ...
        console.error("கேமரா கருவி துவக்கம் தோல்வியடைந்தது:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'கேமரா எதுவும் கண்டறியப்படவில்லை';
            videoSelect.appendChild(option);
        ...
        console.error("சாதனங்களைக் கணக்கிடுவதில் பிழை:", error);
        const option = document.createElement('option');
        option.text = 'சாதனக் கணக்கீடு தோல்வியடைந்தது';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'கேமரா வீடியோ ஸ்ட்ரீம்');
        
        updateStatus('active');
        statusText.textContent = 'சாதாரணமாக இயங்கும்';
        ...
        console.error('குறிப்பிட்ட ஸ்ட்ரீமை தொடங்குவதில் பிழை:', error);
        updateStatus('error');
        statusText.textContent = 'துவக்கம் தோல்வி: ' + (error.name || 'தெரியவில்லை');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'தெரியவில்லை device';
    deviceLabel.setAttribute('title', track.label || 'தெரியவில்லை device');
...
function handleError(error) {
    console.error("கேமரா பிழை:", error);
    updateStatus('error');
    
    let errorMessage = 'தெரியவில்லை error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'பயனர் கேமரா அனுமதியை மறுத்தார்';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'கேமரா சாதனம் எதுவும் இல்லை';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'கேமராவை மற்றொரு பயன்பாடு பயன்படுத்துகிறது';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'கேமரா அளவுரு தேவைகளை பூர்த்தி செய்ய முடியவில்லை';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'HTTPS அல்லது லோக்கல் ஹோஸ்ட் சூழல் தேவை';
    } else {
        errorMessage = error.message || error.name;
    }
    
    statusText.textContent = 'பிழை: ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('கேமரா அணுகல் தோல்வி: ' + errorMessage + '\n\nஉதவி ஆவணங்களைப் பார்க்கவா?')) {
...
function cleanup() {
    ...
    statusText.textContent = 'நிறுத்தப்பட்டது';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'மாறுகிறது...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'மிரர் ஃபிளிப்' : 'கேன்சல் மிரர்';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('உங்கள் உலாவி கேமரா அணுகலை ஆதரிக்கவில்லை. Chrome, Firefox, Edge அல்லது Safari போன்ற நவீன உலாவிகளைப் பயன்படுத்தவும்.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('HTTPS அல்லாத சூழலில் கேமரா செயல்பாடு குறைவாக இருக்கலாம்');
    }
...
window.addEventListener('error', function(e) {
    console.error('கேமரா கருவி உலகளாவிய பிழை:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('கையாளப்படாத வாக்குறுதி நிராகரிப்பு:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'தெரியவில்லை');
