...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'Запрос разрешения камеры';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'Инструменту тестирования веб-камеры требуется доступ к вашей камере для определения параметров производительности. Пожалуйста, подтвердите, разрешаете ли вы доступ?';
        ...
        noticeText.textContent = 'Мы гарантируем: Все данные обрабатываются только локально, а не загружаются на какие-либо серверы.';
        ...
        allowButton.textContent = 'Разрешить доступ';
        ...
        denyButton.textContent = 'Запретить доступ';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'Ожидание подтверждения пользователя...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'Пользователю отказано в доступе';
            updateStatus('error');
        ...
        statusText.textContent = 'Запрашиваю разрешение...';
        ...
        console.error("Не удалось инициализировать инструмент камеры:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'Камера не обнаружена';
            videoSelect.appendChild(option);
        ...
        const option = document.createElement('option');
        option.text = 'Перечисление устройств не удалось';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'Видеопоток с камеры');
        
        updateStatus('active');
        statusText.textContent = 'Работает нормально';
        ...
        console.error('Ошибка запуска определенного потока:', error);
        updateStatus('error');
        statusText.textContent = 'Запуск не удался: ' + (error.name || 'Неизвестный');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'Неизвестный device';
    deviceLabel.setAttribute('title', track.label || 'Неизвестный device');
...
function handleError(error) {
    ...
    let errorMessage = 'Неизвестный error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'Пользователю отказано в разрешении камеры';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'Камера не найдена';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'Камера используется другим приложением';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'Невозможно удовлетворить требования к параметрам камеры';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'Требуется HTTPS или среда локального хоста.';
    } else {
        errorMessage = error.message || error.name;
    }
    
    statusText.textContent = 'Ошибка: ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('Доступ к камере не удался: ' + errorMessage + '\n\nПосмотреть справочную документацию?')) {
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'Зеркало флип' : 'Отменить зеркало';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Ваш браузер не поддерживает доступ к камере. Используйте современные браузеры, такие как Chrome, Firefox, Edge или Safari.');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('Функциональность камеры может быть ограничена в среде, отличной от HTTPS.');
    }
...
window.addEventListener('error', function(e) {
    console.error('Глобальная ошибка инструмента камеры:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('Необработанный отказ от обещания:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'Неизвестный');
