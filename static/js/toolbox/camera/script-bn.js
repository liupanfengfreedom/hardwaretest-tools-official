...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'ক্যামেরা অনুমতি অনুরোধ';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'ওয়েবক্যাম টেস্ট টুলের পারফরম্যান্স প্যারামিটার সনাক্ত করতে আপনার ক্যামেরা ডিভাইসে অ্যাক্সেস প্রয়োজন। আপনি অ্যাক্সেস অনুমতি দিলে নিশ্চিত করুন?';
        ...
        noticeText.textContent = 'আমরা গ্যারান্টি: সমস্ত ডেটা শুধুমাত্র স্থানীয়ভাবে প্রক্রিয়া করা হয়, কোনো সার্ভারে আপলোড করা হয় না।';
        ...
        allowButton.textContent = 'অ্যাক্সেসের অনুমতি দিন';
        ...
        denyButton.textContent = 'অ্যাক্সেস অস্বীকার করুন';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'ব্যবহারকারী নিশ্চিতকরণের জন্য অপেক্ষা করা হচ্ছে...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'ব্যবহারকারী অ্যাক্সেসের অনুমতি অস্বীকার করেছেন৷';
            updateStatus('error');
        ...
        statusText.textContent = 'অনুমতির অনুরোধ করা হচ্ছে...';
        ...
        console.error("ক্যামেরা টুল আরম্ভ করা ব্যর্থ হয়েছে:", error);
        handleError(error);
    }
}
...
async function populateDeviceList(activeVideoId) {
    try {
        ...
        if (videoSelect.options.length > 0) {
            ...
            const option = document.createElement('option');
            option.text = 'কোন ক্যামেরা সনাক্ত করা যায়নি';
            videoSelect.appendChild(option);
        ...
        console.error("ডিভাইস গণনার ত্রুটি:", error);
        const option = document.createElement('option');
        option.text = 'ডিভাইস গণনা ব্যর্থ হয়েছে';
        videoSelect.innerHTML = '';
        videoSelect.appendChild(option);
        videoSelect.disabled = true;
    }
...
async function startStream() {
    ...
    try {
        ...
        videoElement.setAttribute('aria-label', track.label || 'ক্যামেরা ভিডিও স্ট্রিম');
        
        updateStatus('active');
        statusText.textContent = 'স্বাভাবিকভাবে চলছে';
        ...
        console.error('নির্দিষ্ট স্ট্রীম শুরু করতে ত্রুটি:', error);
        updateStatus('error');
        statusText.textContent = 'শুরু ব্যর্থ হয়েছে: ' + (error.name || 'অজানা');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'অজানা device';
    deviceLabel.setAttribute('title', track.label || 'অজানা device');
...
function handleError(error) {
    console.error("ক্যামেরা ত্রুটি:", error);
    updateStatus('error');
    
    let errorMessage = 'অজানা error';
    if (error.name === 'NotAllowedError') {
        errorMessage = 'ব্যবহারকারী ক্যামেরার অনুমতি অস্বীকার করেছেন৷';
    } else if (error.name === 'NotFoundError') {
        errorMessage = 'কোনো ক্যামেরা ডিভাইস পাওয়া যায়নি';
    } else if (error.name === 'NotReadableError') {
        errorMessage = 'ক্যামেরা অন্য অ্যাপ্লিকেশন দ্বারা ব্যবহার করা হচ্ছে';
    } else if (error.name === 'OverconstrainedError') {
        errorMessage = 'ক্যামেরা প্যারামিটারের প্রয়োজনীয়তা পূরণ করতে পারে না';
    } else if (error.name === 'SecurityError') {
        errorMessage = 'HTTPS বা লোকালহোস্ট পরিবেশ প্রয়োজন';
    } else {
        errorMessage = error.message || error.name;
    }
    
    statusText.textContent = 'ত্রুটি: ' + errorMessage;
    statusText.style.color = "#ef4444";
    
    // Show user-friendly prompt
    if (window.confirm('ক্যামেরা অ্যাক্সেস ব্যর্থ হয়েছে: ' + errorMessage + '\n\nসাহায্য ডকুমেন্টেশন দেখুন?')) {
...
function cleanup() {
    ...
    statusText.textContent = 'থেমে গেল';
    statusText.style.color = "var(--text-muted)";
}

// Event listeners (unchanged structure)
videoSelect.onchange = () => {
    statusText.textContent = 'পাল্টানো হচ্ছে...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'মিরর ফ্লিপ' : 'মিরর বাতিল করুন';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('আপনার ব্রাউজার ক্যামেরা অ্যাক্সেস সমর্থন করে না। ক্রোম, ফায়ারফক্স, এজ বা সাফারির মতো আধুনিক ব্রাউজার ব্যবহার করুন।');
        return;
    }
    
    // Check if in HTTPS environment (production requirement)
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
        console.warn('নন-HTTPS পরিবেশে ক্যামেরা কার্যকারিতা সীমিত হতে পারে');
    }
...
window.addEventListener('error', function(e) {
    console.error('ক্যামেরা টুল গ্লোবাল ত্রুটি:', e.error);
...
window.addEventListener('unhandledrejection', function(e) {
    console.error('অনিয়ন্ত্রিত প্রতিশ্রুতি প্রত্যাখ্যান:', e.reason);
    
    // Send error to analytics
    trackPerformanceEvent('unhandled_promise', 'error', e.reason?.message || 'অজানা');
