...
function showPermissionRequestModal() {
    return new Promise((resolve) => {
        ...
        title.textContent = 'Yêu cầu cấp phép máy ảnh';
        title.style.cssText = `
            color: var(--text-main);
            margin-top: 0;
            margin-bottom: 20px;
        ...
        description.textContent = 'Công cụ kiểm tra webcam cần quyền truy cập vào thiết bị camera của bạn để phát hiện các thông số hiệu suất. Vui lòng xác nhận xem bạn có cho phép truy cập không?';
        ...
        noticeText.textContent = 'Chúng tôi đảm bảo: Tất cả dữ liệu chỉ được xử lý cục bộ, không được tải lên bất kỳ máy chủ nào.';
        ...
        allowButton.textContent = 'Cho phép truy cập';
        ...
        denyButton.textContent = 'Từ chối quyền truy cập';
...
async function initCameraTool() {
    try {
        ...
        statusText.textContent = 'Đang chờ xác nhận của người dùng...';
        ...
        if (!userConfirmed) {
            ...
            statusText.textContent = 'Người dùng bị từ chối quyền truy cập';
            updateStatus('error');
            
            // Delay redirect to let user see status change
            setTimeout(() => {
                window.location.href = '/vi/hộp công cụ/';
            }, 1000);
            return;
        }
        
        // User agreed, continue with original process
        statusText.textContent = 'Đang yêu cầu sự cho phép...';
...
async function startStream() {
    ...
    try {
        ...
        updateStatus('active');
        statusText.textContent = 'Chạy bình thường';
        ...
        updateStatus('error');
        statusText.textContent = 'Bắt đầu thất bại: ' + (error.name || 'Không xác định');
...
function analyzeVideoTrack(track) {
    ...
    deviceLabel.textContent = track.label || 'Thiết bị không xác định';
    ...
    capTable.innerHTML = capHtml || `<tr><td colspan="2">Không thể lấy thông tin chi tiết về trình điều khiển</td></tr>`;
}
...
function handleError(error) {
    ...
    if (window.confirm('Truy cập máy ảnh không thành công: ' + errorMessage + '\\n\\nXem tài liệu trợ giúp?')) {
...
videoSelect.onchange = () => {
    statusText.textContent = 'Đang chuyển đổi...';
    updateStatus('loading');
...
document.getElementById('btn-mirror').onclick = () => {
    ...
    btn.textContent = isMirrored ? 'lật gương' : 'Hủy phản chiếu';
};
...
window.addEventListener('load', () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Trình duyệt của bạn không hỗ trợ truy cập máy ảnh. Vui lòng sử dụng các trình duyệt hiện đại như Chrome, Firefox, Edge hoặc Safari.');
        return;
    }
