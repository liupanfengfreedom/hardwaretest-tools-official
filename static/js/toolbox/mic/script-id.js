(async function(){
  ...
  function formatVolumeLevel(db) {
    ...
    if (db < -50) {
      return "Suara jarum jatuh ke lantai";
    }
    if (db < -35) {
      return "Suara bicara pelan";
    }
    if (db < -20) {
      return "Suara bicara normal";
    }
    if (db < -6) {
      return "Suara bicara keras";
    }
    return "Terlalu keras, bisa pecah";
  }
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'Mikropon' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>Tidak ada mikrofon yang terdeteksi</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Menghitung...";
    audioLatencyEl.textContent = "Menguji...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "Menghubungkan...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "Izin ditolak";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "Tidak Terhubung";
      micStatusDisplayEl.className = "";
    ...
    bitDepthDisplayEl.textContent = "Mengambang 32-bit";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "Klik halaman untuk mengaktifkan audio";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "Merekam... <br><span style='font-size:12px'>Lepaskan untuk Diputar</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "Tahan untuk Merekam<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Lepaskan untuk Memutar</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "Memutar ulang..."; 
