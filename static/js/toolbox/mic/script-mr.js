(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "आवाजेची प्रतीक्षा";
    }
    if (db < -50) {
      return "जमिनीवर पिन पडल्याचा आवाज";
    }
    if (db < -35) {
      return "हळू बोलण्याचा आवाज";
    }
    if (db < -20) {
      return "सामान्य बोलण्याचा आवाज";
    }
    if (db < -6) {
      return "मोठ्याने बोलण्याचा आवाज";
    }
    return "आवाज खूप मोठा आहे, विकृत होऊ शकतो";
  }

  function formatVolumeParts(db) {
    if (!Number.isFinite(db)) {
      return { description: "आवाजेची प्रतीक्षा", dbText: "" };
    }
    return {
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'मायक्रोफोन' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>कोणताही मायक्रोफोन आढळला नाही</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "गणना करत आहे...";
    audioLatencyEl.textContent = "चाचणी करत आहे...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "कनेक्ट करत आहे...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "परवानगी नाकारली";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "कनेक्ट केलेले नाही";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "१ (मोनो)";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "ऑडिओ सक्रिय करण्यासाठी पृष्ठावर क्लिक करा";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "रेकॉर्डिंग... <br><span style='font-size:12px'>प्ले करण्यासाठी रिलीज करा</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "रेकॉर्ड करण्यासाठी धरून ठेवा<br><span style='font-size:12px;font-weight:400;opacity:0.8'>प्ले टू रिलीज</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "परत खेळत आहे..."; 
