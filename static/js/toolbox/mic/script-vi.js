(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "Đang chờ âm thanh";
    }
    if (db < -50) {
      return "Tiếng kim rơi xuống sàn";
    }
    if (db < -35) {
      return "Giọng nói nhỏ";
    }
    if (db < -20) {
      return "Giọng nói bình thường";
    }
    if (db < -6) {
      return "Giọng nói lớn";
    }
    return "Quá lớn, có thể bị vỡ tiếng";
  }

  function formatVolumeParts(db) {
    if (!Number.isFinite(db)) {
      return { description: "Đang chờ âm thanh", dbText: "" };
    }
    return {
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'Micrô ' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>Không phát hiện thấy micrô</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Đang tính...";
    audioLatencyEl.textContent = "Đang thử nghiệm...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "Đang kết nối...";
    ...
    try {
      ...
      micStatusDisplayEl.textContent = "Đã kết nối";
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "Quyền bị từ chối";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "Chưa kết nối";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1 (Đơn sắc)";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "Bấm vào trang để kích hoạt âm thanh";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "Đang ghi... <br><span style='font-size:12px'>Phát hành lên Play</span>";
      playStateEl.textContent = "ghi âm"; 
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "Giữ để ghi<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Phát hành để chơi</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "Đang phát lại..."; 
    ...
    player.onended = () => { 
      playStateEl.textContent = "Chế độ chờ"; 
  ...
  permBtn.addEventListener('click', () => {
    statusText.textContent = "Đang yêu cầu...";
    startEngine();
  });
