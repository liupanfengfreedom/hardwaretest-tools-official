(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "आवाज़ की प्रतीक्षा";
    }
    if (db < -50) {
      return "फर्श पर पिन गिरने जैसी आवाज़";
    }
    if (db < -35) {
      return "धीमी बातचीत की आवाज़";
    }
    if (db < -20) {
      return "सामान्य बातचीत की आवाज़";
    }
    if (db < -6) {
      return "तेज़ बातचीत की आवाज़";
    }
    return "आवाज़ बहुत तेज़ है, क्लिप हो सकती है";
  }
  ...
  function formatVolumeWithDb(db) {
    ...
    return `${formatVolumeLevel(db)} (${db.toFixed(1)} डीबी)`;
  }
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'माइक्रोफ़ोन ' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>कोई माइक्रोफ़ोन नहीं मिला</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "हिसाब लगाया जा रहा है...";
    audioLatencyEl.textContent = "परीक्षण...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "कनेक्ट हो रहा है...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "अनुमति नहीं मिली";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "जुड़े नहीं हैं";
      micStatusDisplayEl.className = "";
    ...
    sampleRateDisplayEl.textContent = audioCtx.sampleRate + " हर्ट्ज";
    channelsDisplayEl.textContent = "1 (मोनो)";
    bitDepthDisplayEl.textContent = "32-बिट फ़्लोट";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "ऑडियो सक्रिय करने के लिए पेज पर क्लिक करें";
      const resume = async () => { 
  ...
  function drawVisuals(){
    ...
    function loop(){
      ...
      if(minNoiseDb < -50) { // Only calculate in quiet environments
        ...
        snRatioEl.textContent = snRatioValue.toFixed(1) + " डीबी";
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "रिकॉर्डिंग... <br><span style='font-size:12px'>Play</span> पर रिलीज़ करें";
      playStateEl.textContent = "रिकॉर्डिंग"; 
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "रिकॉर्ड करने के लिए होल्ड करें<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Play करने के लिए रिलीज</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "वापस खेल रहा हूँ..."; 
    ...
    player.onended = () => { 
      playStateEl.textContent = "समर्थन करना"; 
  ...
  permBtn.addEventListener('click', () => {
    statusText.textContent = "निवेदन है...";
    startEngine();
  });
