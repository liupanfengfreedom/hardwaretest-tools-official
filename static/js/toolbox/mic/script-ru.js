(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "Ожидание звука";
    }
    if (db < -50) {
      return "Звук падения булавки на пол";
    }
    if (db < -35) {
      return "Тихая речь";
    }
    ...
    return "Слишком громко, возможны искажения";
  }
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'Микрофон' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>Микрофон не обнаружен</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Расчет...";
    audioLatencyEl.textContent = "Тестирование...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "Подключение...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "Доступ запрещен";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "Не подключен";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1 (моно)";
    bitDepthDisplayEl.textContent = "32-битное число с плавающей запятой";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "Нажмите на страницу, чтобы активировать звук";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "Запись... <br><span style='font-size:12px'>Выпустить для воспроизведения</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "Удерживайте для записи<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Отпустите для воспроизведения</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "Воспроизведение..."; 
