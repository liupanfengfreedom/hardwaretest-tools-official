(async function(){
  ...
  function formatVolumeLevel(db) {
    ...
    if (db < -50) {
      return "Yere düşen iğne sesi";
    }
    if (db < -35) {
      return "Kısık konuşma sesi";
    }
    if (db < -20) {
      return "Normal konuşma sesi";
    }
    if (db < -6) {
      return "Yüksek konuşma sesi";
    }
    return "Çok yüksek, bozulabilir";
  }
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'Mikrofon' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>Mikrofon algılanmadı</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Hesaplanıyor...";
    audioLatencyEl.textContent = "Test...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "Bağlanıyor...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "İzin reddedildi";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "Bağlı Değil";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1 (Tek renkli)";
    bitDepthDisplayEl.textContent = "32 bit Kayan";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "Sesi etkinleştirmek için sayfaya tıklayın";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "Kaydediliyor... <br><span style='font-size:12px'>Oynatmak için Yayınlayın</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "Kaydetmek için Basılı Tutun<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Oynatmak için Yayınlayın</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "Oynatılıyor..."; 
