(async function(){
  ...
  function formatVolumeParts(db) {
    if (!Number.isFinite(db)) {
      return { description: "శబ్దం కోసం వేచి ఉంది", dbText: "" };
    }
    return {
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'మైక్రోఫోన్' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>మైక్రోఫోన్ కనుగొనబడలేదు</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "లెక్కిస్తోంది...";
    audioLatencyEl.textContent = "పరీక్షిస్తోంది...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "కనెక్ట్ అవుతోంది...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "అనుమతి నిరాకరించబడింది";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "కనెక్ట్ కాలేదు";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1 (మోనో)";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "ఆడియోని యాక్టివేట్ చేయడానికి పేజీని క్లిక్ చేయండి";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "రికార్డింగ్... <br><span style='font-size:12px'>ప్లే చేయడానికి విడుదల చేయండి</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "రికార్డ్ చేయడానికి పట్టుకోండి<br><span style='font-size:12px;font-weight:400;opacity:0.8'>ప్లే చేయడానికి విడుదల చేయండి</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "తిరిగి ప్లే అవుతోంది..."; 
