(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "Dey wait for sound";
    }
    if (db < -50) {
      return "Sound like pin fall for ground";
    }
    if (db < -35) {
      return "Low voice";
    }
    if (db < -20) {
      return "Normal talk voice";
    }
    if (db < -6) {
      return "Loud talk voice";
    }
    return "Sound too loud, e fit scatter";
  }

  function formatVolumeParts(db) {
    if (!Number.isFinite(db)) {
      return { description: "Dey wait for sound", dbText: "" };
    }
    return {
  ...
  async function refreshDevices(){
    try{
      ...
      if(inputs.length===0) deviceSelect.innerHTML='<option>No mic show</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Dey calculate...";
    audioLatencyEl.textContent = "Dey test...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "Dey connect...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "Permission no gree";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "No connect";
      micStatusDisplayEl.className = "";
