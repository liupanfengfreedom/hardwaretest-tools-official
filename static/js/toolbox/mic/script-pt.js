(async function(){
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'Microfone' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>Nenhum microfone detectado</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Calculando...";
    audioLatencyEl.textContent = "Testando...";
  ...
  async function startEngine(){
    ...
    try {
      ...
      statusText.textContent = "Correndo";
      ...
      // Update microphone status display
      micStatusDisplayEl.textContent = "Não conectado";
      micStatusDisplayEl.className = "";
    ...
    bitDepthDisplayEl.textContent = "Flutuante de 32 bits";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "Clique na página para ativar o áudio";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "Gravando... <br><span style='font-size:12px'>Libere para reproduzir</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "Segure para gravar<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Solte para reproduzir</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "Reproduzindo..."; 
