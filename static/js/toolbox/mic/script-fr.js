(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "En attente de son";
    }
    if (db < -50) {
      return "Bruit d une épingle qui tombe";
    }
    if (db < -35) {
      return "Voix basse";
    }
    ...
    return "Trop fort, risque de saturation";
  }
  ...
  async function refreshDevices(){
    try{
      ...
      if(inputs.length===0) deviceSelect.innerHTML='<option>Aucun microphone détecté</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Calculateur...";
    audioLatencyEl.textContent = "Essai...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "De liaison...";
    ...
    try {
      ...
      micStatusDisplayEl.textContent = "Connecté";
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "Autorisation refusée";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "Non connecté";
      micStatusDisplayEl.className = "";
    ...
    bitDepthDisplayEl.textContent = "Flottant 32 bits";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "Cliquez sur la page pour activer l'audio";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "Enregistrement... <br><span style='font-size:12px'>Lâcher pour jouer</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "Maintenir pour enregistrer<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Relâcher pour jouer</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "Lecture..."; 
