(async function(){
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'マイクロフォン ' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>マイクが検出されません</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "計算中...";
    audioLatencyEl.textContent = "テスト中...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "接続中...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "許可が拒否されました";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "接続されていません";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1(モノラル)";
    bitDepthDisplayEl.textContent = "32ビット浮動小数点";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "ページをクリックして音声を有効にします";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "録音中... <br><span style='font-size:12px'>リリースして再生</span>";
      playStateEl.textContent = "録音"; 
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "長押しして録音<br><span style='font-size:12px;font-weight:400;opacity:0.8'>離して再生</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "再生中..."; 
