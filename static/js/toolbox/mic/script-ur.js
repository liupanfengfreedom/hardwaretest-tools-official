(async function(){
  ...
  function formatVolumeParts(db) {
    if (!Number.isFinite(db)) {
      return { description: "آواز کا انتظار", dbText: "" };
    }
    return {
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'مائیکروفون' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>کوئی مائیکروفون نہیں ملا</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "حساب کر رہا ہے...";
    audioLatencyEl.textContent = "جانچ ہو رہی ہے...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "منسلک ہو رہا ہے...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "اجازت نامنظور";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "منسلک نہیں ہے۔";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1 (مونو)";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "آڈیو کو چالو کرنے کے لیے صفحہ پر کلک کریں۔";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "ریکارڈنگ... <br><span style='font-size:12px'>چلانے کے لیے ریلیز</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "ریکارڈ کرنے کے لیے ہولڈ کریں<br><span style='font-size:12px;font-weight:400;opacity:0.8'>چلنے کے لیے ریلیز کریں</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "واپس چل رہا ہے..."; 
