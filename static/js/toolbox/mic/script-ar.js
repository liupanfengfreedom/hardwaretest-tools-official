(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "في انتظار الصوت";
    }
    if (db < -50) {
      return "صوت سقوط دبوس على الأرض";
    }
    if (db < -35) {
      return "صوت حديث منخفض";
    }
    if (db < -20) {
      return "صوت حديث عادي";
    }
    if (db < -6) {
      return "صوت حديث مرتفع";
    }
    return "الصوت مرتفع جدًا وقد يتشوه";
  }

  function formatVolumeParts(db) {
    if (!Number.isFinite(db)) {
      return { description: "في انتظار الصوت", dbText: "" };
    }
    return {
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'ميكروفون' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>لم يتم اكتشاف أي ميكروفون</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "جارٍ الحساب...";
    audioLatencyEl.textContent = "اختبار...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "جارٍ الاتصال...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "تم رفض الإذن";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "غير متصل";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1 (أحادي)";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "انقر على الصفحة لتفعيل الصوت";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "جاري التسجيل... <br><span style='font-size:12px'>حرر للتشغيل</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "اضغط مع الاستمرار للتسجيل<br><span style='font-size:12px;font-weight:400;opacity:0.8'>حرر للتشغيل</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "إعادة التشغيل..."; 
