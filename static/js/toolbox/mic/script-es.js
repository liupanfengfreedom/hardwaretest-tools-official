(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "Esperando sonido";
    }
    if (db < -50) {
      return "Sonido de un alfiler al caer";
    }
    if (db < -35) {
      return "Voz baja";
    }
    ...
    return "Demasiado alto, puede saturar";
  }
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'Micrófono' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>No se detectó ningún micrófono</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Calculador...";
    audioLatencyEl.textContent = "Pruebas...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "Conectando...";
    ...
    try {
      ...
      // Update microphone status display
      micStatusDisplayEl.textContent = "No conectado";
      micStatusDisplayEl.className = "";
    ...
    bitDepthDisplayEl.textContent = "Flotante de 32 bits";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "Haga clic en la página para activar el audio.";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "Grabando... <br><span style='font-size:12px'>Liberar para reproducir</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "Mantenga presionado para grabar<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Liberar para reproducir</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "Reproduciendo..."; 
