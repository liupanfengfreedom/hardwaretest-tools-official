(async function(){
  ...
  function formatVolumeLevel(db) {
    ...
    if (db < -50) {
      return "Geräusch einer fallenden Nadel";
    }
    if (db < -35) {
      return "Leise Sprechstimme";
    }
    if (db < -20) {
      return "Normale Sprechstimme";
    }
    if (db < -6) {
      return "Laute Sprechstimme";
    }
    return "Zu laut, kann übersteuern";
  }

  function formatVolumeParts(db) {
    if (!Number.isFinite(db)) {
      return { description: "Warte auf Ton", dbText: "" };
    }
    return {
  ...
  async function refreshDevices(){
    try{
      ...
      if(inputs.length===0) deviceSelect.innerHTML='<option>Kein Mikrofon erkannt</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "Berechnen...";
    audioLatencyEl.textContent = "Testen...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "Verbinden...";
    ...
    try {
      ...
      micStatusDisplayEl.textContent = "Verbunden";
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "Zugriff verweigert";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "Nicht verbunden";
      micStatusDisplayEl.className = "";
    ...
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "Klicken Sie auf die Seite, um Audio zu aktivieren";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "Aufnahme... <br><span style='font-size:12px'>Zur Wiedergabe freigeben</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "Zum Aufzeichnen gedrückt halten<br><span style='font-size:12px;font-weight:400;opacity:0.8'>Zum Abspielen loslassen</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "Wiedergabe..."; 
