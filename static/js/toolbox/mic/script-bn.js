(async function(){
  ...
  function formatVolumeLevel(db) {
    if (!Number.isFinite(db) || db < -70) {
      return "শব্দের অপেক্ষায়";
    }
    if (db < -50) {
      return "মেঝেতে পিন পড়ার শব্দ";
    }
    if (db < -35) {
      return "নরম কথার শব্দ";
    }
    if (db < -20) {
      return "স্বাভাবিক কথার শব্দ";
    }
    if (db < -6) {
      return "জোরে কথার শব্দ";
    }
    return "খুব জোরে, শব্দ বিকৃত হতে পারে";
  }
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'মাইক্রোফোন' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>কোন মাইক্রোফোন শনাক্ত হয়নি</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "গণনা করা হচ্ছে...";
    audioLatencyEl.textContent = "পরীক্ষা করা হচ্ছে...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "সংযোগ করা হচ্ছে...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "অনুমতি অস্বীকার করা হয়েছে";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "সংযুক্ত নয়";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1 (মনো)";
    bitDepthDisplayEl.textContent = "32-বিট ফ্লোট";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "অডিও সক্রিয় করতে পৃষ্ঠায় ক্লিক করুন";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "রেকর্ডিং... <br><span style='font-size:12px'>প্লেতে প্রকাশ করুন</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "রেকর্ড করতে হোল্ড করুন<br><span style='font-size:12px;font-weight:400;opacity:0.8'>প্লেতে রিলিজ করুন</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "বাজানো হচ্ছে..."; 
