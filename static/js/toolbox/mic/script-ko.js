(async function(){
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || '마이크로폰 ' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>마이크가 감지되지 않음</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "계산 중...";
    audioLatencyEl.textContent = "테스트 중...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "연결 중...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "허가가 거부되었습니다";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "연결되지 않음";
      micStatusDisplayEl.className = "";
    ...
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "오디오를 활성화하려면 페이지를 클릭하세요.";
      const resume = async () => { 
  ...
  function drawVisuals(){
    ...
    function loop(){
      ...
      if(minNoiseDb < -50) { // Only calculate in quiet environments
        ...
        snRatioEl.textContent = snRatioValue.toFixed(1) + " 데시벨";
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "녹음 중... <br><span style='font-size:12px'>Play로 출시</span>";
      playStateEl.textContent = "녹음"; 
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "길게 눌러 녹화<br><span style='font-size:12px;font-weight:400;opacity:0.8'>재생하려면 출시</span>";
