(async function(){
  ...
  function formatVolumeParts(db) {
    if (!Number.isFinite(db)) {
      return { description: "ஒலிக்காக காத்திருக்கிறது", dbText: "" };
    }
    return {
  ...
  async function refreshDevices(){
    try{
      ...
      inputs.forEach(d => {
        ...
        opt.textContent = d.label || 'ஒலிவாங்கி' + (deviceSelect.length + 1); 
        deviceSelect.appendChild(opt);
      });
      if(inputs.length===0) deviceSelect.innerHTML='<option>மைக்ரோஃபோன் எதுவும் கண்டறியப்படவில்லை</option>';
    } catch(e){ console.error(e); }
  }
  
  async function runLatencyTest(){
    latencyMsEl.textContent = "கணக்கிடுகிறது...";
    audioLatencyEl.textContent = "சோதனை...";
  ...
  async function startEngine(){
    ...
    micStatusDisplayEl.textContent = "இணைக்கிறது...";
    ...
    try {
      ...
      // Failure: UI update
      console.error(e);
      statusText.textContent = "அனுமதி மறுக்கப்பட்டது";
      statusText.style.color = "#f43f5e";
      statusDot.classList.remove('on');
      statusDot.classList.add('err');
      
      // Update microphone status display
      micStatusDisplayEl.textContent = "இணைக்கப்படவில்லை";
      micStatusDisplayEl.className = "";
    ...
    channelsDisplayEl.textContent = "1 (மோனோ)";
    
    // Autoplay Policy Handling
    if(audioCtx.state === 'suspended'){
      statusText.textContent = "ஆடியோவைச் செயல்படுத்த பக்கத்தைக் கிளிக் செய்யவும்";
      const resume = async () => { 
  ...
  function startRecord(e) {
    ...
    if(mediaStream) { // Only record if stream is active
      ...
      pttBtn.innerHTML = "பதிவு செய்கிறது... <br><span style='font-size:12px'>ப்ளே செய்ய வெளியிடு</span>";
  ...
  function stopRecordAndPlay(e) {
    ...
    pttBtn.innerHTML = "ரெக்கார்டு செய்யப் பிடிக்கவும்<br><span style='font-size:12px;font-weight:400;opacity:0.8'>ப்ளே செய்ய வெளியிடவும்</span>";
    if(recordedBuffers.length === 0) return;
    playStateEl.textContent = "மீண்டும் விளையாடுகிறது..."; 
