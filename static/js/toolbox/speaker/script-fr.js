...
let currentChannel = "Les deux canaux"; // Default channel mode
...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = 'Activé';
    }
...
function setChannelMode(mode) {
    ...
    if (mode === 'Canal gauche') document.getElementById('leftChannelBtn').classList.add('active');
    else if (mode === 'Canal droit') document.getElementById('rightChannelBtn').classList.add('active');
    else if (mode === 'Les deux canaux') document.getElementById('bothChannelsBtn').classList.add('active');
}
...
function stopAll() {
    ...
    playStopText.innerText = 'Démarrer la lecture';
    audioStatusDisplay.innerText = 'Arrêté';
...
function startOscillator(freq) {
    ...
    playStopText.innerText = 'Arrêter la lecture';
    currentFrequencyDisplay.innerText = freq + ' Hz';
    audioStatusDisplay.innerText = 'Lecture d'onde sinusoïdale';
}
...
function startSweep() {
    ...
    audioStatusDisplay.innerText = 'Balayage';
    ...
    oscillator.onended = function() {
        stopAll();
        currentFrequencyDisplay.innerText = '20 000 Hz';
        audioStatusDisplay.innerText = 'Balayage terminé';
        setTimeout(() => {
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = 'Arrêter la lecture';
    currentFrequencyDisplay.innerText = type === 'white' ? 'Bruit blanc' : 'Bruit rose';
    audioStatusDisplay.innerText = (type === 'white' ? 'White' : 'Pink') + 'Jeu de bruit';
...
document.getElementById('leftChannelBtn').onclick = () => setChannelMode('Canal gauche');
document.getElementById('bothChannelsBtn').onclick = () => setChannelMode('Les deux canaux');
document.getElementById('rightChannelBtn').onclick = () => setChannelMode('Canal droit');
