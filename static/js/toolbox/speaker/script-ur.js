...
let currentChannel = "دونوں چینلز"; // Default channel mode
...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = 'چالو';
    }
...
function setChannelMode(mode) {
    ...
    if (mode === 'بائیں چینل') document.getElementById('leftChannelBtn').classList.add('active');
    else if (mode === 'دائیں چینل') document.getElementById('rightChannelBtn').classList.add('active');
    else if (mode === 'دونوں چینلز') document.getElementById('bothChannelsBtn').classList.add('active');
}
...
function stopAll() {
    ...
    playStopText.innerText = 'پلے بیک شروع کریں۔';
    audioStatusDisplay.innerText = 'رک گیا۔';
...
function startOscillator(freq) {
    ...
    playStopText.innerText = 'پلے بیک بند کریں۔';
    currentFrequencyDisplay.innerText = freq + ' Hz';
    audioStatusDisplay.innerText = 'سائن ویو چل رہا ہے۔';
}
...
function startSweep() {
    ...
    oscillator.onended = function() {
        stopAll();
        currentFrequencyDisplay.innerText = '20000 ہرٹج';
        audioStatusDisplay.innerText = 'جھاڑو مکمل';
        setTimeout(() => {
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = 'پلے بیک بند کریں۔';
    currentFrequencyDisplay.innerText = type === 'white' ? 'سفید شور' : 'گلابی شور';
    audioStatusDisplay.innerText = (type === 'white' ? 'White' : 'Pink') + 'شور بجانا';
...
document.getElementById('leftChannelBtn').onclick = () => setChannelMode('بائیں چینل');
document.getElementById('bothChannelsBtn').onclick = () => setChannelMode('دونوں چینلز');
document.getElementById('rightChannelBtn').onclick = () => setChannelMode('دائیں چینل');
