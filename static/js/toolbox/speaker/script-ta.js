...
let currentChannel = "இரண்டு சேனல்களும்"; // Default channel mode
...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = 'செயல்படுத்தப்பட்டது';
    }
...
function setChannelMode(mode) {
    ...
    if (mode === 'இடது சேனல்') document.getElementById('leftChannelBtn').classList.add('active');
    else if (mode === 'வலது சேனல்') document.getElementById('rightChannelBtn').classList.add('active');
    else if (mode === 'இரண்டு சேனல்களும்') document.getElementById('bothChannelsBtn').classList.add('active');
}
...
function stopAll() {
    ...
    playStopText.innerText = 'பிளேபேக்கைத் தொடங்கவும்';
    audioStatusDisplay.innerText = 'நிறுத்தப்பட்டது';
...
function startOscillator(freq) {
    ...
    playStopText.innerText = 'பிளேபேக்கை நிறுத்து';
    currentFrequencyDisplay.innerText = freq + ' Hz';
    audioStatusDisplay.innerText = 'சைன் வேவ் விளையாடுகிறது';
}
...
function startSweep() {
    ...
    audioStatusDisplay.innerText = 'துடைப்பது';
    playStopBtn.disabled = true;
    
    // Update current frequency display to sweep start frequency
    currentFrequencyDisplay.innerText = '10 ஹெர்ட்ஸ்';
    ...
    oscillator.onended = function() {
        stopAll();
        currentFrequencyDisplay.innerText = '20000 ஹெர்ட்ஸ்';
        audioStatusDisplay.innerText = 'ஸ்வீப் முடிந்தது';
        setTimeout(() => {
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = 'பிளேபேக்கை நிறுத்து';
    currentFrequencyDisplay.innerText = type === 'white' ? 'வெள்ளை சத்தம்' : 'பிங்க் சத்தம்';
    audioStatusDisplay.innerText = (type === 'white' ? 'White' : 'Pink') + 'சத்தம் விளையாடுகிறது';
...
document.getElementById('leftChannelBtn').onclick = () => setChannelMode('இடது சேனல்');
document.getElementById('bothChannelsBtn').onclick = () => setChannelMode('இரண்டு சேனல்களும்');
document.getElementById('rightChannelBtn').onclick = () => setChannelMode('வலது சேனல்');
