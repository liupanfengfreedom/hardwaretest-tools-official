...
let currentChannel = "Ambos canales"; // Default channel mode
...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = 'Activado';
    }
...
function setChannelMode(mode) {
    ...
    if (mode === 'Canal izquierdo') document.getElementById('leftChannelBtn').classList.add('active');
    else if (mode === 'Canal derecho') document.getElementById('rightChannelBtn').classList.add('active');
    else if (mode === 'Ambos canales') document.getElementById('bothChannelsBtn').classList.add('active');
}
...
function stopAll() {
    ...
    playStopText.innerText = 'Iniciar reproducción';
    audioStatusDisplay.innerText = 'Interrumpido';
...
function startOscillator(freq) {
    ...
    playStopText.innerText = 'Detener la reproducción';
    currentFrequencyDisplay.innerText = freq + ' Hz';
    audioStatusDisplay.innerText = 'Reproducción de onda sinusoidal';
}
...
function startSweep() {
    ...
    oscillator.onended = function() {
        stopAll();
        currentFrequencyDisplay.innerText = '20000Hz';
        audioStatusDisplay.innerText = 'Barrido completo';
        setTimeout(() => {
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = 'Detener la reproducción';
    currentFrequencyDisplay.innerText = type === 'white' ? 'Ruido Blanco' : 'Ruido Rosa';
    audioStatusDisplay.innerText = (type === 'white' ? 'White' : 'Pink') + 'ruido jugando';
...
document.getElementById('leftChannelBtn').onclick = () => setChannelMode('Canal izquierdo');
document.getElementById('bothChannelsBtn').onclick = () => setChannelMode('Ambos canales');
document.getElementById('rightChannelBtn').onclick = () => setChannelMode('Canal derecho');
