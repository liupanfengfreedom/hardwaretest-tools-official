...
let currentChannel = "Оба канала"; // Default channel mode
...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = 'Активировано';
    }
...
function setChannelMode(mode) {
    ...
    if (mode === 'Левый канал') document.getElementById('leftChannelBtn').classList.add('active');
    else if (mode === 'Правый канал') document.getElementById('rightChannelBtn').classList.add('active');
    else if (mode === 'Оба канала') document.getElementById('bothChannelsBtn').classList.add('active');
}
...
function stopAll() {
    ...
    playStopText.innerText = 'Начать воспроизведение';
    audioStatusDisplay.innerText = 'Остановлено';
...
function startOscillator(freq) {
    ...
    playStopText.innerText = 'Остановить воспроизведение';
    currentFrequencyDisplay.innerText = freq + ' Hz';
    audioStatusDisplay.innerText = 'Синусоидальная игра';
}
...
function startSweep() {
    ...
    oscillator.onended = function() {
        stopAll();
        currentFrequencyDisplay.innerText = '20000 Гц';
        audioStatusDisplay.innerText = 'Развертка завершена';
        setTimeout(() => {
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = 'Остановить воспроизведение';
    currentFrequencyDisplay.innerText = type === 'white' ? 'Белый шум' : 'Розовый шум';
    audioStatusDisplay.innerText = (type === 'white' ? 'White' : 'Pink') + 'Шумовая игра';
...
document.getElementById('leftChannelBtn').onclick = () => setChannelMode('Левый канал');
document.getElementById('bothChannelsBtn').onclick = () => setChannelMode('Оба канала');
document.getElementById('rightChannelBtn').onclick = () => setChannelMode('Правый канал');
