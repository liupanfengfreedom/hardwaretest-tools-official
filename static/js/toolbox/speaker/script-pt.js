...
let currentChannel = "Ambos os canais"; // Default channel mode
...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = 'Ativado';
    }
...
function setChannelMode(mode) {
    ...
    if (mode === 'Canal Esquerdo') document.getElementById('leftChannelBtn').classList.add('active');
    else if (mode === 'Canal direito') document.getElementById('rightChannelBtn').classList.add('active');
    else if (mode === 'Ambos os canais') document.getElementById('bothChannelsBtn').classList.add('active');
}
...
function stopAll() {
    ...
    playStopText.innerText = 'Iniciar a reprodução';
    audioStatusDisplay.innerText = 'Parou';
...
function startOscillator(freq) {
    ...
    playStopText.innerText = 'Parar a reprodução';
    currentFrequencyDisplay.innerText = freq + ' Hz';
    audioStatusDisplay.innerText = 'Reprodução de onda senoidal';
}
...
function startSweep() {
    ...
    audioStatusDisplay.innerText = 'Varrendo';
    ...
    oscillator.onended = function() {
        stopAll();
        currentFrequencyDisplay.innerText = '20.000Hz';
        audioStatusDisplay.innerText = 'Varredura concluída';
        setTimeout(() => {
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = 'Parar a reprodução';
    currentFrequencyDisplay.innerText = type === 'white' ? 'Ruído Branco' : 'Ruído Rosa';
    audioStatusDisplay.innerText = (type === 'white' ? 'White' : 'Pink') + 'Ruído tocando';
...
document.getElementById('leftChannelBtn').onclick = () => setChannelMode('Canal Esquerdo');
document.getElementById('bothChannelsBtn').onclick = () => setChannelMode('Ambos os canais');
document.getElementById('rightChannelBtn').onclick = () => setChannelMode('Canal direito');
