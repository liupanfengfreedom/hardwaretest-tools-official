...
let currentChannel = "كلتا القناتين"; // Default channel mode
...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = 'تفعيلها';
    }
...
function setChannelMode(mode) {
    ...
    if (mode === 'القناة اليسرى') document.getElementById('leftChannelBtn').classList.add('active');
    else if (mode === 'القناة الصحيحة') document.getElementById('rightChannelBtn').classList.add('active');
    else if (mode === 'كلتا القناتين') document.getElementById('bothChannelsBtn').classList.add('active');
}
...
function stopAll() {
    ...
    playStopText.innerText = 'ابدأ التشغيل';
    audioStatusDisplay.innerText = 'توقف';
...
function startOscillator(freq) {
    ...
    playStopText.innerText = 'إيقاف التشغيل';
    currentFrequencyDisplay.innerText = freq + ' Hz';
    audioStatusDisplay.innerText = 'اللعب بموجة جيبية';
}
...
function startSweep() {
    ...
    oscillator.onended = function() {
        stopAll();
        currentFrequencyDisplay.innerText = '20000 هرتز';
        audioStatusDisplay.innerText = 'اكتمال الاجتياح';
        setTimeout(() => {
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = 'إيقاف التشغيل';
    currentFrequencyDisplay.innerText = type === 'white' ? 'الضوضاء البيضاء' : 'الضوضاء الوردية';
    audioStatusDisplay.innerText = (type === 'white' ? 'White' : 'Pink') + 'اللعب بالضوضاء';
...
document.getElementById('leftChannelBtn').onclick = () => setChannelMode('القناة اليسرى');
document.getElementById('bothChannelsBtn').onclick = () => setChannelMode('كلتا القناتين');
document.getElementById('rightChannelBtn').onclick = () => setChannelMode('القناة الصحيحة');
