...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = '활성화됨';
    }
...
function stopAll() {
    ...
    playStopText.innerText = '재생 시작';
    audioStatusDisplay.innerText = '중지됨';
...
function startOscillator(freq) {
    ...
    if (isPlaying && oscillator && !isSweeping && !isNoisePlaying) {
        ...
        currentFrequencyDisplay.innerText = freq + ' 헤르츠';
        return;
    }
    ...
    playStopText.innerText = '재생 중지';
    currentFrequencyDisplay.innerText = freq + ' 헤르츠';
    audioStatusDisplay.innerText = '사인파 재생';
}
...
function startSweep() {
    ...
    audioStatusDisplay.innerText = '청소';
    ...
    sweepInterval = setInterval(() => {
        if (isSweeping && sweepStartTime) {
            ...
            if (elapsed < duration) {
                ...
                currentFrequencyDisplay.innerText = Math.round(currentFreq) + ' 헤르츠';
            }
        }
    ...
    oscillator.onended = function() {
        ...
        audioStatusDisplay.innerText = '스윕 완료';
        setTimeout(() => {
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = '재생 중지';
    currentFrequencyDisplay.innerText = type === 'white' ? '백색 잡음' : '핑크 노이즈';
    audioStatusDisplay.innerText = (type === 'white' ? '하얀색' : '분홍색') + ' 소음 재생';
...
document.querySelectorAll('.freq-btn').forEach(btn => {
    btn.onclick = function() {
        ...
        freqVal.innerText = this.dataset.freq + ' 헤르츠';
...
// Initialization
freqVal.innerText = freqSlider.value + ' 헤르츠';
