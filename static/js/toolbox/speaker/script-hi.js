...
function initAudio() {
    if (!audioCtx) {
        ...
        drawVisualizer();
        audioStatusDisplay.innerText = 'सक्रिय';
    }
...
function stopAll() {
    ...
    playStopText.innerText = 'प्लेबैक प्रारंभ करें';
    audioStatusDisplay.innerText = 'रुक गया';
...
function startOscillator(freq) {
    ...
    if (isPlaying && oscillator && !isSweeping && !isNoisePlaying) {
        ...
        currentFrequencyDisplay.innerText = freq + ' हर्ट्ज';
        return;
    }
    ...
    playStopText.innerText = 'प्लेबैक बंद करो';
    currentFrequencyDisplay.innerText = freq + ' हर्ट्ज';
    audioStatusDisplay.innerText = 'साइन वेव बजाना';
}
...
function startSweep() {
    ...
    sweepInterval = setInterval(() => {
        if (isSweeping && sweepStartTime) {
            ...
            if (elapsed < duration) {
                ...
                currentFrequencyDisplay.innerText = Math.round(currentFreq) + ' हर्ट्ज';
            }
        }
    ...
    oscillator.onended = function() {
        stopAll();
        currentFrequencyDisplay.innerText = '20000 हर्ट्ज';
...
function startNoise(type) {
    ...
    playStopBtn.classList.add('playing');
    playStopText.innerText = 'प्लेबैक बंद करो';
    currentFrequencyDisplay.innerText = type === 'white' ? 'श्वेत रव' : 'गुलाबी शोर';
    audioStatusDisplay.innerText = (type === 'white' ? 'सफ़ेद' : 'गुलाबी') + ' शोर बजाना';
...
document.querySelectorAll('.freq-btn').forEach(btn => {
    btn.onclick = function() {
        ...
        freqVal.innerText = this.dataset.freq + ' हर्ट्ज';
...
// Initialization
freqVal.innerText = freqSlider.value + ' हर्ट्ज';
