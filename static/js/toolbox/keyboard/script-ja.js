    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> 自動スクロール オン' : 
                '<i class="fas fa-scroll"></i> 自動スクロールオフ';
        }
    }
}
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> ログを表示';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> ログを隠す';
        }
    }
}
...
window.startIntervalTest = function() {
    ...
    testStartEntry.innerHTML = `\r\n        <span class="log-time">${timeStr}</span>\r\n        <span class="log-message">キー間隔テストの開始 - 同じキーを素早く繰り返し押してください</span>\r\n    `;
...
function createDiagnosisPanel(steps) {
    ...
    panel.innerHTML = `\r\n        <div class="diagnosis-header">\r\n            <h3><i class="fas fa-stethoscope"></i> キーボード診断モード</h3>\r\n            <button class="close-diagnosis"><i class="fas fa-times"></i></button>\r\n        </div>\r\n        <div class="diagnosis-progress">\r\n            <div class="progress-bar"><div class="progress-fill" style="width: 20%;"></div></div>\r\n            <div class="progress-text">ステップ 1/${steps.length}</div>\r\n        </div>\r\n        <div class="diagnosis-content">\r\n            <div class="step-current">\r\n                <h4 id="step-title">${steps[0].title}</h4>\r\n                <p id="step-instruction">${steps[0].instruction}</p>\r\n                <div class="step-requirements">\r\n                    <strong>チェックポイント:</strong> ${steps[0].check}<br>\r\n                    <small><i class="far fa-clock"></i> 推奨時間: ${steps[0].time} 秒</small>\r\n                </div>\r\n            </div>\r\n            <div class="step-results" id="step-results"></div>\r\n        </div>\r\n        <div class="diagnosis-actions">\r\n            <button class="btn-step" id="prev-step"><i class="fas fa-arrow-left"></i> 前</button>\r\n            <button class="btn-step" id="next-step">次へ <i class="fas fa-arrow-right"></i></button>\r\n            <button class="btn-complete" id="complete-diagnosis">完全な診断とレポートの生成</button>\r\n        </div>\r\n    `;
    ...
    function updateProgress() {
        ...
        document.querySelector('.progress-text').textContent = `ステップ ${currentStep + 1}/${steps.length}`;
        ...
        document.querySelector('.step-requirements').innerHTML = `\r\n            <strong>チェックポイント:</strong> ${steps[currentStep].check}<br>\r\n            <small><i class="far fa-clock"></i> 推奨時間: ${steps[currentStep].time} 秒</small>\r\n        `;
    }
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('診断モードを終了してもよろしいですか?不完全な結果は保存されません。')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
