...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 Совет: быстро нажмите одну и ту же клавишу, чтобы проверить интервалы клавиш. Интервалы <80 мс отмечены красным.', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'Левый сдвиг', 'ShiftRight': 'Правый сдвиг', 'Backspace': 'Backspace',
    'ControlLeft': 'Левый Ctrl', 'ControlRight': 'Правый Ctrl', 'AltLeft': 'Левый Alt', 'AltRight': 'Правый Alt',
    'Tab': 'Tab', 'CapsLock': 'Капс Лок', 'Escape': 'Esc', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'Страница вверх', 'PageDown': 'Страница вниз', 'NumLock': 'Num Lock',
    'ScrollLock': 'Блокировка прокрутки', 'Pause': 'Pause', 'PrintScreen': 'Принтскрин', 'ContextMenu': 'Меню'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> Автоматическая прокрутка включена' : 
                '<i class="fas fa-scroll"></i> Автоматическая прокрутка выключена';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('Серьезное двойное нажатие', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('Умеренное двойное нажатие', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('Незначительное двойное нажатие', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('Слабая защита от ореолов', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('Отличная производительность NKRO', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('Залипание ключей', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('Данные испытаний, результаты диагностики и журналы были сброшены.', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 Совет: быстро нажмите одну и ту же клавишу, чтобы проверить интервалы клавиш. Интервалы <80 мс отмечены красным.",
        "💡Совет: нормальные интервалы работы человека обычно составляют >100 мс, у геймеров они могут достигать 80–120 мс.",
        "💡 Совет. Частые интервалы менее 50 мс могут указывать на ошибки двойного нажатия на клавиатуру.",
        "💡 Совет. Попробуйте одновременно нажать несколько клавиш, чтобы проверить работу функции защиты от ореолов.",
        "💡 Совет: проверяйте все клавиши последовательно, особенно часто используемые клавиши, такие как WASD и пробел.",
        "💡 Совет: обратите внимание, совпадают ли значения прямого и обратного счета. Несоответствие может указывать на залипание ключа."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'Серьезная неисправность двойного крана, требуется немедленный ремонт';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'Умеренная ошибка двойного касания, устраните ее в ближайшее время';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'Незначительное двойное нажатие, внимательно следите';
    } else {
        logLevel = 'normal';
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> Показать журнал';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> Скрыть журнал';
        }
    }
}
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'Автоматическая прокрутка включена' : 'Автоматическая прокрутка отключена', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('Запуск проверки интервала между клавишами: быстро нажмите ту же клавишу (например, клавишу W или пробел).', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">Запуск теста интервала между клавишами. Быстро нажмите одну и ту же клавишу несколько раз.</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 Шаг 1: Проверка функции одной клавиши",
            instruction: "Пожалуйста, протестируйте следующие группы клавиш последовательно, чтобы убедиться в отсутствии не отвечающих клавиш:<br>1. Клавиши направления WASD<br>2. Цифровые клавиши 1-5<br>3. Клавиши со стрелками ↑↓ ←→<br>4. Клавиши пробела и ввода",
            action: "testAllKeys",
            check: "Проверьте, становится ли каждая клавиша белой при нажатии",
            time: 60
        },
        {
            title: "⏱️ Шаг 2: Определение ключевого интервала",
            instruction: "Быстро нажмите одну и ту же клавишу 10 раз (рекомендуется проверить клавишу W и пробел), наблюдайте за выводом журнала.",
            action: "testInterval",
            check: "Соблюдайте ключевые интервалы. В норме должно быть >80 мс. Интервалы <80 мс отмечены красным.",
            time: 30
        },
        {
            title: "🎮 Шаг 3: Тест NKRO на защиту от призраков",
            instruction: "Нажмите всей рукой на среднюю часть клавиатуры, одновременно нажимая как можно больше клавиш.",
            action: "testNKRO",
            check: "Обратите внимание на значение «Текущий параллелизм». Обычные клавиатуры должны иметь ≥6, игровые клавиатуры могут достигать 10+.",
            time: 20
        },
        {
            title: "⚡ Шаг 4: Проверка задержки ответа",
            instruction: "Нажимайте разные клавиши на умеренной скорости, наблюдайте за временем отклика",
            action: "testResponseTime",
            check: "Время интервала должно быть стабильным в пределах 50-200 мс без больших колебаний.",
            time: 30
        },
        {
            title: "🔄 Шаг 5: Обнаружение ключевых конфликтов",
            instruction: "Нажмите одновременно: W+A+Shift+Пробел<br>Затем: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "Все клавиши должны подсветиться одновременно, ни одна не должна выйти из строя.",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'Ключи еще не проверены');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'Обнаружены серьезные проблемы с двойным касанием (интервалы <20 мс).');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'Обнаружены незначительные проблемы с двойным касанием (интервалы <80 мс).');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'Интервал между клавишами нормальный, двойное нажатие не обнаружено');
                } else {
                    addStepResult('info', 'Ключевой интервальный тест еще не выполнен');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'Тест на задержку ответа пройден, судите по опыту использования');
                } else {
                    addStepResult('info', 'Пожалуйста, проверьте больше клавиш, чтобы оценить задержку ответа.');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('Вы уверены, что хотите выйти из режима диагностики? Неполные результаты не будут сохранены.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
    ...
    if (criticalIntervals > 0) {
        issues.push({
            type: 'Серьезное двойное нажатие',
            severity: 'error',
    ...
    if (warningIntervals > 0) {
        warnings.push({
            type: 'Незначительное двойное нажатие',
            severity: 'warning',
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">Серьезное двойное нажатиеs</span>
    ...
    window.saveReportAsImage = function() {
        showToast('Скриншот отчета сохранен в буфер обмена (имитированная функция)', 'info');
