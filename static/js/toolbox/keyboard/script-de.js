...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 Tipp: Drücken Sie schnell dieselbe Taste, um die Tastenintervalle zu testen. Intervalle <80ms sind rot markiert.', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'Linksverschiebung', 'ShiftRight': 'Rechtsverschiebung', 'Backspace': 'Backspace',
    'ControlLeft': 'Linke Strg-Taste', 'ControlRight': 'Rechte Strg-Taste', 'AltLeft': 'Links Alt', 'AltRight': 'Rechts Alt',
    'Tab': 'Tab', 'CapsLock': 'Feststelltaste', 'Escape': 'Esc', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'Seite nach oben', 'PageDown': 'Seite nach unten', 'NumLock': 'Num Lock',
    'ScrollLock': 'Scroll-Sperre', 'Pause': 'Pause', 'PrintScreen': 'Bildschirm drucken', 'ContextMenu': 'Speisekarte'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> Automatisches Scrollen ein' : 
                '<i class="fas fa-scroll"></i> Automatisches Scrollen aus';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('Schweres Doppeltippen', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('Moderates Doppeltippen', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('Leichter Doppeltipp', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('Schwaches Anti-Ghosting', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('Hervorragende NKRO-Leistung', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('Schlüssel kleben', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('Testdaten, Diagnoseergebnisse und Protokolle wurden zurückgesetzt', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 Tipp: Drücken Sie schnell dieselbe Taste, um die Tastenintervalle zu testen. Intervalle <80ms sind rot markiert.",
        "💡 Tipp: Normale menschliche Bedienintervalle liegen typischerweise bei >100 ms, Gamer können 80–120 ms erreichen.",
        "💡 Tipp: Häufige Intervalle <50 ms können auf Fehler beim Doppeltippen der Tastatur hinweisen.",
        "💡 Tipp: Versuchen Sie, mehrere Tasten gleichzeitig zu drücken, um die Anti-Ghosting-Funktion zu testen.",
        "💡 Tipp: Testen Sie alle Tasten nacheinander, insbesondere häufig verwendete Tasten wie WASD und Leertaste.",
        "💡 Tipp: Beobachten Sie, ob die Abwärts-/Aufwärtszählungen übereinstimmen. Eine Nichtübereinstimmung kann darauf hindeuten, dass der Schlüssel feststeckt."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'Schwerwiegender Double-Tap-Fehler, sofortige Reparatur erforderlich';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'Mäßiger Doppeltippfehler, bald beheben';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'Leichtes Doppeltippen, genau beobachten';
    } else {
        logLevel = 'normal';
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> Protokoll anzeigen';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> Protokoll ausblenden';
        }
    }
}
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'Automatischer Bildlauf aktiviert' : 'Automatischer Bildlauf deaktiviert', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('Tastenintervalltest starten: Dieselbe Taste schnell drücken (z. B. W-Taste oder Leertaste)', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">Tastenintervalltest starten - Bitte drücken Sie wiederholt schnell dieselbe Taste</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 Schritt 1: Einzeltastenfunktionstest",
            instruction: "Bitte testen Sie die folgenden Schlüsselgruppen nacheinander, um nach nicht reagierenden Schlüsseln zu suchen:<br>1. WASD-Richtungstasten<br>2. Zifferntasten 1-5<br>3. Pfeiltasten ↑↓←→<br>4. Leertaste und Eingabetaste",
            action: "testAllKeys",
            check: "Überprüfen Sie, ob jede Taste beim Drücken weiß wird",
            time: 60
        },
        {
            title: "⏱️ Schritt 2: Schlüsselintervallerkennung",
            instruction: "Drücken Sie schnell zehnmal dieselbe Taste (empfehlen Sie, die W-Taste und die Leertaste zu testen) und beobachten Sie die Protokollausgabe",
            action: "testInterval",
            check: "Wichtige Intervalle beachten. Normal sollte >80ms sein. Intervalle <80ms sind rot markiert",
            time: 30
        },
        {
            title: "🎮 Schritt 3: NKRO Anti-Ghosting-Test",
            instruction: "Drücken Sie mit Ihrer gesamten Hand auf den mittleren Tastaturbereich und drücken Sie dabei möglichst viele Tasten gleichzeitig",
            action: "testNKRO",
            check: "Beobachten Sie den Wert „Aktuelle Parallelität“. Normale Tastaturen sollten ≥6 haben, Gaming-Tastaturen können 10+ erreichen",
            time: 20
        },
        {
            title: "⚡ Schritt 4: Reaktionsverzögerungstest",
            instruction: "Drücken Sie verschiedene Tasten mit mäßiger Geschwindigkeit und achten Sie auf die Reaktionszeit",
            action: "testResponseTime",
            check: "Die Intervallzeiten sollten ohne große Schwankungen stabil zwischen 50 und 200 ms liegen",
            time: 30
        },
        {
            title: "🔄 Schritt 5: Schlüsselkonflikterkennung",
            instruction: "Gleichzeitig drücken: W+A+Umschalt+Leertaste<br>Dann: Strg+Umschalt+Alt",
            action: "testKeyConflict",
            check: "Alle Tasten sollten gleichzeitig hervorgehoben werden, keine sollte fehlschlagen",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'Noch keine Schlüssel getestet');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'Schwerwiegende Double-Tap-Probleme festgestellt (Intervalle <20 ms)');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'Kleinere Double-Tap-Probleme festgestellt (Intervalle <80 ms)');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'Tastenabstände normal, kein Doppeltipp erkannt');
                } else {
                    addStepResult('info', 'Schlüsselintervalltest noch nicht durchgeführt');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'Antwortverzögerungstest abgeschlossen, Beurteilung anhand der Nutzungserfahrung');
                } else {
                    addStepResult('info', 'Bitte testen Sie weitere Schlüssel, um die Antwortverzögerung zu bewerten');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('Möchten Sie den Diagnosemodus wirklich verlassen? Unvollständige Ergebnisse werden nicht gespeichert.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">Schweres Doppeltippens</span>
    ...
    window.saveReportAsImage = function() {
        showToast('Berichts-Screenshot in Zwischenablage gespeichert (simulierte Funktion)', 'info');
