...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('Serious double-tap', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('Middle double-tap', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('Small double-tap', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('Key dey stick', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('Test data, diagnosis result, and logs don reset finish', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        ...
        "💡 Tip: Normal human key interval usually pass 100ms, gamers fit reach 80-120ms.",
        "💡 Tip: Frequent &lt;50ms intervals may indicate keyboard double-tap faults.",
        "💡 Tip: Try press many keys together make you test anti-ghosting power.",
        "💡 Tip: Test all keys one by one, especially keys wey people use pass like WASD and Spacebar.",
        "💡 Tip: Check whether down/up count match. If dem no match, key fit dey stick."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'Serious double-tap fault, e need repair sharp-sharp';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'Middle double-tap fault, solve am soon';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'Small double-tap, watch am closely';
    } else {
        logLevel = 'normal';
...
window.clearLog = function() {
    ...
    if (logContent.children.length <= 2) return; // Keep initial two messages
        ...
        updateLogStats();
        showToast('Log don clear', 'success');
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'Auto scroll don turn on' : 'Auto scroll don turn off', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('Key interval test dey start: press the same key fast-fast, like W key or Spacebar.', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">Key interval test don start - abeg press the same key fast-fast again and again</span>
    `;
    ...
    const diagnosticSteps = [
        {
            ...
            instruction: "Abeg test these key groups one by one make you check whether any key no respond:<br>1. WASD direction keys<br>2. Number keys 1-5<br>3. Arrow keys ↑↓←→<br>4. Spacebar and Enter keys",
            action: "testAllKeys",
            check: "Check whether each key turn white when you press am",
            time: 60
        },
        {
            title: "⏱️ Step 2: Key interval detection",
            instruction: "Press the same key 10 times fast-fast, wey recommend W key and Spacebar, then watch the log output.",
            action: "testInterval",
        ...
        },
        {
            ...
            instruction: "Press your full hand for the middle keyboard area make you press plenty keys together.",
            action: "testNKRO",
            check: "Check the 'Current Concurrency' value. Normal keyboard suppose get 6 or pass, gaming keyboard fit reach 10+.",
            time: 20
        },
        {
            title: "⚡ Step 4: Response delay test",
            instruction: "Press different keys for normal speed and watch the response time.",
            action: "testResponseTime",
            check: "Interval time suppose steady between 50-200ms without big jump.",
            time: 30
        },
        {
            title: "🔄 Step 5: Key conflict detection",
            instruction: "Press together: W+A+Shift+Space<br>Then: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "All keys suppose highlight together, none suppose fail.",
            time: 30
        }
    ];
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">Serious double-taps</span>
