...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 Consejo: presione rápidamente la misma tecla para probar los intervalos de las teclas. Los intervalos <80 ms están marcados en rojo.', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'Desplazamiento a la izquierda', 'ShiftRight': 'Desplazamiento a la derecha', 'Backspace': 'Backspace',
    'ControlLeft': 'Ctrl izquierdo', 'ControlRight': 'Ctrl derecho', 'AltLeft': 'Alt izquierda', 'AltRight': 'Alt derecha',
    'Tab': 'Tab', 'CapsLock': 'Bloq Mayús', 'Escape': 'ESC', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'Página arriba', 'PageDown': 'Página abajo', 'NumLock': 'Bloqueo numérico',
    'ScrollLock': 'Bloqueo de desplazamiento', 'Pause': 'Pause', 'PrintScreen': 'Imprimir pantalla', 'ContextMenu': 'Menú'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> Desplazamiento automático activado' : 
                '<i class="fas fa-scroll"></i> Desplazamiento automático desactivado';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('Doble toque severo', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('Doble toque moderado', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('Doble toque menor', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('Anti-fantasma débil', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('Excelente desempeño de NKRO', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('Clave pegada', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('Se han restablecido los datos de prueba, los resultados de diagnóstico y los registros.', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 Consejo: presione rápidamente la misma tecla para probar los intervalos de las teclas. Los intervalos <80 ms están marcados en rojo.",
        "💡 Consejo: Los intervalos normales de operación humana suelen ser >100 ms, los jugadores pueden alcanzar 80-120 ms.",
        "💡 Consejo: intervalos frecuentes de <50 ms pueden indicar fallas al tocar dos veces el teclado.",
        "💡 Consejo: Intente presionar varias teclas simultáneamente para probar la capacidad anti-ghosting.",
        "💡 Consejo: Pruebe todas las teclas de forma secuencial, especialmente las teclas de uso frecuente como WASD y la barra espaciadora.",
        "💡 Consejo: observe si los recuentos ascendentes y descendentes coinciden. La falta de coincidencia puede indicar que la clave se atasca."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'Fallo grave de doble toque, se necesita reparación inmediata';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'Fallo moderado de doble toque, solucionarlo pronto';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'Toque dos veces menor, supervise de cerca';
    } else {
        logLevel = 'normal';
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> Mostrar registro';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> Ocultar registro';
        }
    }
}
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'Desplazamiento automático habilitado' : 'Desplazamiento automático desactivado', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('Iniciar la prueba de intervalo de teclas: presione rápidamente la misma tecla (por ejemplo, la tecla W o la barra espaciadora)', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">Iniciar prueba de intervalo de tecla: presione rápidamente la misma tecla repetidamente</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 Paso 1: Prueba de función de una sola tecla",
            instruction: "Pruebe los siguientes grupos de claves secuencialmente para comprobar si hay claves que no responden:<br>1. Teclas direccionales WASD<br>2. Teclas numéricas 1-5<br>3. Teclas de flecha ↑↓←→<br>4. Barra espaciadora y teclas Enter",
            action: "testAllKeys",
            check: "Compruebe si cada tecla se vuelve blanca cuando se presiona",
            time: 60
        },
        {
            title: "⏱️ Paso 2: Detección de intervalos clave",
            instruction: "Presione rápidamente la misma tecla 10 veces (se recomienda probar la tecla W y la barra espaciadora), observe la salida del registro",
            action: "testInterval",
            check: "Observe los intervalos clave. Lo normal debe ser >80 ms. Los intervalos <80 ms están marcados en rojo",
            time: 30
        },
        {
            title: "🎮 Paso 3: Prueba anti-fantasma de NKRO",
            instruction: "Presione toda su mano en el área central del teclado, presionando tantas teclas como sea posible simultáneamente",
            action: "testNKRO",
            check: "Observe el valor de 'Simultaneidad actual'. Los teclados normales deben tener ≥6, los teclados para juegos pueden llegar a 10+",
            time: 20
        },
        {
            title: "⚡ Paso 4: Prueba de retraso de respuesta",
            instruction: "Presione diferentes teclas a velocidad moderada, observe el tiempo de respuesta",
            action: "testResponseTime",
            check: "Los tiempos de intervalo deben ser estables entre 50 y 200 ms sin grandes fluctuaciones",
            time: 30
        },
        {
            title: "🔄 Paso 5: Detección de conflictos clave",
            instruction: "Presione simultáneamente: W+A+Shift+Espacio<br>Luego: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "Todas las teclas deben resaltarse simultáneamente, ninguna debe fallar",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'Aún no se han probado claves');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'Se detectaron problemas graves de doble toque (intervalos <20 ms)');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'Se detectaron problemas menores al tocar dos veces (intervalos <80 ms)');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'Intervalos de teclas normales, no se detecta doble toque');
                } else {
                    addStepResult('info', 'Prueba de intervalo clave aún no realizada');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'Prueba de retraso de respuesta completada, juzgue según la experiencia de uso');
                } else {
                    addStepResult('info', 'Pruebe más claves para evaluar el retraso de la respuesta');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('¿Está seguro de que desea salir del modo de diagnóstico? Los resultados incompletos no se guardarán.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">Doble toque severos</span>
    ...
    window.saveReportAsImage = function() {
        showToast('Captura de pantalla del informe guardada en el portapapeles (función simulada)', 'info');
