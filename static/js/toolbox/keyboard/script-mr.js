...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 टीप: की अंतराल तपासण्यासाठी तीच की वेगाने दाबा. अंतराल <80ms लाल रंगात चिन्हांकित केले आहेत.', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'डावी शिफ्ट', 'ShiftRight': 'उजवीकडे शिफ्ट', 'Backspace': 'Backspace',
    'ControlLeft': 'डावीकडे Ctrl', 'ControlRight': 'उजवे Ctrl', 'AltLeft': 'डावीकडे Alt', 'AltRight': 'उजवे Alt',
    'Tab': 'Tab', 'CapsLock': 'कॅप्स लॉक', 'Escape': 'Esc', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'पृष्ठ वर', 'PageDown': 'पृष्ठ खाली', 'NumLock': 'नंबर लॉक',
    'ScrollLock': 'स्क्रोल लॉक', 'Pause': 'Pause', 'PrintScreen': 'प्रिंट स्क्रीन', 'ContextMenu': 'मेनू'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> स्वयं स्क्रोल चालू' : 
                '<i class="fas fa-scroll"></i> स्वयं स्क्रोल बंद';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('गंभीर डबल-टॅप', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('मध्यम दोनदा टॅप करा', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('किरकोळ डबल-टॅप', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('कमकुवत अँटी-गोस्टिंग', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('उत्कृष्ट NKRO कामगिरी', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('की स्टिकिंग', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('चाचणी डेटा, निदान परिणाम आणि लॉग रीसेट केले गेले आहेत', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 टीप: की अंतराल तपासण्यासाठी तीच की वेगाने दाबा. अंतराल <80ms लाल रंगात चिन्हांकित केले आहेत.",
        "💡 टीप: सामान्य मानवी ऑपरेशन मध्यांतर साधारणत: 100ms पेक्षा जास्त असते, गेमर 80-120ms पर्यंत पोहोचू शकतात.",
        "💡 टीप: वारंवार <50ms अंतराल कीबोर्ड डबल-टॅप दोष दर्शवू शकतात.",
        "💡 टीप: अँटी-गोस्टिंग क्षमता तपासण्यासाठी एकाच वेळी अनेक की दाबण्याचा प्रयत्न करा.",
        "💡 टीप: सर्व की क्रमाक्रमाने तपासा, विशेषत: WASD आणि Spacebar सारख्या वारंवार वापरल्या जाणाऱ्या की.",
        "💡 टीप: खाली/वर मोजणी जुळत आहेत का ते पहा. न जुळणे हे की स्टिकिंग दर्शवू शकते."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'गंभीर डबल-टॅप दोष, त्वरित दुरुस्ती आवश्यक आहे';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'मध्यम डबल-टॅप दोष, लवकरच पत्ता';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'किरकोळ डबल-टॅप करा, लक्षपूर्वक निरीक्षण करा';
    } else {
        logLevel = 'normal';
...
window.clearLog = function() {
    ...
    if (logContent.children.length <= 2) return; // Keep initial two messages
        ...
        updateLogStats();
        showToast('लॉग साफ केला', 'success');
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> लॉग दाखवा';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> लॉग लपवा';
        }
    }
}
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'स्वयं स्क्रोल सक्षम केले' : 'ऑटो स्क्रोल अक्षम', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('की अंतराल चाचणी सुरू करणे: तीच की वेगाने दाबा (उदा. W की किंवा स्पेसबार)', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">की मध्यांतर चाचणी सुरू करत आहे - कृपया तीच की वारंवार दाबा</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 पायरी 1: सिंगल की फंक्शन टेस्ट",
            instruction: "नॉन-रिस्पॉन्सिव्ह की तपासण्यासाठी कृपया खालील की गटांची क्रमवार चाचणी करा:<br>1. WASD दिशात्मक की<br>2. नंबर की 1-5<br>3. बाण की ↑↓←→<br>४. स्पेसबार आणि एंटर की",
            action: "testAllKeys",
            check: "दाबल्यावर प्रत्येक कळ पांढरी होते का ते तपासा",
            time: 60
        },
        {
            title: "⏱️ पायरी 2: की इंटरव्हल डिटेक्शन",
            instruction: "तीच की 10 वेळा वेगाने दाबा (डब्ल्यू की आणि स्पेसबारची चाचणी करण्याची शिफारस करा), लॉग आउटपुटचे निरीक्षण करा",
            action: "testInterval",
            check: "मुख्य अंतराल पहा. सामान्य >80ms असावे. अंतराल <80ms लाल चिन्हांकित केले आहेत",
            time: 30
        },
        {
            title: "🎮 पायरी 3: NKRO अँटी-गोस्टिंग चाचणी",
            instruction: "एकाच वेळी शक्य तितक्या की दाबून, मध्यम कीबोर्ड क्षेत्रावर तुमचा संपूर्ण हात दाबा",
            action: "testNKRO",
            check: "'करंट कॉन्करन्सी' मूल्याचे निरीक्षण करा. सामान्य कीबोर्डमध्ये ≥6 असणे आवश्यक आहे, गेमिंग कीबोर्ड 10+ पर्यंत पोहोचू शकतात",
            time: 20
        },
        {
            title: "⚡ पायरी 4: प्रतिसाद विलंब चाचणी",
            instruction: "मध्यम वेगाने भिन्न की दाबा, प्रतिसाद वेळ पहा",
            action: "testResponseTime",
            check: "मोठ्या चढउतारांशिवाय मध्यांतर वेळ 50-200ms दरम्यान स्थिर असावी",
            time: 30
        },
        {
            title: "🔄 पायरी 5: की संघर्ष ओळख",
            instruction: "एकाच वेळी दाबा: W+A+Shift+Space<br>नंतर: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "सर्व की एकाच वेळी हायलाइट केल्या पाहिजेत, कोणतीही अयशस्वी होऊ नये",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'अद्याप कोणत्याही कळा तपासल्या नाहीत');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'गंभीर डबल-टॅप समस्या आढळल्या (मांतर <20ms)');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'किरकोळ डबल-टॅप समस्या आढळल्या (मध्यांतर <80ms)');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'की अंतराल सामान्य, कोणतेही डबल-टॅप आढळले नाही');
                } else {
                    addStepResult('info', 'मुख्य अंतराल चाचणी अद्याप पूर्ण झालेली नाही');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'प्रतिसाद विलंब चाचणी पूर्ण झाली, वापराच्या अनुभवावर आधारित न्याय करा');
                } else {
                    addStepResult('info', 'कृपया प्रतिसाद विलंबाचे मूल्यांकन करण्यासाठी अधिक की तपासा');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('तुम्हाला खात्री आहे की तुम्ही डायग्नोस्टिक मोडमधून बाहेर पडू इच्छिता? अपूर्ण परिणाम जतन केले जाणार नाहीत.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">गंभीर डबल-टॅपs</span>
    ...
    window.saveReportAsImage = function() {
        showToast('रिपोर्ट स्क्रीनशॉट क्लिपबोर्डवर सेव्ह केला (सिम्युलेटेड फंक्शन)', 'info');
