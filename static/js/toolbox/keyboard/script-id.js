...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 Tip: Tekan cepat tombol yang sama untuk menguji interval tombol. Interval <80ms ditandai dengan warna merah.', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'Pergeseran Kiri', 'ShiftRight': 'Pergeseran Kanan', 'Backspace': 'Backspace',
    'ControlLeft': 'Ctrl Kiri', 'ControlRight': 'Ctrl kanan', 'AltLeft': 'Alt Kiri', 'AltRight': 'Alt Kanan',
    'Tab': 'Tab', 'CapsLock': 'Kunci huruf kapital', 'Escape': 'Esc', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'Halaman Atas', 'PageDown': 'Halaman Bawah', 'NumLock': 'Kunci Angka',
    'ScrollLock': 'Kunci Gulir', 'Pause': 'Pause', 'PrintScreen': 'Layar Cetak', 'ContextMenu': 'Menu'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> Gulir Otomatis Aktif' : 
                '<i class="fas fa-scroll"></i> Gulir Otomatis Mati';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('Ketuk Dua Kali Parah', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('Ketuk Dua Kali Sedang', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('Ketuk Dua Kali Kecil', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('Anti-Ghosting yang Lemah', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('Performa NKRO Luar Biasa', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('Penempelan Kunci', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('Data pengujian, hasil diagnosis, dan log telah diatur ulang', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 Tip: Tekan cepat tombol yang sama untuk menguji interval tombol. Interval <80ms ditandai dengan warna merah.",
        "💡 Tips: Interval pengoperasian normal manusia biasanya >100 md, gamer dapat mencapai 80-120 md.",
        "💡 Tip: Interval <50 ms yang sering mungkin menunjukkan kesalahan ketuk dua kali pada keyboard.",
        "💡 Tip: Coba tekan beberapa tombol secara bersamaan untuk menguji kemampuan anti-ghosting.",
        "💡 Tip: Uji semua tombol secara berurutan, terutama tombol yang sering digunakan seperti WASD dan Spacebar.",
        "💡 Tip: Amati apakah hitungan turun/naik cocok. Ketidakcocokan mungkin mengindikasikan kunci menempel."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'Kesalahan ketuk dua kali parah, perlu perbaikan segera';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'Kesalahan ketuk dua kali sedang, segera atasi';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'Ketuk dua kali kecil, pantau dengan cermat';
    } else {
        logLevel = 'normal';
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> Tampilkan Log';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> Sembunyikan Log';
        }
    }
}
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'Gulir otomatis diaktifkan' : 'Gulir otomatis dinonaktifkan', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('Tes interval tombol awal: Tekan cepat tombol yang sama (misalnya, tombol W atau Spasi)', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">Memulai tes interval kunci - Tekan dengan cepat tombol yang sama berulang kali</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 Langkah 1: Uji Fungsi Tombol Tunggal",
            instruction: "Silakan uji grup kunci berikut secara berurutan untuk memeriksa kunci non-responsif:<br>1. Tombol arah WASD<br>2. Tombol angka 1-5<br>3. Tombol panah ↑↓←→<br>4. Spasi dan tombol Enter",
            action: "testAllKeys",
            check: "Periksa apakah setiap tombol berubah warna menjadi putih saat ditekan",
            time: 60
        },
        {
            title: "⏱️ Langkah 2: Deteksi Interval Kunci",
            instruction: "Tekan cepat tombol yang sama sebanyak 10 kali (rekomendasikan pengujian tombol W dan Spasi), amati keluaran log",
            action: "testInterval",
            check: "Amati interval kunci. Normalnya harus >80ms. Interval <80ms ditandai dengan warna merah",
            time: 30
        },
        {
            title: "🎮 Langkah 3: Tes Anti-Ghosting NKRO",
            instruction: "Tekan seluruh tangan Anda pada area tengah keyboard, tekan tombol sebanyak mungkin secara bersamaan",
            action: "testNKRO",
            check: "Amati nilai 'Konkurensi Saat Ini'. Keyboard normal seharusnya memiliki ≥6, keyboard gaming bisa mencapai 10+",
            time: 20
        },
        {
            title: "⚡ Langkah 4: Tes Penundaan Respons",
            instruction: "Tekan tombol yang berbeda dengan kecepatan sedang, amati waktu respons",
            action: "testResponseTime",
            check: "Waktu interval harus stabil antara 50-200 ms tanpa fluktuasi yang besar",
            time: 30
        },
        {
            title: "🔄 Langkah 5: Deteksi Konflik Utama",
            instruction: "Tekan secara bersamaan: W+A+Shift+Spasi<br>Kemudian: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "Semua kunci harus disorot secara bersamaan, tidak ada yang gagal",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'Belum ada kunci yang diuji');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'Terdeteksi masalah ketuk dua kali yang parah (interval <20 md)');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'Terdeteksi masalah ketuk dua kali kecil (interval <80 md)');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'Interval tombol normal, tidak ada ketukan dua kali yang terdeteksi');
                } else {
                    addStepResult('info', 'Tes interval kunci belum dilakukan');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'Tes penundaan respons selesai, nilai berdasarkan pengalaman penggunaan');
                } else {
                    addStepResult('info', 'Silakan uji lebih banyak kunci untuk mengevaluasi penundaan respons');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('Apakah Anda yakin ingin keluar dari mode diagnostik? Hasil yang tidak lengkap tidak akan disimpan.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">Ketuk Dua Kali Parahs</span>
    ...
    window.saveReportAsImage = function() {
        showToast('Laporkan tangkapan layar yang disimpan ke clipboard (fungsi simulasi)', 'info');
