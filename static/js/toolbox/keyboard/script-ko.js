    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> 자동 스크롤 켜기' : 
                '<i class="fas fa-scroll"></i> 자동 스크롤 끄기';
        }
    }
}
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> 로그 표시';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> 로그 숨기기';
        }
    }
}
...
window.startIntervalTest = function() {
    ...
    testStartEntry.innerHTML = `\r\n        <span class="log-time">${timeStr}</span>\r\n        <span class="log-message">키 간격 테스트 시작 - 같은 키를 빠르게 반복해서 눌러주세요</span>\r\n    `;
...
function createDiagnosisPanel(steps) {
    ...
    panel.innerHTML = `\r\n        <div class="diagnosis-header">\r\n            <h3><i class="fas fa-stethoscope"></i> 키보드 진단 모드</h3>\r\n            <button class="close-diagnosis"><i class="fas fa-times"></i></button>\r\n        </div>\r\n        <div class="diagnosis-progress">\r\n            <div class="progress-bar"><div class="progress-fill" style="width: 20%;"></div></div>\r\n            <div class="progress-text">1단계/${steps.length}</div>\r\n        </div>\r\n        <div class="diagnosis-content">\r\n            <div class="step-current">\r\n                <h4 id="step-title">${steps[0].title}</h4>\r\n                <p id="step-instruction">${steps[0].instruction}</p>\r\n                <div class="step-requirements">\r\n                    <strong>체크 포인트:</strong> ${steps[0].check}<br>\r\n                    <small><i class="far fa-clock"></i> 권장 시간: ${steps[0].time}초</small>\r\n                </div>\r\n            </div>\r\n            <div class="step-results" id="step-results"></div>\r\n        </div>\r\n        <div class="diagnosis-actions">\r\n            <button class="btn-step" id="prev-step"><i class="fas fa-arrow-left"></i> 이전</button>\r\n            <button class="btn-step" id="next-step">다음 <i class="fas fa-arrow-right"></i></button>\r\n            <button class="btn-complete" id="complete-diagnosis">진단 완료 및 보고서 생성</button>\r\n        </div>\r\n    `;
    ...
    function updateProgress() {
        ...
        document.querySelector('.progress-text').textContent = `단계 ${currentStep + 1}/${steps.length}`;
        ...
        document.querySelector('.step-requirements').innerHTML = `\r\n            <strong>체크 포인트:</strong> ${steps[currentStep].check}<br>\r\n            <small><i class="far fa-clock"></i> 권장 시간: ${steps[currentStep].time}초</small>\r\n        `;
    }
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('진단 모드를 종료하시겠습니까? 불완전한 결과는 저장되지 않습니다.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
