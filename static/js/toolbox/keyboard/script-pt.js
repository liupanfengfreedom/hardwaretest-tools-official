...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 Dica: pressione rapidamente a mesma tecla para testar os intervalos das teclas. Intervalos <80ms estão marcados em vermelho.', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'Deslocamento Esquerdo', 'ShiftRight': 'Mudança para a direita', 'Backspace': 'Backspace',
    'ControlLeft': 'Ctrl esquerdo', 'ControlRight': 'Ctrl direito', 'AltLeft': 'Alt esquerdo', 'AltRight': 'Alt direito',
    'Tab': 'Tab', 'CapsLock': 'Caps Lock', 'Escape': 'Esc', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'Página para cima', 'PageDown': 'Página para baixo', 'NumLock': 'Num Lock',
    'ScrollLock': 'Bloqueio de rolagem', 'Pause': 'Pause', 'PrintScreen': 'Imprimir tela', 'ContextMenu': 'Menu'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> Rolagem automática ativada' : 
                '<i class="fas fa-scroll"></i> Rolagem automática desativada';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('Toque duplo grave', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('Toque duplo moderado', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('Toque duplo menor', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('Anti-fantasma fraco', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('Excelente desempenho NKRO', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('Chave colada', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('Dados de teste, resultados de diagnóstico e registros foram redefinidos', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 Dica: pressione rapidamente a mesma tecla para testar os intervalos das teclas. Intervalos <80ms estão marcados em vermelho.",
        "💡 Dica: Os intervalos normais de operação humana são normalmente >100ms, os jogadores podem atingir 80-120ms.",
        "💡 Dica: Intervalos frequentes <50 ms podem indicar falhas de toque duplo no teclado.",
        "💡 Dica: tente pressionar várias teclas simultaneamente para testar a capacidade anti-ghosting.",
        "💡 Dica: teste todas as teclas sequencialmente, especialmente as teclas usadas com frequência, como WASD e barra de espaço.",
        "💡 Dica: observe se as contagens decrescentes/aumentantes coincidem. A incompatibilidade pode indicar travamento da chave."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'Falha grave de toque duplo, reparo imediato necessário';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'Falha moderada de toque duplo, resolva em breve';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'Pequeno toque duplo, monitore de perto';
    } else {
        logLevel = 'normal';
...
window.clearLog = function() {
    ...
    if (logContent.children.length <= 2) return; // Keep initial two messages
        ...
        updateLogStats();
        showToast('Registro limpo', 'success');
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'Rolagem automática ativada' : 'Rolagem automática desativada', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('Iniciando teste de intervalo de teclas: pressione rapidamente a mesma tecla (por exemplo, tecla W ou barra de espaço)', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">Iniciando teste de intervalo de teclas - Pressione rapidamente a mesma tecla repetidamente</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 Etapa 1: teste de função de tecla única",
            instruction: "Teste os seguintes grupos de chaves sequencialmente para verificar se há chaves que não respondem:<br>1. Teclas direcionais WASD<br>2. Teclas numéricas de 1 a 5<br>3. Teclas de seta ↑↓←→<br>4. Barra de espaço e teclas Enter",
            action: "testAllKeys",
            check: "Verifique se cada tecla fica branca quando pressionada",
            time: 60
        },
        {
            title: "⏱️ Etapa 2: detecção de intervalo chave",
            instruction: "Pressione rapidamente a mesma tecla 10 vezes (recomendamos testar a tecla W e a barra de espaço), observe a saída do log",
            action: "testInterval",
            check: "Observe os intervalos principais. Normal deve ser >80ms. Intervalos <80ms são marcados em vermelho",
            time: 30
        },
        {
            title: "🎮 Etapa 3: Teste anti-ghosting NKRO",
            instruction: "Pressione toda a mão na área central do teclado, pressionando tantas teclas quanto possível simultaneamente",
            action: "testNKRO",
            check: "Observe o valor de 'Simultaneidade Atual'. Os teclados normais devem ter ≥6, os teclados para jogos podem chegar a 10+",
            time: 20
        },
        {
            title: "⚡ Etapa 4: teste de atraso de resposta",
            instruction: "Pressione teclas diferentes em velocidade moderada, observe o tempo de resposta",
            action: "testResponseTime",
            check: "Os tempos de intervalo devem ser estáveis ​​entre 50-200ms sem grandes flutuações",
            time: 30
        },
        {
            title: "🔄 Etapa 5: detecção de conflitos principais",
            instruction: "Pressione simultaneamente: W+A+Shift+Espaço<br>Então: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "Todas as teclas devem ser destacadas simultaneamente, nenhuma deve falhar",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'Nenhuma chave testada ainda');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'Detectados problemas graves de toque duplo (intervalos <20 ms)');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'Pequenos problemas de toque duplo detectados (intervalos <80 ms)');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'Intervalos de teclas normais, nenhum toque duplo detectado');
                } else {
                    addStepResult('info', 'Teste de intervalo chave ainda não realizado');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'Teste de atraso de resposta concluído, julgue com base na experiência de uso');
                } else {
                    addStepResult('info', 'Teste mais chaves para avaliar o atraso na resposta');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('Tem certeza de que deseja sair do modo de diagnóstico? Resultados incompletos não serão salvos.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">Toque duplo graves</span>
    ...
    window.saveReportAsImage = function() {
        showToast('Captura de tela do relatório salva na área de transferência (função simulada)', 'info');
