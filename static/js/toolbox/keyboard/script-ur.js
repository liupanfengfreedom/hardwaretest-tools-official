...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 ٹپ: کلیدی وقفوں کو جانچنے کے لیے تیزی سے ایک ہی کلید کو دبائیں۔ وقفے <80ms کو سرخ رنگ میں نشان زد کیا گیا ہے۔', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'بائیں شفٹ', 'ShiftRight': 'دائیں شفٹ', 'Backspace': 'Backspace',
    'ControlLeft': 'بائیں Ctrl', 'ControlRight': 'دائیں Ctrl', 'AltLeft': 'بائیں Alt', 'AltRight': 'دائیں Alt',
    'Tab': 'Tab', 'CapsLock': 'کیپس لاک', 'Escape': 'Esc', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'صفحہ اوپر', 'PageDown': 'صفحہ نیچے', 'NumLock': 'نمبر لاک',
    'ScrollLock': 'اسکرول لاک', 'Pause': 'Pause', 'PrintScreen': 'پرنٹ اسکرین', 'ContextMenu': 'مینو'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> آٹو اسکرول آن' : 
                '<i class="fas fa-scroll"></i> آٹو سکرول آف';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('شدید ڈبل تھپتھپائیں۔', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('اعتدال پسند ڈبل تھپتھپائیں۔', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('معمولی ڈبل تھپتھپائیں۔', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('کمزور اینٹی گھوسٹنگ', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('بہترین NKRO کارکردگی', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('کلیدی چسپاں کرنا', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('ٹیسٹ ڈیٹا، تشخیصی نتائج، اور لاگز کو دوبارہ ترتیب دیا گیا ہے۔', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 ٹپ: کلیدی وقفوں کو جانچنے کے لیے تیزی سے ایک ہی کلید کو دبائیں۔ وقفے <80ms کو سرخ رنگ میں نشان زد کیا گیا ہے۔",
        "💡 ٹپ: عام انسانی آپریشن کے وقفے عام طور پر 100ms سے زیادہ ہوتے ہیں، گیمرز 80-120ms تک پہنچ سکتے ہیں۔",
        "💡 ٹپ: متواتر <50ms کے وقفے کی بورڈ کے ڈبل ٹیپ کی خرابیوں کی نشاندہی کر سکتے ہیں۔",
        "💡 ٹپ: اینٹی گھوسٹنگ صلاحیت کو جانچنے کے لیے ایک ساتھ متعدد کلیدیں دبانے کی کوشش کریں۔",
        "💡 مشورہ: تمام کلیدوں کو ترتیب وار جانچیں، خاص طور پر WASD اور Spacebar جیسی کثرت سے استعمال ہونے والی کلیدیں۔",
        "💡 ٹپ: دیکھیں کہ آیا نیچے/اوپر کی گنتی مماثل ہے۔ مماثلت کلیدی چپکنے کی نشاندہی کر سکتی ہے۔"
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'شدید ڈبل ٹیپ کی خرابی، فوری مرمت کی ضرورت ہے۔';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'اعتدال پسند ڈبل ٹیپ غلطی، جلد ہی پتہ کریں';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'معمولی ڈبل تھپتھپائیں، قریب سے نگرانی کریں۔';
    } else {
        logLevel = 'normal';
...
window.clearLog = function() {
    ...
    if (logContent.children.length <= 2) return; // Keep initial two messages
        ...
        updateLogStats();
        showToast('لاگ صاف ہو گیا۔', 'success');
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> لاگ دکھائیں۔';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-ey-slash"></i> لاگ چھپائیں۔';
        }
    }
}
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'آٹو اسکرول فعال ہے۔' : 'آٹو سکرول غیر فعال ہے۔', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('کلیدی وقفہ ٹیسٹ شروع کرنا: ایک ہی کلید کو تیزی سے دبائیں (مثال کے طور پر، ڈبلیو کی یا اسپیس بار)', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">کلیدی وقفہ ٹیسٹ شروع کر رہا ہے - براہ کرم تیزی سے ایک ہی کلید کو بار بار دبائیں۔</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 مرحلہ 1: سنگل کلیدی فنکشن ٹیسٹ",
            instruction: "غیر جوابی کلیدوں کو چیک کرنے کے لیے براہ کرم درج ذیل کلیدی گروپس کو ترتیب وار جانچیں:<br>1۔ WASD سمتاتی کلیدیں<br>2۔ نمبر کیز 1-5<br>3۔ تیر والی کلیدیں ↑↓←→<br>4۔ اسپیس بار اور انٹر کیز",
            action: "testAllKeys",
            check: "چیک کریں کہ کیا دبانے پر ہر کلید سفید ہو جاتی ہے۔",
            time: 60
        },
        {
            title: "⏱️ مرحلہ 2: اہم وقفہ کا پتہ لگانا",
            instruction: "اسی کلید کو تیزی سے 10 بار دبائیں (W key اور Spacebar کی جانچ کرنے کی تجویز کریں)، لاگ آؤٹ پٹ کا مشاہدہ کریں",
            action: "testInterval",
            check: "اہم وقفوں کا مشاہدہ کریں۔ نارمل 80ms سے زیادہ ہونا چاہیے۔ وقفے <80ms کو سرخ نشان زد کیا گیا ہے۔",
            time: 30
        },
        {
            title: "🎮 مرحلہ 3: NKRO اینٹی گھوسٹنگ ٹیسٹ",
            instruction: "اپنے پورے ہاتھ کو کی بورڈ کے درمیانی حصے پر دبائیں، ایک ساتھ زیادہ سے زیادہ کیز دبائیں",
            action: "testNKRO",
            check: "'موجودہ کنکرنسی' قدر کا مشاہدہ کریں۔ عام کی بورڈز میں ≥6 ہونا چاہیے، گیمنگ کی بورڈ 10+ تک پہنچ سکتے ہیں۔",
            time: 20
        },
        {
            title: "⚡ مرحلہ 4: جوابی تاخیر کا ٹیسٹ",
            instruction: "اعتدال کی رفتار سے مختلف کلیدوں کو دبائیں، جوابی وقت کا مشاہدہ کریں۔",
            action: "testResponseTime",
            check: "وقفہ کے اوقات بڑے اتار چڑھاو کے بغیر 50-200ms کے درمیان مستحکم ہونے چاہئیں",
            time: 30
        },
        {
            title: "🔄 مرحلہ 5: کلیدی تنازعات کا پتہ لگانا",
            instruction: "ایک ساتھ دبائیں: W+A+Shift+Space<br>پھر: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "تمام چابیاں بیک وقت نمایاں ہونی چاہئیں، کوئی بھی ناکام نہیں ہونا چاہیے۔",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'ابھی تک کوئی چابیاں ٹیسٹ نہیں کی گئیں۔');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'شدید ڈبل ٹیپ مسائل کا پتہ چلا (وقفے <20ms)');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'دو بار تھپتھپانے والے معمولی مسائل کا پتہ چلا (وقفے <80ms)');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'کلیدی وقفے معمول کے مطابق، دو بار تھپتھپانے کا کوئی پتہ نہیں چلا');
                } else {
                    addStepResult('info', 'کلیدی وقفہ ٹیسٹ ابھی تک نہیں ہوا ہے۔');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'جواب میں تاخیر کا ٹیسٹ مکمل، استعمال کے تجربے کی بنیاد پر فیصلہ کریں۔');
                } else {
                    addStepResult('info', 'جواب میں تاخیر کا اندازہ لگانے کے لیے براہ کرم مزید کلیدوں کی جانچ کریں۔');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('کیا آپ واقعی تشخیصی موڈ سے باہر نکلنا چاہتے ہیں؟ نامکمل نتائج کو محفوظ نہیں کیا جائے گا۔')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">شدید ڈبل تھپتھپائیں۔s</span>
    ...
    window.saveReportAsImage = function() {
        showToast('رپورٹ اسکرین شاٹ کلپ بورڈ میں محفوظ کیا گیا (نقلی فنکشن)', 'info');
