...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 చిట్కా: కీ విరామాలను పరీక్షించడానికి అదే కీని వేగంగా నొక్కండి. విరామాలు <80ms ఎరుపు రంగులో గుర్తించబడ్డాయి.', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'ఎడమ షిఫ్ట్', 'ShiftRight': 'కుడి షిఫ్ట్', 'Backspace': 'Backspace',
    'ControlLeft': 'ఎడమ Ctrl', 'ControlRight': 'కుడి Ctrl', 'AltLeft': 'ఎడమ ఆల్ట్', 'AltRight': 'కుడి ఆల్ట్',
    'Tab': 'Tab', 'CapsLock': 'క్యాప్స్ లాక్', 'Escape': 'Esc', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'పేజీ పైకి', 'PageDown': 'పేజీ డౌన్', 'NumLock': 'నమ్ లాక్',
    'ScrollLock': 'స్క్రోల్ లాక్', 'Pause': 'Pause', 'PrintScreen': 'ప్రింట్ స్క్రీన్', 'ContextMenu': 'మెనూ'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> ఆటో స్క్రోల్ ఆన్' : 
                '<i class="fas fa-scroll"></i> ఆటో స్క్రోల్ ఆఫ్';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('తీవ్రమైన డబుల్-ట్యాప్', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('రెండుసార్లు నొక్కండి', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('మైనర్ డబుల్ ట్యాప్', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('బలహీనమైన యాంటీ-గోస్టింగ్', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('అద్భుతమైన NKRO పనితీరు', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('కీ అంటుకోవడం', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
window.resetTest = function() {
        ...
        showToast('పరీక్ష డేటా, నిర్ధారణ ఫలితాలు మరియు లాగ్‌లు రీసెట్ చేయబడ్డాయి', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 చిట్కా: కీ విరామాలను పరీక్షించడానికి అదే కీని వేగంగా నొక్కండి. విరామాలు <80ms ఎరుపు రంగులో గుర్తించబడ్డాయి.",
        "💡 చిట్కా: సాధారణ మానవ ఆపరేషన్ విరామాలు సాధారణంగా >100ms, గేమర్స్ 80-120ms చేరుకోవచ్చు.",
        "💡 చిట్కా: తరచుగా <50ms విరామాలు కీబోర్డ్ రెండుసార్లు నొక్కడం లోపాలను సూచించవచ్చు.",
        "💡 చిట్కా: యాంటీ-ఘోస్టింగ్ సామర్థ్యాన్ని పరీక్షించడానికి ఏకకాలంలో బహుళ కీలను నొక్కడం ప్రయత్నించండి.",
        "💡 చిట్కా: అన్ని కీలను వరుసగా పరీక్షించండి, ముఖ్యంగా WASD మరియు Spacebar వంటి తరచుగా ఉపయోగించే కీలు.",
        "💡 చిట్కా: డౌన్/అప్ కౌంట్‌లు సరిపోలుతున్నాయో లేదో గమనించండి. అసమతుల్యత కీ అంటుకోవడాన్ని సూచించవచ్చు."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'తీవ్రమైన డబుల్-ట్యాప్ లోపం, తక్షణ మరమ్మతు అవసరం';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'మోడరేట్ డబుల్-ట్యాప్ తప్పు, త్వరలో పరిష్కరించండి';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'చిన్నగా రెండుసార్లు నొక్కండి, దగ్గరగా పర్యవేక్షించండి';
    } else {
        logLevel = 'normal';
...
window.clearLog = function() {
    ...
    if (logContent.children.length <= 2) return; // Keep initial two messages
        ...
        updateLogStats();
        showToast('లాగ్ క్లియర్ చేయబడింది', 'success');
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> లాగ్ చూపించు';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> లాగ్‌ను దాచు';
        }
    }
}
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'ఆటో స్క్రోల్ ప్రారంభించబడింది' : 'ఆటో స్క్రోల్ డిజేబుల్ చేయబడింది', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('ప్రారంభ కీ విరామం పరీక్ష: అదే కీని వేగంగా నొక్కండి (ఉదా., W కీ లేదా Spacebar)', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">కీ విరామం పరీక్షను ప్రారంభిస్తోంది - దయచేసి అదే కీని పదే పదే వేగంగా నొక్కండి</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 దశ 1: సింగిల్ కీ ఫంక్షన్ టెస్ట్",
            instruction: "దయచేసి ప్రతిస్పందించని కీల కోసం తనిఖీ చేయడానికి క్రింది కీలక సమూహాలను వరుసగా పరీక్షించండి:<br>1. WASD డైరెక్షనల్ కీలు<br>2. సంఖ్య కీలు 1-5<br>3. బాణం కీలు ↑↓←→<br>4. Spacebar మరియు Enter కీలు",
            action: "testAllKeys",
            check: "నొక్కినప్పుడు ప్రతి కీ తెల్లగా మారుతుందో లేదో తనిఖీ చేయండి",
            time: 60
        },
        {
            title: "⏱️ దశ 2: కీ ఇంటర్వెల్ డిటెక్షన్",
            instruction: "అదే కీని 10 సార్లు వేగంగా నొక్కండి (W కీ మరియు స్పేస్‌బార్‌ని పరీక్షించమని సిఫార్సు చేయండి), లాగ్ అవుట్‌పుట్‌ను గమనించండి",
            action: "testInterval",
            check: "కీలక విరామాలను గమనించండి. సాధారణ> 80ms ఉండాలి. విరామాలు <80ms ఎరుపు రంగులో గుర్తించబడ్డాయి",
            time: 30
        },
        {
            title: "🎮 దశ 3: NKRO యాంటీ-గోస్టింగ్ టెస్ట్",
            instruction: "మీ మొత్తం చేతిని మధ్య కీబోర్డ్ ప్రాంతంపై నొక్కండి, ఒకేసారి వీలైనన్ని ఎక్కువ కీలను నొక్కండి",
            action: "testNKRO",
            check: "'కరెంట్ కరెన్సీ' విలువను గమనించండి. సాధారణ కీబోర్డ్‌లు ≥6ని కలిగి ఉండాలి, గేమింగ్ కీబోర్డ్‌లు 10+కి చేరుకోవచ్చు",
            time: 20
        },
        {
            title: "⚡ దశ 4: ప్రతిస్పందన ఆలస్యం పరీక్ష",
            instruction: "మితమైన వేగంతో వేర్వేరు కీలను నొక్కండి, ప్రతిస్పందన సమయాన్ని గమనించండి",
            action: "testResponseTime",
            check: "విరామ సమయాలు పెద్ద హెచ్చుతగ్గులు లేకుండా 50-200ms మధ్య స్థిరంగా ఉండాలి",
            time: 30
        },
        {
            title: "🔄 దశ 5: కీలక సంఘర్షణ గుర్తింపు",
            instruction: "ఏకకాలంలో నొక్కండి: W+A+Shift+Space<br>తర్వాత: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "అన్ని కీలు ఏకకాలంలో హైలైట్ చేయాలి, ఏదీ విఫలం కాకూడదు",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'ఇంకా కీలు ఏవీ పరీక్షించబడలేదు');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'తీవ్రమైన డబుల్-ట్యాప్ సమస్యలు గుర్తించబడ్డాయి (విరామాలు <20మి.)');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'చిన్న డబుల్-ట్యాప్ సమస్యలు గుర్తించబడ్డాయి (విరామాలు <80మి.)');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'కీ విరామాలు సాధారణమైనవి, రెండుసార్లు నొక్కడం కనుగొనబడలేదు');
                } else {
                    addStepResult('info', 'కీ ఇంటర్వెల్ పరీక్ష ఇంకా నిర్వహించబడలేదు');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'ప్రతిస్పందన ఆలస్యం పరీక్ష పూర్తయింది, వినియోగ అనుభవం ఆధారంగా తీర్పు ఇవ్వండి');
                } else {
                    addStepResult('info', 'ప్రతిస్పందన ఆలస్యాన్ని అంచనా వేయడానికి దయచేసి మరిన్ని కీలను పరీక్షించండి');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('మీరు ఖచ్చితంగా డయాగ్నస్టిక్ మోడ్ నుండి నిష్క్రమించాలనుకుంటున్నారా? అసంపూర్ణ ఫలితాలు సేవ్ చేయబడవు.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">తీవ్రమైన డబుల్-ట్యాప్s</span>
    ...
    window.saveReportAsImage = function() {
        showToast('స్క్రీన్‌షాట్ క్లిప్‌బోర్డ్‌లో సేవ్ చేయబడిందని నివేదించండి (అనుకరణ ఫంక్షన్)', 'info');
