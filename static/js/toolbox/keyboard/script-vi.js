    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> Tự động bật' : 
                '<i class="fas fa-scroll"></i> Tự động tắt';
        }
    }
}
...
window.resetTest = function() {
        ...
        infoCode.innerText = '-';
        infoWhich.innerText = '-';
        infoTime.innerText = '- bệnh đa xơ cứng';
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> Hiển thị nhật ký';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> Ẩn nhật ký';
        }
    }
}
...
window.startIntervalTest = function() {
    ...
    testStartEntry.innerHTML = `\r\n        <span class="log-time">${timeStr}</span>\r\n        <span class="log-message">Bắt đầu kiểm tra khoảng thời gian của phím - Vui lòng nhấn nhanh cùng một phím nhiều lần</span>\r\n    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 Bước 1: Kiểm tra chức năng phím đơn",
        ...
        {
            title: "⏱️ Bước 2: Phát hiện khoảng thời gian chính",
        ...
        {
            title: "🎮 Bước 3: Kiểm tra khả năng chống bóng ma của NKRO",
        ...
        {
            title: "⚡ Bước 4: Kiểm tra độ trễ phản hồi",
        ...
        {
            title: "🔄 Bước 5: Phát hiện xung đột chính",
...
function createDiagnosisPanel(steps) {
    ...
    panel.innerHTML = `\r\n        <div class="diagnosis-header">\r\n            <h3><i class="fas fa-stethoscope"></i> Chế độ chẩn đoán bàn phím</h3>\r\n            <button class="close-diagnosis"><i class="fas fa-times"></i></button>\r\n        </div>\r\n        <div class="diagnosis-progress">\r\n            <div class="progress-bar"><div class="progress-fill" style="width: 20%;"></div></div>\r\n            <div class="progress-text">Bước 1/${steps.length}</div>\r\n        </div>\r\n        <div class="diagnosis-content">\r\n            <div class="step-current">\r\n                <h4 id="step-title">${steps[0].title}</h4>\r\n                <p id="step-instruction">${steps[0].instruction}</p>\r\n                <div class="step-requirements">\r\n                    <strong>Điểm kiểm tra:</strong> ${steps[0].check}<br>\r\n                    <small><i class="far fa-clock"></i> Thời gian đề xuất: ${steps[0].time} giây</small>\r\n                </div>\r\n            </div>\r\n            <div class="step-results" id="step-results"></div>\r\n        </div>\r\n        <div class="diagnosis-actions">\r\n            <button class="btn-step" id="prev-step"><i class="fas fa-arrow-left"></i> Trước</button>\r\n            <button class="btn-step" id="next-step">Tiếp theo <i class="fas fa-arrow-right"></i></button>\r\n            <button class="btn-complete" id="complete-diagnosis">Chẩn đoán hoàn chỉnh và tạo báo cáo</button>\r\n        </div>\r\n    `;
    ...
    function updateProgress() {
        ...
        document.querySelector('.progress-text').textContent = `Bước ${currentStep + 1}/${steps.length}`;
        ...
        document.querySelector('.step-requirements').innerHTML = `\r\n            <strong>Điểm kiểm tra:</strong> ${steps[currentStep].check}<br>\r\n            <small><i class="far fa-clock"></i> Thời gian đề xuất: ${steps[currentStep].time} giây</small>\r\n        `;
    }
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('Bạn có chắc chắn muốn thoát khỏi chế độ chẩn đoán không? Kết quả không đầy đủ sẽ không được lưu.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
