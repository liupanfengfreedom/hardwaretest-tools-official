...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Show welcome tip
    setTimeout(() => {
        showToast('💡 نصيحة: اضغط بسرعة على نفس المفتاح لاختبار الفواصل الزمنية بين المفاتيح. يتم تمييز الفواصل الزمنية <80 مللي ثانية باللون الأحمر.', 'info');
    }, 1000);
});
...
const keyNamesEn = {
    ...
    'Enter': 'Enter', 'ShiftLeft': 'التحول الأيسر', 'ShiftRight': 'التحول الصحيح', 'Backspace': 'Backspace',
    'ControlLeft': 'السيطرة اليسرى', 'ControlRight': 'السيطرة اليمنى', 'AltLeft': 'البديل الأيسر', 'AltRight': 'البديل الصحيح',
    'Tab': 'Tab', 'CapsLock': 'Caps lock', 'Escape': 'خروج', 'Insert': 'Insert', 'Delete': 'Delete',
    'Home': 'Home', 'End': 'End', 'PageUp': 'صفحة لأعلى', 'PageDown': 'الصفحة لأسفل', 'NumLock': 'قفل الأرقام',
    'ScrollLock': 'قفل التمرير', 'Pause': 'Pause', 'PrintScreen': 'شاشة الطباعة', 'ContextMenu': 'قائمة طعام'
};
    ...
    if (savedAutoScroll !== null) {
        ...
        if (toggleBtn) {
            toggleBtn.innerHTML = logAutoScroll ? 
                '<i class="fas fa-scroll"></i> تشغيل التمرير التلقائي' : 
                '<i class="fas fa-scroll"></i> إيقاف التمرير التلقائي';
        }
    }
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (keyLastPressTime[keyCode]) {
        ...
        if (diagnosisActive) {
            if (interval < 20) {
                addDiagnosisResult('النقر المزدوج الشديد', `Key ${getKeyDisplayName(keyCode)} interval only ${interval}ms`, 'error');
            } else if (interval < 50) {
                addDiagnosisResult('انقر نقرًا مزدوجًا متوسطًا', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'warning');
            } else if (interval < 80) {
                addDiagnosisResult('النقر المزدوج البسيط', `Key ${getKeyDisplayName(keyCode)} interval ${interval}ms`, 'info');
            }
        }
    }
    ...
    if (pressedKeys.size > maxConcurrentKeys) {
        ...
        if (diagnosisActive) {
            if (maxConcurrentKeys < 6) {
                addDiagnosisResult('مكافحة الظلال ضعيفة', `Max concurrent keys only ${maxConcurrentKeys}`, 'warning');
            } else if (maxConcurrentKeys >= 20) {
                addDiagnosisResult('أداء NKRO ممتاز', `Supports ${maxConcurrentKeys} key rollover`, 'success');
            }
        }
    }
...
document.addEventListener('keyup', (e) => {
    ...
    if (keyEl) {
        ...
        if (keyStats[e.code].down !== keyStats[e.code].up && diagnosisActive) {
            addDiagnosisResult('لصق المفتاح', `Key ${getKeyDisplayName(e.code)}: Down ${keyStats[e.code].down} vs Up ${keyStats[e.code].up}`, 'error');
        }
    }
...
function updateStats() {
    ...
    if (pressedKeys.size > 20) {
        ...
        activeCountEl.style.textShadow = '0 0 10 بكسل #00ffaa';
...
window.resetTest = function() {
        ...
        showToast('تمت إعادة تعيين بيانات الاختبار ونتائج التشخيص والسجلات', 'success');
...
window.showKeyboardTips = function() {
    const tips = [
        "💡 نصيحة: اضغط بسرعة على نفس المفتاح لاختبار الفواصل الزمنية بين المفاتيح. يتم تمييز الفواصل الزمنية <80 مللي ثانية باللون الأحمر.",
        "💡 نصيحة: عادةً ما تكون فترات التشغيل البشري العادية أكبر من 100 مللي ثانية، وقد يصل اللاعبون إلى 80-120 مللي ثانية.",
        "💡 نصيحة: قد تشير الفواصل الزمنية المتكررة التي تقل عن 50 مللي ثانية إلى أخطاء النقر المزدوج على لوحة المفاتيح.",
        "💡 نصيحة: حاول الضغط على مفاتيح متعددة في وقت واحد لاختبار القدرة على مقاومة الظلال.",
        "💡 نصيحة: اختبر جميع المفاتيح بشكل تسلسلي، وخاصة المفاتيح المستخدمة بشكل متكرر مثل WASD ومفتاح المسافة.",
        "💡 نصيحة: لاحظ ما إذا كانت الأعداد السفلية/الأعلى متطابقة. قد يشير عدم التطابق إلى التصاق المفتاح."
    ];
...
function logKeyInterval(keyCode, interval) {
    ...
    if (interval < 20) {
        ...
        warningMessage = 'خطأ شديد في النقر المزدوج، يلزم الإصلاح الفوري';
    } else if (interval < 50) {
        logLevel = 'error';
        logMessage = `Key[${keyName}] Interval:${interval}ms ❌ Abnormal! Possible double-tap fault`;
        warningMessage = 'خطأ متوسط ​​بالنقر المزدوج، ومعالجته قريبًا';
    } else if (interval < 80) {
        logLevel = 'warning';
        logMessage = `Key[${keyName}] Interval:${interval}ms ⚠️ Short! May indicate early double-tap`;
        warningMessage = 'انقر نقرًا مزدوجًا بسيطًا، وراقب عن كثب';
    } else {
        logLevel = 'normal';
...
window.toggleLog = function() {
    ...
    if (logContent && logContainer) {
        if (logContainer.style.maxHeight && logContainer.style.maxHeight !== '0px') {
            ...
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye"></i> عرض السجل';
        } else {
            logContainer.style.maxHeight = '500px';
            logContainer.style.opacity = '1';
            document.getElementById('toggle-log').innerHTML = '<i class="fas fa-eye-slash"></i> إخفاء السجل';
        }
    }
}
...
window.toggleAutoScroll = function() {
    ...
    showToast(logAutoScroll ? 'تم تمكين التمرير التلقائي' : 'تم تعطيل التمرير التلقائي', 'info');
}

// Start interval test
window.startIntervalTest = function() {
    showToast('بدء اختبار الفاصل الزمني للمفتاح: اضغط بسرعة على نفس المفتاح (على سبيل المثال، مفتاح W أو مفتاح المسافة)', 'info');
    ...
    testStartEntry.innerHTML = `
        <span class="log-time">${timeStr}</span>
        <span class="log-message">بدء اختبار الفاصل الزمني للمفتاح - يرجى الضغط بسرعة على نفس المفتاح بشكل متكرر</span>
    `;
...
window.startDiagnosis = function() {
    ...
    const diagnosticSteps = [
        {
            title: "🔍 الخطوة 1: اختبار وظيفة المفتاح الواحد",
            instruction: "يرجى اختبار مجموعات المفاتيح التالية بالتسلسل للتحقق من المفاتيح غير المستجيبة:<br>1. مفاتيح اتجاه WASD<br>2. مفاتيح الأرقام 1-5<br>3. مفاتيح الأسهم ↑↓←←<br>4. مفتاح المسافة ومفاتيح الإدخال",
            action: "testAllKeys",
            check: "تحقق مما إذا كان كل مفتاح يتحول إلى اللون الأبيض عند الضغط عليه",
            time: 60
        },
        {
            title: "⏱️ الخطوة 2: الكشف عن الفاصل الزمني الرئيسي",
            instruction: "اضغط بسرعة على نفس المفتاح 10 مرات (يوصى باختبار مفتاح W ومفتاح المسافة)، ولاحظ إخراج السجل",
            action: "testInterval",
            check: "مراقبة الفواصل الزمنية الرئيسية. يجب أن يكون الطبيعي> 80 مللي ثانية. يتم تمييز الفواصل الزمنية <80 مللي ثانية باللون الأحمر",
            time: 30
        },
        {
            title: "🎮 الخطوة 3: اختبار NKRO لمكافحة الظلال",
            instruction: "اضغط بيدك بالكامل على منطقة لوحة المفاتيح الوسطى، مع الضغط على أكبر عدد ممكن من المفاتيح في وقت واحد",
            action: "testNKRO",
            check: "لاحظ قيمة "التزامن الحالي". يجب أن تحتوي لوحات المفاتيح العادية على ≥6، ويمكن أن تصل لوحات مفاتيح الألعاب إلى 10+",
            time: 20
        },
        {
            title: "⚡ الخطوة 4: اختبار تأخير الاستجابة",
            instruction: "اضغط على مفاتيح مختلفة بسرعة متوسطة، ولاحظ زمن الاستجابة",
            action: "testResponseTime",
            check: "يجب أن تكون الفواصل الزمنية مستقرة بين 50-200 مللي ثانية دون تقلبات كبيرة",
            time: 30
        },
        {
            title: "🔄 الخطوة 5: الكشف عن التضارب الرئيسي",
            instruction: "اضغط في الوقت نفسه: W+A+Shift+Space<br>ثم: Ctrl+Shift+Alt",
            action: "testKeyConflict",
            check: "يجب أن يتم تمييز جميع المفاتيح في وقت واحد، ولا ينبغي أن يفشل أي منها",
            time: 30
        }
    ];
...
function createDiagnosisPanel(steps) {
        ...
        switch(currentStep) {
            case 0: // Single key function test
                if (testedKeys.size >= 20) {
                    ...
                    addStepResult('error', 'لم يتم اختبار المفاتيح حتى الآن');
                }
                break;
            case 1: // Key interval detection
                ...
                if (hasCriticalInterval) {
                    addStepResult('error', 'تم اكتشاف مشكلات خطيرة في النقر المزدوج (فواصل زمنية أقل من 20 مللي ثانية)');
                } else if (hasWarningInterval) {
                    addStepResult('warning', 'تم اكتشاف مشكلات بسيطة في النقر المزدوج (الفواصل الزمنية أقل من 80 مللي ثانية)');
                } else if (Object.keys(keyPressIntervals).length > 0) {
                    addStepResult('success', 'الفواصل الزمنية بين المفاتيح طبيعية، ولم يتم اكتشاف أي نقرة مزدوجة');
                } else {
                    addStepResult('info', 'اختبار الفاصل الزمني الرئيسي لم يتم إجراؤه بعد');
                }
                break;
            case 2: // NKRO test
            ...
            case 3: // Response delay test
                ...
                if (testedKeys.size > 10) {
                    addStepResult('success', 'تم الانتهاء من اختبار تأخير الاستجابة، ويتم الحكم بناءً على تجربة الاستخدام');
                } else {
                    addStepResult('info', 'يرجى اختبار المزيد من المفاتيح لتقييم تأخير الاستجابة');
                }
                break;
    ...
    document.querySelector('.close-diagnosis').addEventListener('click', () => {
        if (confirm('هل أنت متأكد أنك تريد الخروج من وضع التشخيص؟ لن يتم حفظ النتائج غير المكتملة.')) {
            panel.remove();
            diagnosisActive = false;
        }
    });
...
function generateDiagnosisReport() {
        ...
        <div class="diagnosis-report">
            ...
            <div class="interval-stats">
                ...
                <div class="interval-grid">
                    ...
                    <div class="interval-item">
                        <span class="interval-label">النقر المزدوج الشديدs</span>
    ...
    window.saveReportAsImage = function() {
        showToast('الإبلاغ عن لقطة شاشة محفوظة في الحافظة (وظيفة محاكاة)', 'info');
