...
function clearData() {
    ...
    logContainer.innerHTML = '<div class="text-slate-500 italic">Đang chờ dữ liệu...</div>';
    eventCountEl.innerText = `0 sự kiện`;
...
function downloadCSV() {
    if (logs.length === 0) {
        alert('Không có dữ liệu để xuất. Hãy thực hiện kiểm tra đầu tiên.');
        return;
    }
...
function init() {
    ...
    initialLog.textContent = `Công cụ kiểm tra đã sẵn sàng (${startTime}) - Bắt đầu di chuyển chuột để kiểm tra`;
