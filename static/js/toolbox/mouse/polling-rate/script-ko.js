...
function clearData() {
    ...
    logContainer.innerHTML = '<div class="text-slate-500 italic">데이터를 기다리는 중...</div>';
    eventCountEl.innerText = `0개의 이벤트`;
...
function downloadCSV() {
    if (logs.length === 0) {
        alert('내보낼 데이터가 없습니다. 먼저 테스트를 수행해 보세요.');
        return;
    }
...
function init() {
    ...
    initialLog.textContent = `테스트 도구 준비됨(${startTime}) - 테스트를 위해 마우스를 움직이기 시작합니다.`;
