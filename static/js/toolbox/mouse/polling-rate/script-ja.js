...
function clearData() {
    ...
    logContainer.innerHTML = '<div class="text-slate-500 italic">データを待っています...</div>';
    eventCountEl.innerText = `0件のイベント`;
...
function downloadCSV() {
    if (logs.length === 0) {
        alert('エクスポートするデータがありません。最初にテストを実行してください。');
        return;
    }
...
function init() {
    ...
    initialLog.textContent = `テスト ツールの準備ができました (${startTime}) - マウスを動かしてテストを開始します`;
