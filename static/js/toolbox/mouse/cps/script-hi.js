...
class CPSTester {
    constructor() {
        ...
        this.buttonModes = {
            any: { 
                name: 'Any Button', 
                icon: 'fas fa-mouse',
                description: 'बाएँ, दाएँ और मध्य बटन सभी पर क्लिक किया जा सकता है'
            },
            left: { 
                name: 'Left Button Mode', 
                icon: 'fas fa-hand-point-up',
                description: 'क्लिक करने के लिए केवल माउस के बाएँ बटन का उपयोग करें'
            },
            right: { 
                name: 'Right Button Mode', 
                icon: 'fas fa-hand-point-down',
                description: 'क्लिक करने के लिए केवल माउस के दाएँ बटन का उपयोग करें'
            },
            middle: { 
                name: 'Middle Button Mode', 
                icon: 'fas fa-mouse-pointer',
                description: 'क्लिक करने के लिए केवल माउस के मध्य बटन का उपयोग करें'
            }
        };
    ...
    init() {
        ...
        this.showNotification('सक्रिय करने के लिए माउस को परीक्षण क्षेत्र में ले जाएं, फिर उलटी गिनती शुरू करने के लिए माउस पर क्लिक करें', 'info');
    ...
    setMode(modeId) {
        if (this.state.testStatus === 'running' || this.state.testStatus === 'counting') {
            this.showNotification('परीक्षण प्रगति पर है, मोड स्विच नहीं किया जा सकता!', 'error');
            return;
        }
        ...
        this.showNotification(`${mode.name} पर स्विच किया गया`, 'success');
    }
    ...
    setButtonMode(modeId) {
        ...
        this.showNotification(`${this.buttonModes[modeId].name} पर स्विच किया गया`, 'success');
    }
    ...
    activateTestArea() {
        ...
        if (this.state.testStatus === 'finished') {
            this.showNotification('परीक्षण पूरा हो गया, कृपया नया परीक्षण शुरू करने के लिए "पुनः प्रारंभ परीक्षण" बटन पर क्लिक करें', 'info');
            return;
        }
        ...
        const buttonMode = this.buttonModes[this.state.buttonMode];
        document.getElementById('click-title').textContent = 'शुरू करने के लिए तैयार';
        ...
        this.showNotification(`परीक्षण क्षेत्र सक्रिय, ${instructionText}`, 'info');
    }
    ...
    deactivateTestArea() {
        if (this.state.testStatus === 'active') {
            ...
            // Update instruction information
            document.getElementById('click-title').textContent = 'माउस सीपीएस टेस्ट';
            ...
            this.showNotification('परीक्षण क्षेत्र निष्क्रिय कर दिया गया', 'info');
        }
    }
    ...
    handleMouseDown(event) {
        ...
        if (this.state.testStatus === 'active') {
            ...
            if (!this.isButtonAllowed(event.button)) {
                ...
                this.showNotification(`वर्तमान मोड केवल ${allowedButton} क्लिक की अनुमति देता है`, 'error');
                return;
            }
        ...
        if (this.state.testStatus === 'running') {
            ...
            if (!this.isButtonAllowed(event.button)) {
                ...
                if (!this.state.lastButtonWarning || Date.now() - this.state.lastButtonWarning > 2000) {
                    this.showNotification(`कृपया क्लिक करने के लिए ${allowedButton} का उपयोग करें`, 'error');
    ...
    startCountdown() {
        ...
        // Update instruction information
        document.getElementById('click-title').textContent = 'शुरू करने के लिए तैयार';
        document.getElementById('click-instruction').textContent = 'जल्द ही उल्टी गिनती शुरू हो जाएगी';
    ...
    beginCountdown() {
        ...
        const countdownInterval = setInterval(() => {
            countdownElement.textContent = count > 0 ? count.toString() : 'जाना!';
    ...
    beginTest() {
        ...
        // Update instruction information
        document.getElementById('click-title').textContent = 'परीक्षण प्रगति पर है';
        ...
        this.showNotification('परीक्षण शुरू!', 'success');
    }
    ...
    finishTest() {
        ...
        document.getElementById('click-title').textContent = 'परीक्षण पूरा हुआ';
        document.getElementById('click-instruction').textContent = 'नया परीक्षण शुरू करने के लिए नीचे "पुनः प्रारंभ परीक्षण" बटन पर क्लिक करें';
        ...
        this.showNotification('परीक्षण पूरा हुआ! नया परीक्षण शुरू करने के लिए "पुनः प्रारंभ परीक्षण" बटन पर क्लिक करें', 'success');
    }
    ...
    resetTest() {
        ...
        resultsDashboard.innerHTML = `\r\n            <h2><i class="fas fa-chart-bar"></i> परीक्षण परिणाम</h2>\r\n            <div class="no-results">\r\n                <i class="fas fa-mouse-pointer" style="font-size: 3rem; color: var(--text-muted); display: block; text-align: center; margin: 20px 0;"></i>\r\n                <p style="text-align: center; color: var(--text-secondary);">परीक्षण पूरा करने के बाद परिणाम देखें</p>\r\n            </div>\r\n        `;
        
        this.updateRealTimeStats();
        
        this.showNotification('रीसेट पूरा हो गया, सक्रिय करने और नया परीक्षण शुरू करने के लिए माउस को परीक्षण क्षेत्र में ले जाएं', 'info');
    }
    ...
    updateControlButtons() {
        ...
        switch (this.state.testStatus) {
            case 'running':
                ...
                restartBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> परीक्षण प्रगति पर है...';
                break;
            ...
            case 'waiting':
                ...
                restartBtn.classList.remove('highlight');
                restartBtn.innerHTML = '<i class="fas fa-redo"></i> परीक्षण पुनः आरंभ करें';
                break;
                
            case 'finished':
                restartBtn.disabled = false;
                restartBtn.classList.add('highlight');
                restartBtn.innerHTML = '<i class="fas fa-play-circle"></i> नया परीक्षण शुरू करने के लिए क्लिक करें';
                break;
    ...
    loadHistory() {
        try {
            ...
            history.slice(0, 5).forEach(item => {
                const historyElement = document.createElement('div');
                historyElement.className = 'history-item';
                historyElement.innerHTML = `\r\n                    <div class="history-header" style="display: flex; justify-content: space-between;">\r\n                        <span class="history-time">${item.timestamp}</span>\r\n                        <span class="history-mode">${item.mode}</span>\r\n                    </div>\r\n                    <div class="history-body" style="display: flex; justify-content: space-between; align-items: center; margin-top: 5px;">\r\n                        <span class="history-cps">${item.cps.toFixed(1)} CPS</span>\r\n                        <span class="history-clicks" style="color: var(--text-secondary); font-size: 0.9em;">${item.totalClicks} क्लिक</span>\r\n                    </div>\r\n                    <div class="history-footer" style="display: flex; justify-content: space-between; font-size: 0.8em; color: var(--text-muted); margin-top: 5px;">\r\n                        <span>${item.buttonMode}</span>\r\n                        <span style="color: var(--secondary-color); font-weight: 600;">स्कोर: ${item.score || 0}</span>\r\n                    </div>\r\n                `;
    ...
    toggleTheme() {
        ...
        this.showNotification(`${nextTheme} थीम पर स्विच किया गया`, 'success');
    }
    ...
    toggleFullscreen() {
        if (!document.fullscreenElement) {
            document.documentElement.requestFullscreen().catch(err => {
                ...
                this.showNotification('फ़ुलस्क्रीन मोड में प्रवेश करने में असमर्थ', 'error');
            });
        } else {
    ...
    createSettingsPanel() {
        ...
        container.innerHTML = `\r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-palette"></i> उपस्थिति सेटिंग्स</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <span>थीम</span>\r\n                        <select id="theme-select" class="setting-input">\r\n                            <option value="dark" ${settings.theme === 'dark' ? 'selected' : ''}>डार्क थीम</option>\r\n                            <option value="light" ${settings.theme === 'light' ? 'selected' : ''}>लाइट थीम</option>\r\n                            <option value="high-contrast" ${settings.theme === 'high-contrast' ? 'selected' : ''}>हाई कंट्रास्ट</option>\r\n                            <option value="eye-care" ${settings.theme === 'eye-care' ? 'selected' : ''}>नेत्र देखभाल मोड</option>\r\n                        </select>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-film"></i> एनिमेशन प्रभाव</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="animations-toggle" ${settings.animationsEnabled ? 'checked' : ''}>\r\n                        <span>एनिमेशन सक्षम करें</span>\r\n                    </label>\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="particles-toggle" ${settings.showParticles ? 'checked' : ''}>\r\n                        <span>क्लिक कण प्रभाव दिखाएं</span>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-cogs"></i> परीक्षण सेटिंग्स</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <span>उलटी गिनती अवधि</span>\r\n                        <select id="countdown-select" class="setting-input">\r\n                            <option value="3" ${settings.countdownDuration === 3 ? 'selected' : ''}>3 सेकंड</option>\r\n                            <option value="5" ${settings.countdownDuration === 5 ? 'selected' : ''}>5 सेकंड</option>\r\n                            <option value="10" ${settings.countdownDuration === 10 ? 'selected' : ''}>10 सेकंड</option>\r\n                        </select>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-save"></i> डेटा प्रबंधन</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="autosave-toggle" ${settings.autoSaveResults ? 'checked' : ''}>\r\n                        <span>परीक्षण परिणाम को स्वतः सहेजें</span>\r\n                    </label>\r\n                </div>\r\n                <div class="setting-buttons">\r\n                    <button id="export-btn" class="setting-btn">\r\n                        <i class="fas fa-download"></i> निर्यात डेटा\r\n                    </button>\r\n                    <button id="clear-btn" class="setting-btn danger">\r\n                        <i class="fas fa-trash"></i> सभी डेटा साफ़ करें\r\n                    </button>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-footer">\r\n                <button id="reset-settings" class="setting-btn">\r\n                    <i class="fas fa-undo"></i> डिफ़ॉल्ट सेटिंग्स पुनर्स्थापित करें\r\n                </button>\r\n            </div>\r\n        `;
        ...
        document.getElementById('clear-btn').addEventListener('click', () => {
            if (confirm('क्या आप वाकई सारा डेटा साफ़ करना चाहते हैं? इस एक्शन को वापस नहीं किया जा सकता।')) {
                localStorage.clear();
                this.showNotification('डेटा साफ़ किया गया', 'success');
                setTimeout(() => location.reload(), 1000);
            }
        });
        
        document.getElementById('reset-settings').addEventListener('click', () => {
            if (confirm('क्या आप वाकई डिफ़ॉल्ट सेटिंग्स पुनर्स्थापित करना चाहते हैं?')) {
                this.updateSettings({
                    theme: 'dark',
                    soundEnabled: false,
                    animationsEnabled: true,
                ...
                this.showNotification('डिफ़ॉल्ट सेटिंग्स पुनर्स्थापित की गईं', 'success');
            }
        });
    }
    ...
    exportData() {
        try {
            ...
            this.showNotification('डेटा सफलतापूर्वक निर्यात किया गया', 'success');
        } catch (error) {
            console.error('Failed to export data:', error);
            this.showNotification('निर्यात विफल, कृपया पुनः प्रयास करें', 'error');
        }
    }
