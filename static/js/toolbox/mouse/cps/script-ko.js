...
class CPSTester {
    constructor() {
        ...
        this.buttonModes = {
            any: { 
                name: 'Any Button', 
                icon: 'fas fa-mouse',
                description: '왼쪽, 오른쪽, 가운데 버튼 모두 클릭 가능'
            },
            left: { 
                name: 'Left Button Mode', 
                icon: 'fas fa-hand-point-up',
                description: '클릭하려면 마우스 왼쪽 버튼만 사용하세요.'
            },
            right: { 
                name: 'Right Button Mode', 
                icon: 'fas fa-hand-point-down',
                description: '마우스 오른쪽 버튼만 사용하여 클릭하세요.'
            },
            middle: { 
                name: 'Middle Button Mode', 
                icon: 'fas fa-mouse-pointer',
                description: '클릭하려면 마우스 가운데 버튼만 사용하세요.'
            }
        };
    ...
    init() {
        ...
        this.showNotification('마우스를 테스트 영역으로 이동하여 활성화한 다음 마우스를 클릭하여 카운트다운을 시작하세요.', 'info');
    ...
    setMode(modeId) {
        if (this.state.testStatus === 'running' || this.state.testStatus === 'counting') {
            this.showNotification('테스트 진행 중, 모드를 전환할 수 없습니다!', 'error');
            return;
        }
        ...
        this.showNotification(`${mode.name}로 전환됨`, 'success');
    }
    ...
    setButtonMode(modeId) {
        ...
        this.showNotification(`${this.buttonModes[modeId].name}로 전환됨`, 'success');
    }
    ...
    activateTestArea() {
        ...
        if (this.state.testStatus === 'finished') {
            this.showNotification('테스트가 완료되었습니다. 새 테스트를 시작하려면 "테스트 다시 시작" 버튼을 클릭하세요.', 'info');
            return;
        }
        ...
        const buttonMode = this.buttonModes[this.state.buttonMode];
        document.getElementById('click-title').textContent = '시작 준비';
        ...
        this.showNotification(`테스트 영역 활성화, ${instructionText}`, 'info');
    }
    ...
    deactivateTestArea() {
        if (this.state.testStatus === 'active') {
            ...
            // Update instruction information
            document.getElementById('click-title').textContent = '마우스 CPS 테스트';
            ...
            this.showNotification('테스트 영역이 비활성화되었습니다.', 'info');
        }
    }
    ...
    handleMouseDown(event) {
        ...
        if (this.state.testStatus === 'active') {
            ...
            if (!this.isButtonAllowed(event.button)) {
                ...
                this.showNotification(`현재 모드에서는 ${allowedButton} 클릭만 허용됩니다.`, 'error');
                return;
            }
        ...
        if (this.state.testStatus === 'running') {
            ...
            if (!this.isButtonAllowed(event.button)) {
                ...
                if (!this.state.lastButtonWarning || Date.now() - this.state.lastButtonWarning > 2000) {
                    this.showNotification(`${allowedButton}를 사용하여 클릭하세요.`, 'error');
    ...
    startCountdown() {
        ...
        // Update instruction information
        document.getElementById('click-title').textContent = '시작 준비';
        document.getElementById('click-instruction').textContent = '카운트다운이 곧 ​​시작됩니다';
    ...
    beginCountdown() {
        ...
        const countdownInterval = setInterval(() => {
            countdownElement.textContent = count > 0 ? count.toString() : '가다!';
    ...
    beginTest() {
        ...
        // Update instruction information
        document.getElementById('click-title').textContent = '테스트 진행 중';
        ...
        this.showNotification('테스트가 시작되었습니다!', 'success');
    }
    ...
    finishTest() {
        ...
        document.getElementById('click-title').textContent = '테스트 완료';
        document.getElementById('click-instruction').textContent = '새 테스트를 시작하려면 아래의 "테스트 다시 시작" 버튼을 클릭하세요.';
        ...
        this.showNotification('테스트 완료! 새로운 테스트를 시작하려면 "테스트 다시 시작" 버튼을 클릭하세요.', 'success');
    }
    ...
    resetTest() {
        ...
        resultsDashboard.innerHTML = `\r\n            <h2><i class="fas fa-chart-bar"></i> 테스트 결과</h2>\r\n            <div class="no-results">\r\n                <i class="fas fa-mouse-pointer" style="font-size: 3rem; color: var(--text-muted); display: block; text-align: center; margin: 20px 0;"></i>\r\n                <p style="text-align: center; color: var(--text-secondary);">테스트 완료 후 결과 보기</p>\r\n            </div>\r\n        `;
        
        this.updateRealTimeStats();
        
        this.showNotification('재설정이 완료되었습니다. 마우스를 테스트 영역으로 이동하여 활성화하고 새 테스트를 시작하세요.', 'info');
    }
    ...
    updateControlButtons() {
        ...
        switch (this.state.testStatus) {
            case 'running':
                ...
                restartBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 테스트 진행 중...';
                break;
            ...
            case 'waiting':
                ...
                restartBtn.classList.remove('highlight');
                restartBtn.innerHTML = '<i class="fas fa-redo"></i> 재시작 테스트';
                break;
                
            case 'finished':
                restartBtn.disabled = false;
                restartBtn.classList.add('highlight');
                restartBtn.innerHTML = '<i class="fas fa-play-circle"></i> 새 테스트를 시작하려면 클릭하세요.';
                break;
    ...
    loadHistory() {
        try {
            ...
            history.slice(0, 5).forEach(item => {
                const historyElement = document.createElement('div');
                historyElement.className = 'history-item';
                historyElement.innerHTML = `\r\n                    <div class="history-header" style="display: flex; justify-content: space-between;">\r\n                        <span class="history-time">${item.timestamp}</span>\r\n                        <span class="history-mode">${item.mode}</span>\r\n                    </div>\r\n                    <div class="history-body" style="display: flex; justify-content: space-between; align-items: center; margin-top: 5px;">\r\n                        <span class="history-cps">${item.cps.toFixed(1)} CPS</span>\r\n                        <span class="history-clicks" style="color: var(--text-secondary); font-size: 0.9em;">${item.totalClicks} 클릭</span>\r\n                    </div>\r\n                    <div class="history-footer" style="display: flex; justify-content: space-between; font-size: 0.8em; color: var(--text-muted); margin-top: 5px;">\r\n                        <span>${item.buttonMode}</span>\r\n                        <span style="color: var(--secondary-color); font-weight: 600;">점수: ${item.score || 0}</span>\r\n                    </div>\r\n                `;
    ...
    toggleTheme() {
        ...
        this.showNotification(`${nextTheme} 테마로 전환됨`, 'success');
    }
    ...
    toggleFullscreen() {
        if (!document.fullscreenElement) {
            document.documentElement.requestFullscreen().catch(err => {
                ...
                this.showNotification('전체 화면 모드로 들어갈 수 없습니다', 'error');
            });
        } else {
    ...
    createSettingsPanel() {
        ...
        container.innerHTML = `\r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-palette"></i> 모양 설정</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <span>테마</span>\r\n                        <select id="theme-select" class="setting-input">\r\n                            <option value="dark" ${settings.theme === 'dark' ? 'selected' : ''}>다크 테마</option>\r\n                            <option value="light" ${settings.theme === 'light' ? 'selected' : ''}>라이트 테마</option>\r\n                            <option value="high-contrast" ${settings.theme === 'high-contrast' ? 'selected' : ''}>고대비</option>\r\n                            <option value="eye-care" ${settings.theme === 'eye-care' ? 'selected' : ''}>아이 케어 모드</option>\r\n                        </select>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-film"></i> 애니메이션 효과</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="animations-toggle" ${settings.animationsEnabled ? 'checked' : ''}>\r\n                        <span>애니메이션 활성화</span>\r\n                    </label>\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="particles-toggle" ${settings.showParticles ? 'checked' : ''}>\r\n                        <span>클릭 입자 효과 표시</span>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-cogs"></i> 테스트 설정</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <span>카운트다운 기간</span>\r\n                        <select id="countdown-select" class="setting-input">\r\n                            <option value="3" ${settings.countdownDuration === 3 ? 'selected' : ''}>3초</option>\r\n                            <option value="5" ${settings.countdownDuration === 5 ? 'selected' : ''}>5초</option>\r\n                            <option value="10" ${settings.countdownDuration === 10 ? 'selected' : ''}>10초</option>\r\n                        </select>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-save"></i> 데이터 관리</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="autosave-toggle" ${settings.autoSaveResults ? 'checked' : ''}>\r\n                        <span>자동 저장 테스트 결과</span>\r\n                    </label>\r\n                </div>\r\n                <div class="setting-buttons">\r\n                    <button id="export-btn" class="setting-btn">\r\n                        <i class="fas fa-download"></i> 데이터 내보내기\r\n                    </button>\r\n                    <button id="clear-btn" class="setting-btn danger">\r\n                        <i class="fas fa-trash"></i> 모든 데이터 지우기\r\n                    </button>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-footer">\r\n                <button id="reset-settings" class="setting-btn">\r\n                    <i class="fas fa-undo"></i> 기본 설정 복원\r\n                </button>\r\n            </div>\r\n        `;
        ...
        document.getElementById('reset-settings').addEventListener('click', () => {
            if (confirm('기본 설정을 복원하시겠습니까?')) {
                ...
                this.showNotification('기본 설정이 복원되었습니다.', 'success');
            }
        });
    }
    ...
    exportData() {
        try {
            ...
            this.showNotification('내보내기에 실패했습니다. 다시 시도해 주세요.', 'error');
        }
    }
