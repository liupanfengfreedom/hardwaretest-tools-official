...
class CPSTester {
    constructor() {
        ...
        this.buttonModes = {
            any: { 
                name: 'Any Button', 
                icon: 'fas fa-mouse',
                description: 'Các nút trái, phải và giữa đều có thể được nhấp vào'
            },
            left: { 
                name: 'Left Button Mode', 
                icon: 'fas fa-hand-point-up',
                description: 'Chỉ dùng chuột trái để click'
            },
            right: { 
                name: 'Right Button Mode', 
                icon: 'fas fa-hand-point-down',
                description: 'Chỉ dùng chuột phải để click'
            },
            middle: { 
                name: 'Middle Button Mode', 
                icon: 'fas fa-mouse-pointer',
                description: 'Chỉ sử dụng nút giữa chuột để click'
            }
        };
    ...
    init() {
        ...
        this.showNotification('Di chuyển chuột đến khu vực kiểm tra để kích hoạt, sau đó nhấn chuột để bắt đầu đếm ngược', 'info');
    ...
    setMode(modeId) {
        if (this.state.testStatus === 'running' || this.state.testStatus === 'counting') {
            this.showNotification('Đang kiểm tra, không thể chuyển đổi chế độ!', 'error');
            return;
        }
        ...
        this.showNotification(`Đã chuyển sang ${mode.name}`, 'success');
    }
    ...
    setButtonMode(modeId) {
        ...
        this.showNotification(`Đã chuyển sang ${this.buttonModes[modeId].name}`, 'success');
    }
    ...
    activateTestArea() {
        ...
        if (this.state.testStatus === 'finished') {
            this.showNotification('Bài kiểm tra đã hoàn tất, vui lòng nhấp vào nút "Khởi động lại bài kiểm tra" để bắt đầu bài kiểm tra mới', 'info');
            return;
        }
        ...
        const buttonMode = this.buttonModes[this.state.buttonMode];
        document.getElementById('click-title').textContent = 'Sẵn sàng để bắt đầu';
        ...
        this.showNotification(`Khu vực thử nghiệm được kích hoạt, ${instructionText}`, 'info');
    }
    ...
    deactivateTestArea() {
        if (this.state.testStatus === 'active') {
            ...
            // Update instruction information
            document.getElementById('click-title').textContent = 'Kiểm tra CPS chuột';
            ...
            this.showNotification('Khu vực kiểm tra bị vô hiệu hóa', 'info');
        }
    }
    ...
    handleMouseDown(event) {
        ...
        if (this.state.testStatus === 'active') {
            ...
            if (!this.isButtonAllowed(event.button)) {
                ...
                this.showNotification(`Chế độ hiện tại chỉ cho phép nhấp chuột ${allowedButton}`, 'error');
                return;
            }
        ...
        if (this.state.testStatus === 'running') {
            ...
            if (!this.isButtonAllowed(event.button)) {
                ...
                if (!this.state.lastButtonWarning || Date.now() - this.state.lastButtonWarning > 2000) {
                    this.showNotification(`Vui lòng sử dụng ${allowedButton} để nhấp vào`, 'error');
    ...
    startCountdown() {
        ...
        // Update instruction information
        document.getElementById('click-title').textContent = 'Sẵn sàng để bắt đầu';
        document.getElementById('click-instruction').textContent = 'Việc đếm ngược sẽ sớm bắt đầu';
    ...
    beginCountdown() {
        ...
        const countdownInterval = setInterval(() => {
            countdownElement.textContent = count > 0 ? count.toString() : 'Đi!';
    ...
    beginTest() {
        ...
        // Update instruction information
        document.getElementById('click-title').textContent = 'Đang tiến hành kiểm tra';
        ...
        this.showNotification('Thử nghiệm bắt đầu!', 'success');
    }
    ...
    finishTest() {
        ...
        document.getElementById('click-title').textContent = 'Đã hoàn thành kiểm tra';
        document.getElementById('click-instruction').textContent = 'Nhấp vào nút "Khởi động lại bài kiểm tra" bên dưới để bắt đầu bài kiểm tra mới';
        ...
        this.showNotification('Kiểm tra đã hoàn thành! Nhấp vào nút "Khởi động lại bài kiểm tra" để bắt đầu bài kiểm tra mới', 'success');
    }
    ...
    resetTest() {
        ...
        resultsDashboard.innerHTML = `\r\n            <h2><i class="fas fa-chart-bar"></i> Kết quả xét nghiệm</h2>\r\n            <div class="no-results">\r\n                <i class="fas fa-mouse-pointer" style="font-size: 3rem; color: var(--text-muted); display: block; text-align: center; margin: 20px 0;"></i>\r\n                <p style="text-align: center; color: var(--text-secondary);">Xem kết quả sau khi hoàn thành bài kiểm tra</p>\r\n            </div>\r\n        `;
        
        this.updateRealTimeStats();
        
        this.showNotification('Thiết lập lại hoàn tất, di chuyển chuột đến khu vực kiểm tra để kích hoạt và bắt đầu kiểm tra mới', 'info');
    }
    ...
    updateControlButtons() {
        ...
        switch (this.state.testStatus) {
            case 'running':
                ...
                restartBtn.innerHTML = 'Đang tiến hành thử nghiệm <i class="fas fa-spinner fa-spin"></i>...';
                break;
            ...
            case 'waiting':
                ...
                restartBtn.classList.remove('highlight');
                restartBtn.innerHTML = 'Kiểm tra khởi động lại <i class="fas fa-redo"></i>';
                break;
                
            case 'finished':
                restartBtn.disabled = false;
                restartBtn.classList.add('highlight');
                restartBtn.innerHTML = '<i class="fas fa-play-circle"></i> Nhấp để bắt đầu thử nghiệm mới';
                break;
    ...
    loadHistory() {
        try {
            ...
            history.slice(0, 5).forEach(item => {
                const historyElement = document.createElement('div');
                historyElement.className = 'history-item';
                historyElement.innerHTML = `\r\n                    <div class="history-header" style="display: flex; justify-content: space-between;">\r\n                        <span class="history-time">${item.timestamp}</span>\r\n                        <span class="history-mode">${item.mode}</span>\r\n                    </div>\r\n                    <div class="history-body" style="display: flex; justify-content: space-between; align-items: center; margin-top: 5px;">\r\n                        <span class="history-cps">${item.cps.toFixed(1)} CPS</span>\r\n                        <span class="history-clicks" style="color: var(--text-secondary); font-size: 0.9em;">${item.totalClicks} lần nhấp</span>\r\n                    </div>\r\n                    <div class="history-footer" style="display: flex; justify-content: space-between; font-size: 0.8em; color: var(--text-muted); margin-top: 5px;">\r\n                        <span>${item.buttonMode}</span>\r\n                        Điểm <span style="color: var(--secondary-color); font-weight: 600;">: ${item.score || 0}</span>\r\n                    </div>\r\n                `;
    ...
    toggleTheme() {
        ...
        this.showNotification(`Đã chuyển sang chủ đề ${nextTheme}`, 'success');
    }
    ...
    toggleFullscreen() {
        if (!document.fullscreenElement) {
            document.documentElement.requestFullscreen().catch(err => {
                ...
                this.showNotification('Không vào được chế độ toàn màn hình', 'error');
            });
        } else {
    ...
    createSettingsPanel() {
        ...
        container.innerHTML = `\r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-palette"></i> Cài đặt giao diện</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <span>Chủ đề</span>\r\n                        <select id="theme-select" class="setting-input">\r\n                            <option value="dark" ${settings.theme === 'dark' ? 'selected' : ''}>Chủ đề tối</option>\r\n                            <option value="light" ${settings.theme === 'light' ? 'selected' : ''}>Chủ đề nhẹ</option>\r\n                            <option value="high-contrast" ${settings.theme === 'high-contrast' ? 'selected' : ''}>Độ tương phản cao</option>\r\n                            <option value="eye-care" ${settings.theme === 'eye-care' ? 'selected' : ''}>Chế độ chăm sóc mắt</option>\r\n                        </select>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-film"></i> Hiệu ứng hoạt hình</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="animations-toggle" ${settings.animationsEnabled ? 'checked' : ''}>\r\n                        <span>Bật Ảnh động</span>\r\n                    </label>\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="particles-toggle" ${settings.showParticles ? 'checked' : ''}>\r\n                        <span>Hiển thị hiệu ứng hạt nhấp chuột</span>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-cogs"></i> Cài đặt kiểm tra</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <span>Thời gian đếm ngược</span>\r\n                        <select id="countdown-select" class="setting-input">\r\n                            <option value="3" ${settings.countdownDuration === 3 ? 'selected' : ''}>3 giây</option>\r\n                            <option value="5" ${settings.countdownDuration === 5 ? 'selected' : ''}>5 giây</option>\r\n                            <option value="10" ${settings.countdownDuration === 10 ? 'selected' : ''}>10 giây</option>\r\n                        </select>\r\n                    </label>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-section">\r\n                <h3><i class="fas fa-save"></i> Quản lý dữ liệu</h3>\r\n                <div class="setting-group">\r\n                    <label class="setting-label">\r\n                        <input type="checkbox" id="autosave-toggle" ${settings.autoSaveResults ? 'checked' : ''}>\r\n                        <span>Tự động lưu kết quả kiểm tra</span>\r\n                    </label>\r\n                </div>\r\n                <div class="setting-buttons">\r\n                    <button id="export-btn" class="setting-btn">\r\n                        <i class="fas fa-download"></i> Xuất dữ liệu\r\n                    </button>\r\n                    <button id="clear-btn" class="setting-btn danger">\r\n                        <i class="fas fa-trash"></i> Xóa tất cả dữ liệu\r\n                    </button>\r\n                </div>\r\n            </div>\r\n            \r\n            <div class="settings-footer">\r\n                <button id="reset-settings" class="setting-btn">\r\n                    <i class="fas fa-undo"></i> Khôi phục cài đặt mặc định\r\n                </button>\r\n            </div>\r\n        `;
        ...
        document.getElementById('clear-btn').addEventListener('click', () => {
            if (confirm('Bạn có chắc chắn muốn xóa tất cả dữ liệu không? Không thể hoàn tác hành động này.')) {
                localStorage.clear();
                this.showNotification('Đã xóa dữ liệu', 'success');
    ...
    exportData() {
        try {
            ...
            this.showNotification('Dữ liệu được xuất thành công', 'success');
        } catch (error) {
            console.error('Failed to export data:', error);
            this.showNotification('Xuất không thành công, vui lòng thử lại', 'error');
        }
    }
