...
const RATING_PAGE_ID = "page62"; // 实际使用可用 window.location.pathname
...
function checkUserRatingStatus() {
    ...
    if (userRated) {
        const score = parseInt(userRated);
        // 禁用所有评分按钮
        document.querySelectorAll('.rating-hit-area').forEach(area => {
            area.style.pointerEvents = 'none';
            area.style.opacity = '0.5';
            area.title = `您已经给出了${score}星的评分，谢谢！`;
        });
        
        // 显示已评分提示
        ratingStatusMsg.style.color = "#666";
        ratingStatusMsg.innerText = `感谢您给出${score}星的评分！`;
...
async function ratingSubmitRating(score) {
    ...
    // 先检查本地是否已评分
    const userRatedScore = checkUserRatingStatus();
    if (userRatedScore !== null) {
        // 已评分，显示对应提示
        ratingStatusMsg.style.color = "#666";
        ratingStatusMsg.innerText = `您已经给出了${userRatedScore}星的评分，谢谢！`;
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            ratingStatusMsg.innerText = "评分成功！";
