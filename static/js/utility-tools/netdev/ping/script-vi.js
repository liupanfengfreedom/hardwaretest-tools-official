...
function renderHistory() {
    ...
    if (h.length === 0) {
        list.innerHTML = `<div class="p-4 text-slate-500 text-sm italic text-center">Không có lịch sử tìm kiếm</div>`;
        return;
    }
...
function togglePing() {
    if(!running) {
        ...
        if(!target) return alert("Vui lòng nhập địa chỉ hoặc tên miền mục tiêu");
...
function start(target) {
    ...
    btn.innerHTML = 'Kiểm tra dừng <i data-lucide="square"></i>';
...
function stop() {
    ...
    const btn = document.getElementById('btn');
    btn.innerHTML = '<i data-lucide="play"></i> Bắt đầu thử nghiệm';
