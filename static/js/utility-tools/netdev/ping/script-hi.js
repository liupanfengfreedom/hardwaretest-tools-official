...
function renderHistory() {
    ...
    if (h.length === 0) {
        list.innerHTML = `<div class="p-4 text-slate-500 text-sm italic text-center">कोई खोज इतिहास नहीं</div>`;
        return;
    }
...
function togglePing() {
    if(!running) {
        ...
        if(!target) return alert("कृपया एक लक्ष्य पता या डोमेन दर्ज करें");
...
function start(target) {
    ...
    btn.innerHTML = '<i data-lucide="square"></i> स्टॉप टेस्ट';
...
function stop() {
    ...
    const btn = document.getElementById('btn');
    btn.innerHTML = '<i data-lucide="play"></i> परीक्षण प्रारंभ करें';
