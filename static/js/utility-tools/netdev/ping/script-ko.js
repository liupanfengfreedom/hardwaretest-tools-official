...
function renderHistory() {
    ...
    if (h.length === 0) {
        list.innerHTML = `<div class="p-4 text-slate-500 text-sm italic text-center">검색 기록이 없습니다</div>`;
        return;
    }
...
function togglePing() {
    if(!running) {
        ...
        if(!target) return alert("대상 주소나 도메인을 입력하세요.");
...
function start(target) {
    ...
    btn.innerHTML = '<i data-lucide="square"></i> 테스트 중지';
...
function stop() {
    ...
    const btn = document.getElementById('btn');
    btn.innerHTML = '<i data-lucide="play"></i> 테스트 시작';
