...
function renderHistory() {
    ...
    if (h.length === 0) {
        list.innerHTML = `<div class="p-4 text-slate-500 text-sm italic text-center">検索履歴はありません</div>`;
        return;
    }
...
function togglePing() {
    if(!running) {
        ...
        if(!target) return alert("ターゲットのアドレスまたはドメインを入力してください");
...
function start(target) {
    ...
    btn.innerHTML = '<i data-lucide="square"></i> テストの停止';
...
function stop() {
    ...
    const btn = document.getElementById('btn');
    btn.innerHTML = '<i data-lucide="play"></i> テストの開始';
