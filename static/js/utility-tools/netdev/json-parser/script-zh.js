...
function clearAll() {
  ...
  reset();
  setStatus('', '等待输入');
  document.getElementById('schar').textContent = '0 字符';
  document.getElementById('sline').textContent = '0 行';
}

function reset() {
  const emp = n => `<div class="empty"><div class="empty-ico">{ }</div>解析后显示</div>`;
  document.getElementById('tree').innerHTML = '<div class="empty"><div class="empty-ico">{ }</div>输入 JSON 并点击解析</div>';
  document.getElementById('out-fmt').innerHTML = emp();
  document.getElementById('out-cmp').innerHTML = emp();
  document.getElementById('sgrid').innerHTML = `<div style="grid-column:1/-1"><div class="empty" style="height:200px"><div class="empty-ico">{ }</div>解析后显示统计</div></div>`;
  document.getElementById('errbox').classList.remove('on');
}

function onInp() {
  const v = document.getElementById('inp').value;
  document.getElementById('schar').textContent = v.length.toLocaleString() + ' 字符';
  document.getElementById('sline').textContent = (v ? v.split('\n').length : 0) + ' 行';
  clearTimeout(timer);
  timer = setTimeout(() => { if (v.trim()) parseJSON(); else { reset(); setStatus('', '等待输入'); } }, 500);
}
...
function doSearch() {
  ...
  document.getElementById('scnt').textContent = count ? `${count} 结果` : '无结果';
}
...
function renderStats(data, raw) {
  ...
  const items = [
    ['原始大小', fmtB(sz)], ['压缩大小', fmtB(csz)],
    ['对象数', s.obj], ['数组数', s.arr],
    ['字符串', s.str], ['数字', s.num],
    ['布尔值', s.bool], ['null', s.nul],
    ['总 Key 数', s.keys], ['最大深度', s.depth],
  ];
