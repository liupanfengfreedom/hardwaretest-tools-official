  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="Excluir este registro">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('Por favor insira o URL'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'Carregando...';
  ...
  try {
    ...
    alert('Erro no formato JSON dos cabeçalhos de solicitação');
    return;
  }
...
// ========== Chave-Valor Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Chaveboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlChave || e.metaChave)) sendRequest(); };
});