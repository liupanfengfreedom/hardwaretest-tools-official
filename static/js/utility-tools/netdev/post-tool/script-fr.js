  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="Supprimer cet enregistrement">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('Veuillez entrer l'URL'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'Chargement...';
  ...
  try {
    ...
    alert('Erreur de format JSON des en-têtes de requête');
    return;
  }
...
// ========== Clé-Valeur Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Cléboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlClé || e.metaClé)) sendRequest(); };
});