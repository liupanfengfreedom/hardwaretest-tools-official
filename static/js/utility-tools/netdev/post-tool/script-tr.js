  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="Bu kaydı sil">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('Lütfen URL'yi girin'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'Yükleniyor...';
  ...
  try {
    ...
    alert('İstek üstbilgileri JSON biçimi hatası');
    return;
  }
...
// ========== Anahtar-Değer Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Anahtarboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlAnahtar || e.metaAnahtar)) sendRequest(); };
});