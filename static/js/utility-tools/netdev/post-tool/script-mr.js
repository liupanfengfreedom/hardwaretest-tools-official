  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="हा रेकॉर्ड हटवा">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('कृपया URL प्रविष्ट करा'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'लोड करत आहे...';
  ...
  try {
    ...
    alert('शीर्षलेख JSON स्वरूपन त्रुटीची विनंती करा');
    return;
  }
...
// ========== की-मूल्य Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // कीboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlकी || e.metaकी)) sendRequest(); };
});