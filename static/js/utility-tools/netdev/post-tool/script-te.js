  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="ఈ రికార్డును తొలగించండి">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('దయచేసి URLని నమోదు చేయండి'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'లోడ్ అవుతోంది...';
  ...
  try {
    ...
    alert('అభ్యర్థన శీర్షికలు JSON ఫార్మాట్ లోపం');
    return;
  }
...
// ========== కీ-విలువ Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // కీboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlకీ || e.metaకీ)) sendRequest(); };
});