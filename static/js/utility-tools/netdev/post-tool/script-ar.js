  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="حذف هذا السجل">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('الرجاء إدخال عنوان URL'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'تحميل...';
  ...
  try {
    ...
    alert('خطأ في تنسيق طلب الرؤوس JSON');
    return;
  }
...
// ========== مفتاح-قيمة Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // مفتاحboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlمفتاح || e.metaمفتاح)) sendRequest(); };
});