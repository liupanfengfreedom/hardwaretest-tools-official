  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="இந்த பதிவை நீக்கவும்">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('URL ஐ உள்ளிடவும்'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'ஏற்றுகிறது...';
  ...
  try {
    ...
    alert('கோரிக்கை தலைப்புகள் JSON வடிவமைப்பு பிழை');
    return;
  }
...
// ========== முக்கிய-மதிப்பு Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // முக்கியboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlமுக்கிய || e.metaமுக்கிய)) sendRequest(); };
});