  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="Hapus catatan ini">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('Silakan masukkan URL'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'Memuat...';
  ...
  try {
    ...
    alert('Kesalahan format JSON header permintaan');
    return;
  }
...
// ========== Kunci-Nilai Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Kunciboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlKunci || e.metaKunci)) sendRequest(); };
});