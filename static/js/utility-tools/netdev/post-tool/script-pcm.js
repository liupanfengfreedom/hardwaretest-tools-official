  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="Delete dis record">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('Abeg put URL'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'Dey load...';
  ...
  try {
    ...
    alert('Request header JSON format get error');
    return;
  }
