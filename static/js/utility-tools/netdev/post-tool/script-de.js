  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="Löschen Sie diesen Datensatz">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('Bitte geben Sie die URL ein'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'Laden...';
  ...
  try {
    ...
    alert('Fehler im JSON-Format der Anforderungsheader');
    return;
  }
...
// ========== Schlüssel-Wert Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Schlüsselboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlSchlüssel || e.metaSchlüssel)) sendRequest(); };
});