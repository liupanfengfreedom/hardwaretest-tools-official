  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="Eliminar este registro">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('Por favor ingrese la URL'); return; }
  ...
  try {
    ...
    alert('Error de formato JSON de encabezados de solicitud');
    return;
  }
...
// ========== Llave-Valor Pair Editing Logic (Request Headers/Body) ==========
function createFieldRow(containerId, key = '', value = '') {
  ...
  kInp.placeholder = 'Llave'; kInp.value = key;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Llaveboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlLlave || e.metaLlave)) sendRequest(); };
});