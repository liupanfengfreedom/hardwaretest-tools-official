  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="এই রেকর্ড মুছুন">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('URL লিখুন'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'লোড হচ্ছে...';
  ...
  try {
    ...
    alert('শিরোনাম JSON বিন্যাস ত্রুটি অনুরোধ');
    return;
  }
...
// ========== চাবি-মান Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // চাবিboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlচাবি || e.metaচাবি)) sendRequest(); };
});