  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="اس ریکارڈ کو حذف کریں۔">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  if (!url) { alert('براہ کرم URL درج کریں۔'); return; }

  // Record history
  saveToHistory(url);

  document.getElementById('response').textContent = 'لوڈ ہو رہا ہے...';
  ...
  try {
    ...
    alert('ہیڈر کی درخواست کریں JSON فارمیٹ کی خرابی۔');
    return;
  }
...
// ========== چابی-قدر Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // چابیboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlچابی || e.metaچابی)) sendRequest(); };
});