  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map(url => `
      <div class="history-item" data-url="${encodeURIComponent(url)}">
        ...
        <button class="history-delete-btn" title="Удалить эту запись">×</button>
      </div>
    `).join('');
...
async function sendRequest() {
  ...
  document.getElementById('response').textContent = 'Загрузка...';
  ...
  try {
    ...
    alert('Ошибка формата JSON заголовков запроса');
    return;
  }
...
// ========== Ключ-Ценить Pair Editing Logic (Request Headers/Body) ==========
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Ключboard shortcuts
  document.onkeydown = (e) => { if (e.key === 'Enter' && (e.ctrlКлюч || e.metaКлюч)) sendRequest(); };
});