...
function renderHistList(filter){
  ...
  if(!filtered.length){
    list.innerHTML='<div class="url-hist-empty">'+(arr.length?'无匹配记录':'暂无历史记录')+'</div>';
    return;
  }
  list.innerHTML=filtered.map(function(item,idx){
    ...
    return '<div class="url-hist-item" data-url="'+esc(host)+'" data-proto="'+protoStr+'" data-idx="'+idx+'" onmousedown="pickHist(event,this)">'
      ...
      +'<button class="url-hist-del" onmousedown="delHist(event,\''+esc(full)+'\')" title="删除">✕</button>'
      +'</div>';
  }).join('');
}
...
function doConn(){
  ...
  if(cb){ cb.dataset.origText = cb.dataset.origText || cb.textContent; cb.textContent = '正在连接…'; cb.disabled = true; cb.classList.add('busy'); }
  ...
  setSt('ing','正在连接...');
  try{ws=new WebSocket(url);}catch(e){addSys('连接失败: '+e.message,'error');setSt('err','ERROR');busyState='';if(cb&&cb.dataset.origText){cb.textContent=cb.dataset.origText;cb.disabled=false;cb.classList.remove('busy');}return;}
  ws.onopen=function(){
    busyState = '';
    setSt('on','CONNECTED');
    ...
    document.getElementById('sendbtn').disabled=false;connAt=Date.now();startUp();sent=0;recv=0;bytes=0;updStats();addSys('已连接到 '+url);startPing();
  };
  ...
  ws.onclose=function(e){
    ...
    document.getElementById('sendbtn').disabled=true;stopUp();stopPing();addSys('连接关闭 [code: '+e.code+'] '+(e.reason||''));ws=null;
  };
  ws.onerror=function(){
    busyState = '';
    setSt('err','ERROR');addSys('连接发生错误','error');
    var cb2=document.getElementById('cb2'), db=document.getElementById('db');
...
function doDisc(){
  ...
  if(db){ db.dataset.origText = db.dataset.origText || db.textContent; db.textContent = '正在断开…'; db.disabled = true; db.classList.add('busy'); }
  if(ws){
    try{ ws.close(1000,'User disconnect'); }
    catch(e){ /* ignore */ }
    // do not null-out ws here; let onclose handle cleanup
  } else {
    // no active socket — clear busy state and restore
    busyState = '';
...
function rLog(){var el=document.getElementById('ml');el.innerHTML='';var s=document.getElementById('srch').value.toLowerCase();var f=log.filter(function(e){return(curF==='all'||e.dir===curF)&&(!s||e.content.toLowerCase().includes(s))});if(!f.length){el.innerHTML='<div class="empty"><span>无匹配消息</span></div>';return;}f.forEach(function(e){appendE(e);});}
...
function clrLog(){log=[];sent=0;recv=0;bytes=0;updStats();document.getElementById('ml').innerHTML='<div class="empty"><span>日志已清空</span></div>';}
...
function fmtJ(){var el=document.getElementById('inp');try{el.value=JSON.stringify(JSON.parse(el.value),null,2);}catch(e){addSys('JSON 格式无效','error');}}
function minJ(){var el=document.getElementById('inp');try{el.value=JSON.stringify(JSON.parse(el.value));}catch(e){addSys('JSON 格式无效','error');}}
function clrInp(){document.getElementById('inp').value='';}
function esc(s){return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}

// 初始化
addSys('WS Terminal v2.4.0 已就绪');
addSys('推荐：wss://echo.websocket.org — 发送任意内容将原样返回');
addSys('提示：连接成功后每10秒自动发送 ping 检测延迟');

// 页面打开时，如果预设端点中存在已选中项，则预填 URL 输入框
