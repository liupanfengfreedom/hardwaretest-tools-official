...
function renderHistList(filter){
  ...
  if(!filtered.length){
    list.innerHTML='<div class="url-hist-empty">'+(arr.length?'कोई मेल नहीं':'कोई इतिहास नहीं')+'</div>';
    return;
  }
...
function doConn(){
  ...
  if(cb){ cb.dataset.origText = cb.dataset.origText || cb.textContent; cb.textContent = 'कनेक्ट हो रहा है...'; cb.disabled = true; cb.classList.add('busy'); }
...
function doDisc(){
  ...
  if(db){ db.dataset.origText = db.dataset.origText || db.textContent; db.textContent = 'डिस्कनेक्ट हो रहा है...'; db.disabled = true; db.classList.add('busy'); }
  if(ws){
...
function appendE(e){var el=document.getElementById('ml');var emp=el.querySelector('.empty');if(emp)emp.remove();if(curF!=='all'&&e.dir!==curF)return;var s=document.getElementById('srch').value.toLowerCase();if(s&&!e.content.toLowerCase().includes(s))return;var d=document.createElement('div');d.className='mr';var dm={send:'SND',recv:'RCV',sys:'SYS',error:'ERR'};var dc={send:'mdsend',recv:'mdrecv',sys:'mdsys',error:'mderr'};var cc={sys:'mcsys',error:'mcerr'};var disp=e.content;if(e.dir==='recv'||e.dir==='send'){try{disp=JSON.stringify(JSON.parse(e.content),null,2);}catch(x){}}d.innerHTML='<div class="mt">'+e.t+'</div><div class=\'md '+(dc[e.dir]||'mdsys')+'">'+(dm[e.dir]||'एसवाईएस')+'</div><div class=\'mc '+(cc[e.dir]||'')+'">'+esc(disp)+'</div>';el.appendChild(d);if(autoS)el.scrollTop=el.scrollHeight;}
function rLog(){var el=document.getElementById('ml');el.innerHTML='';var s=document.getElementById('srch').value.toLowerCase();var f=log.filter(function(e){return(curF==='all'||e.dir===curF)&&(!s||e.content.toLowerCase().includes(s))});if(!f.length){el.innerHTML='<div class="empty"><span>कोई मेल खाता संदेश नहीं</span></div>';return;}f.forEach(function(e){appendE(e);});}
...
function clrLog(){log=[];sent=0;recv=0;bytes=0;updStats();document.getElementById('ml').innerHTML='<div class="empty"><span>लॉग साफ़ किया गया</span></div>';}
...
function renderLC(){var c=document.getElementById('lc');if(!latH.length){c.innerHTML='';return;}var mx=Math.max.apply(null,latH.concat([1]));c.innerHTML=latH.map(function(v){return'<div class=\'lb\' शैली=\'ऊंचाई:'+Math.max(2,Math.round(v/mx*14))+'px"></div>';}).join('');}
