...
function renderHistList(filter){
  ...
  if(!filtered.length){
    list.innerHTML='<div class="url-hist-empty">'+(arr.length?'Không có kết quả phù hợp':'Không có lịch sử')+'</div>';
    return;
  }
...
function doConn(){
  ...
  if(cb){ cb.dataset.origText = cb.dataset.origText || cb.textContent; cb.textContent = 'Đang kết nối…'; cb.disabled = true; cb.classList.add('busy'); }
...
function doDisc(){
  ...
  if(db){ db.dataset.origText = db.dataset.origText || db.textContent; db.textContent = 'Đang ngắt kết nối…'; db.disabled = true; db.classList.add('busy'); }
  if(ws){
...
function rLog(){var el=document.getElementById('ml');el.innerHTML='';var s=document.getElementById('srch').value.toLowerCase();var f=log.filter(function(e){return(curF==='all'||e.dir===curF)&&(!s||e.content.toLowerCase().includes(s))});if(!f.length){el.innerHTML='<div class="empty"><span>Không có tin nhắn phù hợp</span></div>';return;}f.forEach(function(e){appendE(e);});}
...
function clrLog(){log=[];sent=0;recv=0;bytes=0;updStats();document.getElementById('ml').innerHTML='<div class="empty"><span>Đã xóa nhật ký</span></div>';}
...
function renderLC(){var c=document.getElementById('lc');if(!latH.length){c.innerHTML='';return;}var mx=Math.max.apply(null,latH.concat([1]));c.innerHTML=latH.map(function(v){return'<div class="lb" style="chiều cao:'+Math.max(2,Math.round(v/mx*14))+'px"></div>';}).join('');}
