...
function renderHistList(filter){
  ...
  if(!filtered.length){
    list.innerHTML='<div class="url-hist-empty">'+(arr.length?'一致しません':'履歴なし')+'</div>';
    return;
  }
...
function doConn(){
  ...
  if(cb){ cb.dataset.origText = cb.dataset.origText || cb.textContent; cb.textContent = '接続中…'; cb.disabled = true; cb.classList.add('busy'); }
...
function doDisc(){
  ...
  if(db){ db.dataset.origText = db.dataset.origText || db.textContent; db.textContent = '切断中…'; db.disabled = true; db.classList.add('busy'); }
  if(ws){
...
function clrLog(){log=[];sent=0;recv=0;bytes=0;updStats();document.getElementById('ml').innerHTML='<div class="empty"><span>ログがクリアされました</span></div>';}
...
function renderLC(){var c=document.getElementById('lc');if(!latH.length){c.innerHTML='';return;}var mx=Math.max.apply(null,latH.concat([1]));c.innerHTML=latH.map(function(v){return'<div class="lb" style="高さ:'+Math.max(2,Math.round(v/mx*14))+'px"></div>';}).join('');}
