...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            if (countElement) {
                countElement.textContent = `${visibleInSection} ferramentas`;
            }
        ...
        if (resultsHint) {
            if (term.length > 0) {
                resultsHint.textContent = "Foram encontradas {count} ferramentas correspondentes".replace('{count}', visibleCardsCount);
            } else {
                resultsHint.textContent = "Digite uma palavra-chave para encontrar rapidamente a ferramenta que você precisa";
            }
        }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = "Pressione ESC para limpar • Ctrl+K para pesquisar rapidamente";
    });
