...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        if (term.length > 0) {
            ...
            if (resultsHint) {
                resultsHint.textContent = `${visibleCardsCount} 매칭 도구 발견`;
            }
        } else {
            const resultsHint = document.querySelector('.search-hint');
            if (resultsHint) {
                resultsHint.textContent = '필요한 도구를 빠르게 찾으려면 키워드를 입력하세요.';
            }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = '지우려면 ESC를 누르세요. • 빠르게 검색하려면 Ctrl+K를 누르세요.';
    });
