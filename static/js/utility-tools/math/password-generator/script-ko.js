...
function generate() {
    ...
    setTimeout(() => {
        try {
            ...
            if (!pool) {
                output.innerText = "사용 가능한 문자가 없습니다. 제외 항목을 줄여주세요.";
                updateMeter(0, 0);
                updateAnalysis("", 0);
                resetGenerateButton();
                return;
            ...
            if (requirements.length > len) {
                output.innerText = "선택한 길이에 필요한 유형이 너무 많습니다. 길이를 늘리거나 요구 사항을 줄이세요.";
                updateMeter(0, 0);
                updateAnalysis("", 0);
                resetGenerateButton();
                return;
            ...
            for (let type of requirements) {
                if (getAvailableChars(type).length === 0) {
                    output.innerText = `"${type}"에는 사용 가능한 문자가 없습니다. 요구 사항을 선택 취소하거나 해당 문자 제외를 중지하세요.`;
                    updateMeter(0, 0);
                    updateAnalysis("", 0);
                    resetGenerateButton();
                    return;
            ...
            output.innerText = "생성에 실패했습니다. 다시 시도해 주세요.";
...
function resetGenerateButton() {
    ...
    generateIcon.textContent = '🔄';
    generateText.textContent = '새 비밀번호 생성';
}
...
function updateCrackTime(entropy) {
    ...
    crackTime.textContent = `예상 크랙 시간: ${timeText}`;
}
...
function copyAction() {
    ...
    navigator.clipboard.writeText(text).then(() => {
        ...
        toast.textContent = "복사에 실패했습니다. 수동으로 선택하고 복사하세요.";
        ...
        setTimeout(() => {
            ...
            toast.textContent = "비밀번호가 클립보드에 복사되었습니다!";
        }, 2000);
    });
}
...
function exportPasswords() {
    ...
    toast.textContent = `${passwordHistory.length} 비밀번호를 내보냈습니다.`;
    toast.classList.add('show');
    setTimeout(() => {
        toast.classList.remove('show');
    }, 2000);
