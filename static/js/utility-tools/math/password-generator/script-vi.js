...
function generate() {
    ...
    setTimeout(() => {
        try {
            ...
            if (!pool) {
                output.innerText = "Không có ký tự nào — vui lòng giảm loại trừ.";
                updateMeter(0, 0);
                updateAnalysis("", 0);
                resetGenerateButton();
                return;
            ...
            if (requirements.length > len) {
                output.innerText = "Quá nhiều loại bắt buộc cho độ dài đã chọn. Tăng chiều dài hoặc giảm yêu cầu.";
                updateMeter(0, 0);
                updateAnalysis("", 0);
                resetGenerateButton();
                return;
            ...
            for (let type of requirements) {
                if (getAvailableChars(type).length === 0) {
                    output.innerText = `"${type}" không có sẵn ký tự nào — hãy bỏ chọn yêu cầu hoặc dừng loại trừ các ký tự này.`;
                    updateMeter(0, 0);
                    updateAnalysis("", 0);
                    resetGenerateButton();
                    return;
            ...
            output.innerText = "Thế hệ thất bại. Vui lòng thử lại.";
...
function resetGenerateButton() {
    ...
    generateIcon.textContent = '🔄';
    generateText.textContent = 'Tạo mật khẩu mới';
}
...
function updateCrackTime(entropy) {
    ...
    crackTime.textContent = `Thời gian crack dự kiến: ${timeText}`;
}
...
function copyAction() {
    ...
    navigator.clipboard.writeText(text).then(() => {
        ...
        toast.textContent = "Sao chép không thành công. Vui lòng chọn và sao chép thủ công.";
        ...
        setTimeout(() => {
            ...
            toast.textContent = "Đã sao chép mật khẩu vào clipboard!";
        }, 2000);
    });
}
...
function exportPasswords() {
    if (passwordHistory.length === 0) {
        alert("Không có lịch sử mật khẩu để xuất.");
        return;
    }
    ...
    toast.textContent = `Đã xuất mật khẩu ${passwordHistory.length}.`;
    toast.classList.add('show');
    setTimeout(() => {
        toast.classList.remove('show');
    }, 2000);
