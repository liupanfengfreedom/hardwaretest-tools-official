...
function generate() {
    ...
    setTimeout(() => {
        try {
            ...
            if (!pool) {
                output.innerText = "使用可能な文字がありません — 除外を減らしてください。";
                updateMeter(0, 0);
                updateAnalysis("", 0);
                resetGenerateButton();
                return;
            ...
            if (requirements.length > len) {
                output.innerText = "選択した長さに対して必要なタイプが多すぎます。長さを長くするか、要件を減らしてください。";
                updateMeter(0, 0);
                updateAnalysis("", 0);
                resetGenerateButton();
                return;
            ...
            for (let type of requirements) {
                if (getAvailableChars(type).length === 0) {
                    output.innerText = `「${type}」には使用可能な文字がありません。要件のチェックを外すか、これらの文字の除外を停止してください。`;
                    updateMeter(0, 0);
                    updateAnalysis("", 0);
                    resetGenerateButton();
                    return;
            ...
            output.innerText = "生成に失敗しました。もう一度試してください。";
...
function resetGenerateButton() {
    ...
    generateIcon.textContent = '🔄';
    generateText.textContent = '新しいパスワードを生成する';
}
...
function updateCrackTime(entropy) {
    ...
    crackTime.textContent = `推定クラックタイム: ${timeText}`;
}
...
function copyAction() {
    ...
    navigator.clipboard.writeText(text).then(() => {
        ...
        toast.textContent = "コピーに失敗しました。手動で選択してコピーしてください。";
        ...
        setTimeout(() => {
            ...
            toast.textContent = "パスワードがクリップボードにコピーされました!";
        }, 2000);
    });
}
...
function exportPasswords() {
    ...
    toast.textContent = `${passwordHistory.length} パスワードをエクスポートしました。`;
    toast.classList.add('show');
    setTimeout(() => {
        toast.classList.remove('show');
    }, 2000);
