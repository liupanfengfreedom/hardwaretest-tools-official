let inputFormat = 'text';
  ...
  async function generateKeyPair() {
    ...
    try {
      ...
      setStatus('✗ 密钥生成失败: ' + e.message, 'error');
    }
  }

  function downloadPrivateKey() {
    const pem = document.getElementById('privateKeyDisplay').value;
    if (!pem) { showToast('暂无私钥', 'error'); return; }
  ...
  function copyConfig(btn) {
    ...
    const doSuccess = () => {
      ...
      showToast('配置已复制为 JSON');
      setTimeout(() => { btn.innerHTML = orig; btn.classList.remove('copied'); }, 2000);
    };
    if (navigator.clipboard?.writeText)
      navigator.clipboard.writeText(json).then(doSuccess).catch(() => fallbackCopy(json) ? doSuccess() : showToast('复制失败', 'error'));
    else
      fallbackCopy(json) ? doSuccess() : showToast('复制失败', 'error');
  }
  ...
  function loadFile(file) {
    ...
    reader.onload = (e) => {
      ...
      const reason = isBinaryFile ? '二进制文件 — 直接使用原始字节加密' : '文件已加载 — 格式固定为 TEXT';
  ...
  function tryTextDecode(bytes) {
    try { return new TextDecoder('utf-8', { fatal: true }).decode(bytes); }
      ...
      return '[二进制文件 — ' + bytes.length + ' bytes]\n\n' +
  ...
  function getInputBytes() {
    ...
    if (!text) throw new Error('请输入要处理的内容');
  ...
  function downloadOutputFile() {
    if (!outputRawBytes) { showToast('暂无加密数据', 'error'); return; }
    ...
    URL.revokeObjectURL(a.href); showToast('文件下载中: ' + base + '.enc');
  }
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      if (!pubPem) throw new Error('请输入或生成公钥');

      let pubDer;
      try { pubDer = pemToDer(pubPem); }
      catch { throw new Error('公钥格式无效，请确认为 PEM 格式'); }
      ...
      try {
        ...
        throw new Error('公钥导入失败 — ' + (e.message || '格式错误或与选择的 Hash 不匹配'));
      }
      ...
      try {
        ...
        if (inputBytes.length > maxBytes) {
          throw new Error(`明文过大（${inputBytes.length} bytes），RSA-OAEP/${hash} 单次最多加密 ${maxBytes} bytes。请缩短内容或使用混合加密。`);
        }
        throw new Error('加密失败: ' + (e.message || '未知错误'));
      }
      ...
      document.getElementById('outputFileLabel').textContent = base + '.enc — 可下载二进制文件';
      strip.classList.add('visible');
      document.getElementById('copyConfigStrip').classList.add('visible');

      setStatus(`✓ 加密成功 — ${formatBytes(inputBytes.length)} → ${formatBytes(encBytes.length)} (RSA-OAEP/${document.getElementById('hash').value} ${document.getElementById('keySize').value}-bit)`, 'success');
    } catch(e) {
      const msg = e.message || '';
      if (msg.includes('公钥')) flashField(document.getElementById('publicKey'));
  ...
  function copyField(fieldId, btn) {
    ...
    if (navigator.clipboard?.writeText)
      navigator.clipboard.writeText(val).then(doSuccess).catch(() => fallbackCopy(val) ? doSuccess() : showToast('复制失败，请手动复制', 'error'));
    else fallbackCopy(val) ? doSuccess() : showToast('复制失败，请手动复制', 'error');
  }
  ...
  function copyOutput(btn) {
    ...
    const doSuccess = () => {
      ...
      btn.style.borderColor = 'var(--accent2)'; btn.style.color = 'var(--accent2)'; showToast('已复制到剪贴板');
      setTimeout(() => { btn.innerHTML = orig; btn.style.borderColor = ''; btn.style.color = ''; }, 1800);
    };
    if (navigator.clipboard?.writeText)
      navigator.clipboard.writeText(text).then(doSuccess).catch(() => fallbackCopy(text) ? doSuccess() : showToast('复制失败，请手动复制', 'error'));
    else fallbackCopy(text) ? doSuccess() : showToast('复制失败，请手动复制', 'error');
  }
  ...
  function updateFormatTabAvailability() {
    ...
    const reasons = []; if (!hexOk) reasons.push('含非 hex 字符'); if (!b64Ok) reasons.push('含非 Base64 字符');
    const tooltip = wrap.querySelector('.custom-tooltip');
  ...
  function updateOutputStat(text) {
    document.getElementById('outputStat').textContent = text ? text.length + ' chars' : '0 bytes';
  }

  // ── Init ─────────────────────────────────────────────────
  updateHints();
  setStatus('就绪 — 粘贴公钥或生成密钥对后点击加密');