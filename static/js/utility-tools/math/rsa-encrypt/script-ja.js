let inputFormat = 'text';
  ...
  async function generateKeyPair() {
    ...
    try {
      ...
      setStatus('✗ キーの生成に失敗しました: ' + e.message, 'error');
    }
  }
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      if (!pubPem) throw new Error('公開キーを貼り付けるか生成してください');

      let pubDer;
      try { pubDer = pemToDer(pubPem); }
      catch { throw new Error('公開キーの形式が無効です。 PEM形式を使用してください'); }
      ...
      try {
        ...
        throw new Error('公開キーのインポートに失敗しました - ' + (e.message || 'invalid format or hash mismatch'));
      }
      ...
      try {
        ...
        if (inputBytes.length > maxBytes) {
          throw new Error(`平文が大きすぎます (${inputBytes.length} バイト)。 RSA-OAEP/${hash} は、操作ごとに最大 ${maxBytes} バイトを暗号化できます。メッセージを短くするか、ハイブリッド暗号化を使用してください。`);
        }
        throw new Error('暗号化に失敗しました: ' + (e.message || 'unknown error'));
      }
      ...
      document.getElementById('outputFileLabel').textContent = base + '.enc - ダウンロード可能なバイナリ ファイル';
      strip.classList.add('visible');
      document.getElementById('copyConfigStrip').classList.add('visible');

      setStatus(`✓ 暗号化成功 - ${formatBytes(inputBytes.length)} -> ${formatBytes(encBytes.length)} (RSA-OAEP/${document.getElementById('hash').value} ${document.getElementById('keySize').value}-bit)`, 'success');
    } catch(e) {
      const msg = e.message || '';
  ...
  function clearInput() {
    ...
    document.getElementById('inputStat').textContent = '0バイト';
    setStatus('準備完了 - 公開キーを構成し、「暗号化」をクリックします');
  }
  ...
  function updateOutputStat(text) {
    document.getElementById('outputStat').textContent = text ? text.length + ' chars' : '0バイト';
  }

  // ── Init ─────────────────────────────────────────────────
  updateHints();
  setStatus('準備完了 - 公開キーを貼り付けるかキーペアを生成し、[暗号化] をクリックします。');
