let inputFormat = 'text';
  ...
  async function generateKeyPair() {
    ...
    try {
      ...
      setStatus('✗ Tạo khóa không thành công: ' + e.message, 'error');
    }
  }
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      if (!pubPem) throw new Error('Vui lòng dán hoặc tạo khóa công khai');

      let pubDer;
      try { pubDer = pemToDer(pubPem); }
      catch { throw new Error('Định dạng khóa công khai không hợp lệ. Vui lòng sử dụng định dạng PEM'); }
      ...
      try {
        ...
        throw new Error('Nhập khóa công khai không thành công - ' + (e.message || 'invalid format or hash mismatch'));
      }
      ...
      try {
        ...
        if (inputBytes.length > maxBytes) {
          throw new Error(`Bản rõ quá lớn (${inputBytes.length} byte). RSA-OAEP/${hash} có thể mã hóa tối đa byte ${maxBytes} cho mỗi thao tác. Rút ngắn tin nhắn hoặc sử dụng mã hóa kết hợp.`);
        }
        throw new Error('Mã hóa không thành công: ' + (e.message || 'unknown error'));
      }
      ...
      document.getElementById('outputFileLabel').textContent = base + '.enc - tệp nhị phân có thể tải xuống';
      strip.classList.add('visible');
      document.getElementById('copyConfigStrip').classList.add('visible');

      setStatus(`✓ Mã hóa thành công - ${formatBytes(inputBytes.length)} -> ${formatBytes(encBytes.length)} (RSA-OAEP/${document.getElementById('hash').value} ${document.getElementById('keySize').value}-bit)`, 'success');
    } catch(e) {
      const msg = e.message || '';
  ...
  function clearInput() {
    ...
    document.getElementById('inputStat').textContent = '0 byte';
    setStatus('Sẵn sàng - định cấu hình khóa chung, sau đó nhấp vào Mã hóa');
  }
  ...
  function updateOutputStat(text) {
    document.getElementById('outputStat').textContent = text ? text.length + ' chars' : '0 byte';
  }

  // ── Init ─────────────────────────────────────────────────
  updateHints();
  setStatus('Sẵn sàng - dán khóa chung hoặc tạo cặp khóa, sau đó nhấp vào Mã hóa');
