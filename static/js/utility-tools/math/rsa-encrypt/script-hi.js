let inputFormat = 'text';
  ...
  async function generateKeyPair() {
    ...
    try {
      ...
      setStatus('✗ मुख्य पीढ़ी विफल: ' + e.message, 'error');
    }
  }
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      if (!pubPem) throw new Error('कृपया सार्वजनिक कुंजी चिपकाएँ या जनरेट करें');

      let pubDer;
      try { pubDer = pemToDer(pubPem); }
      catch { throw new Error('अमान्य सार्वजनिक कुंजी प्रारूप. कृपया PEM प्रारूप का उपयोग करें'); }
      ...
      try {
        ...
        throw new Error('सार्वजनिक कुंजी आयात विफल - ' + (e.message || 'invalid format or hash mismatch'));
      }
      ...
      try {
        ...
        if (inputBytes.length > maxBytes) {
          throw new Error(`सादा पाठ बहुत बड़ा है (${inputBytes.length} बाइट्स)। RSA-OAEP/${hash} प्रति ऑपरेशन अधिकतम ${maxBytes} बाइट्स एन्क्रिप्ट कर सकता है। संदेश को छोटा करें या हाइब्रिड एन्क्रिप्शन का उपयोग करें।`);
        }
        throw new Error('एन्क्रिप्शन विफल: ' + (e.message || 'unknown error'));
      }
      ...
      document.getElementById('outputFileLabel').textContent = base + '.enc - डाउनलोड करने योग्य बाइनरी फ़ाइल';
      strip.classList.add('visible');
      document.getElementById('copyConfigStrip').classList.add('visible');

      setStatus(`✓ एन्क्रिप्शन सफल - ${formatBytes(inputBytes.length)} -> ${formatBytes(encBytes.length)} (RSA-OAEP/${document.getElementById('hash').value} ${document.getElementById('keySize').value}-बिट)`, 'success');
    } catch(e) {
      const msg = e.message || '';
  ...
  function clearInput() {
    ...
    document.getElementById('inputStat').textContent = '0 बाइट्स';
    setStatus('तैयार - सार्वजनिक कुंजी कॉन्फ़िगर करें, फिर एन्क्रिप्ट करें पर क्लिक करें');
  }
  ...
  function updateOutputStat(text) {
    document.getElementById('outputStat').textContent = text ? text.length + ' chars' : '0 बाइट्स';
  }

  // ── Init ─────────────────────────────────────────────────
  updateHints();
  setStatus('तैयार - एक सार्वजनिक कुंजी चिपकाएँ या एक कुंजी युग्म उत्पन्न करें, फिर एन्क्रिप्ट करें पर क्लिक करें');
