let inputFormat = 'text';
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML = `Hex key needs <span id="keyLengthHint">${keyHexChars}</span> characters (${ks / 8} bytes)`;
  ...
  function getIVBytes() {
    ...
    if (!/^[0-9a-fA-F]+$/.test(ivStr)) {
      throw new Error(`The IV must be hex (0-9, a-f). ${mode} mode requires ${neededHexChars} characters.`);
    }
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      document.getElementById('outputFileLabel').textContent = baseName + '.enc - binary file ready to download';
  ...
  function clearInput() {
    ...
    document.getElementById('inputStat').textContent = '0 bytes';
    setStatus('Ready - configure the key and click Encrypt');
  }
  ...
  checkAes192Support().then(supported => {
    if (!supported) {
      ...
      if (opt) {
        opt.disabled = true;
        opt.textContent = '192-bit (unsupported)';
      }
