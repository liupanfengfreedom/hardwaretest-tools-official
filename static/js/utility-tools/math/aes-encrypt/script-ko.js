let inputFormat = 'text';
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML = `16진수 키에는 <span id="keyLengthHint">${keyHexChars}</span> 문자(${ks / 8} 바이트)가 필요합니다.`;
  ...
  function getKeyBytes() {
    ...
    if (!keyStr) throw new Error('키를 입력하거나 생성하세요');
    ...
    if (keyStr.length !== neededHexChars) {
      throw new Error(`${ks} 비트 키에는 ${neededHexChars} 16진수 문자가 필요합니다. 현재 길이: ${keyStr.length}. 새 키를 생성하세요.`);
    }
  ...
  function getIVBytes() {
    ...
    if (!ivStr) throw new Error(`IV를 입력하거나 생성하십시오(${mode} 모드에는 ${neededHexChars} 16진수 문자가 필요함)`);

    // Must be valid hex
    if (!/^[0-9a-fA-F]+$/.test(ivStr)) {
      throw new Error(`IV는 16진수(0-9, a-f)여야 합니다. ${mode} 모드에는 ${neededHexChars} 문자가 필요합니다.`);
    }

    if (ivStr.length !== neededHexChars) {
      throw new Error(`${mode} 모드에는 IV에 대해 ${neededHexChars} 16진수 문자(${neededBytes} 바이트)가 필요합니다. 현재 길이: ${ivStr.length}. 새 IV를 생성하십시오.`);
    }
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      if (ks === 192 && !(await checkAes192Support())) {
        ...
        generateKey();
        updateHints(false);
        throw new Error('이 브라우저는 AES-192를 지원하지 않으므로 도구가 자동으로 AES-256으로 전환되었습니다.');
      }
      ...
      document.getElementById('outputFileLabel').textContent = baseName + '.enc - 다운로드 준비가 완료된 바이너리 파일';
      ...
      setStatus(`✓ 암호화 성공 - ${formatBytes(inputBytes.length)} -> ${formatBytes(encryptedBytes.length)} (${mode}/AES-${document.getElementById('keySize').value})`, 'success');
    } catch(e) {
  ...
  function clearInput() {
    ...
    document.getElementById('inputStat').textContent = '0바이트';
    setStatus('준비 - 키를 구성하고 암호화를 클릭합니다.');
  }
