let inputFormat = 'text';
  ...
  function setStatus(msg, type = 'idle') {
    ...
    txt.textContent = msg;
    time.textContent = type !== 'idle' ? new Date().toLocaleTimeString() : '';
    if (type === 'error') {
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML = `Hex 密钥需 <span id="keyLengthHint">${keyHexChars}</span> 字符 (${ks / 8} bytes)`;
    ...
    if (clearIV) {
      ...
      showToast('模式已切换，请重新生成 IV', 'error');
    }
  ...
  function updateFormatTabAvailability() {
    ...
    // Tooltip
    const reasons = [];
    if (!hexOk) reasons.push('含非 hex 字符，HEX 不可用');
    if (!b64Ok) reasons.push('含非 Base64 字符，BASE64 不可用');
    const tooltip = wrap.querySelector('.custom-tooltip');
    if (reasons.length) {
  ...
  document.getElementById('inputText').addEventListener('keydown', function(e) {
    ...
    if (inputFormat === 'hex' && !/[0-9a-fA-F\s]/.test(e.key)) {
      e.preventDefault();
      showFormatBlockToast('HEX 模式仅允许输入 0–9, a–f');
    } else if (inputFormat === 'base64' && !/[A-Za-z0-9+/=]/.test(e.key)) {
      e.preventDefault();
      showFormatBlockToast('BASE64 模式仅允许输入字母、数字和 +/=');
    }
  });
  ...
  function getKeyBytes() {
    ...
    if (!keyStr) throw new Error('请输入或生成密钥');
    ...
    if (keyStr.length !== neededHexChars) {
      throw new Error(`${ks}-bit 密钥需要 ${neededHexChars} 个 hex 字符，当前为 ${keyStr.length} 个，请重新生成密钥`);
    }
  ...
  function getIVBytes() {
    ...
    if (!ivStr) throw new Error(`请输入或生成 IV（${mode} 模式需要 ${neededHexChars} 个 hex 字符）`);

    // Must be valid hex
    if (!/^[0-9a-fA-F]+$/.test(ivStr)) {
      throw new Error(`IV 必须是 hex 格式（0-9, a-f），当前模式需要 ${neededHexChars} 个字符`);
    }

    if (ivStr.length !== neededHexChars) {
      throw new Error(`${mode} 模式 IV 需要 ${neededHexChars} 个 hex 字符（${neededBytes} bytes），当前为 ${ivStr.length} 个字符。请重新生成 IV`);
    }
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      if (ks === 192 && !(await checkAes192Support())) {
        ...
        generateKey();
        updateHints(false);
        throw new Error('当前浏览器不支持 AES-192，已自动切换为 AES-256');
      }
      ...
      document.getElementById('outputFileLabel').textContent = baseName + '.enc — 可下载二进制文件';
      ...
      setStatus(`✓ 加密成功 — ${formatBytes(inputBytes.length)} → ${formatBytes(encryptedBytes.length)} (${mode}/AES-${document.getElementById('keySize').value})`, 'success');
    } catch(e) {
      // Flash the field that caused the error
      const msg = e.message || '';
      if (msg.includes('密钥')) flashField(document.getElementById('secretKey'));
  ...
  function copyField(fieldId, btn) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(val).then(doSuccess).catch(() => {
        fallbackCopy(val) ? doSuccess() : showToast('复制失败，请手动复制', 'error');
      });
    } else {
      fallbackCopy(val) ? doSuccess() : showToast('复制失败，请手动复制', 'error');
    }
