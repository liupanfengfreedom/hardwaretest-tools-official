let inputFormat = 'text';
  ...
  function setStatus(msg, type = 'idle') {
    ...
    txt.textContent = msg;
    time.textContent = type !== 'idle' ? new Date().toLocaleTimeString('en-GB', { hour12: false }) : '';
    if (type === 'error') {
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML = `Khóa hex cần các ký tự <span id="keyLengthHint">${keyHexChars}</span> (byte ${ks / 8})`;
  ...
  function getKeyBytes() {
    ...
    if (!keyStr) throw new Error('Vui lòng nhập hoặc tạo khóa');
    ...
    if (keyStr.length !== neededHexChars) {
      throw new Error(`Khóa bit ${ks} yêu cầu ký tự thập lục phân ${neededHexChars}. Độ dài hiện tại: ${keyStr.length}. Vui lòng tạo một khóa mới.`);
    }
  ...
  function getIVBytes() {
    ...
    if (!ivStr) throw new Error(`Vui lòng nhập hoặc tạo IV (chế độ ${mode} yêu cầu các ký tự hex ${neededHexChars})`);

    // Must be valid hex
    if (!/^[0-9a-fA-F]+$/.test(ivStr)) {
      throw new Error(`IV phải ở dạng hex (0-9, a-f). Chế độ ${mode} yêu cầu các ký tự ${neededHexChars}.`);
    }

    if (ivStr.length !== neededHexChars) {
      throw new Error(`Chế độ ${mode} yêu cầu các ký tự hex ${neededHexChars} (${neededBytes} byte) cho IV. Độ dài hiện tại: ${ivStr.length}. Vui lòng tạo IV mới.`);
    }
  ...
  function getInputBytes() {
    ...
    if (!text) throw new Error('Vui lòng nhập nội dung để xử lý');
    if (inputFormat === 'hex') {
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      if (ks === 192 && !(await checkAes192Support())) {
        ...
        generateKey();
        updateHints(false);
        throw new Error('Trình duyệt này không hỗ trợ AES-192 nên công cụ tự động chuyển sang AES-256');
      }
      ...
      document.getElementById('outputFileLabel').textContent = baseName + '.enc - tệp nhị phân sẵn sàng để tải xuống';
      ...
      setStatus(`✓ Mã hóa thành công - ${formatBytes(inputBytes.length)} -> ${formatBytes(encryptedBytes.length)} (${mode}/AES-${document.getElementById('keySize').value})`, 'success');
    } catch(e) {
  ...
  function clearInput() {
    ...
    document.getElementById('inputStat').textContent = '0 byte';
    setStatus('Sẵn sàng - định cấu hình khóa và nhấp vào Mã hóa');
  }
  ...
  checkAes192Support().then(supported => {
    if (!supported) {
      ...
      if (opt) {
        opt.disabled = true;
        opt.textContent = '192-bit (không được hỗ trợ)';
      }
