let inputFormat = 'text';
  ...
  function setStatus(msg, type = 'idle') {
    ...
    txt.textContent = msg;
    time.textContent = type !== 'idle' ? new Date().toLocaleTimeString('英語版', { hour12: false }) : '';
    if (type === 'error') {
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML = `16 進数キーには <span id="keyLengthHint">${keyHexChars}</span> 文字 (${ks / 8} バイト) が必要です`;
    ...
    document.getElementById('infoMode').textContent = mode + ' モード';
  ...
  function hexToBytes(hex) {
    hex = hex.replace(/\s/g, '');
    if (hex.length % 2 !== 0) throw new Error('無効な 16 進数文字列');
  ...
  function getKeyBytes() {
    ...
    if (!keyStr) throw new Error('キーを入力または生成してください');

    if (!/^[0-9a-fA-F]+$/.test(keyStr)) {
      throw new Error('キーは、0 ～ 9 と a ～ f のみを含む 16 進文字列である必要があります。');
    }

    if (keyStr.length !== neededHexChars) {
      throw new Error(`${ks} ビット キーには ${neededHexChars} 16 進文字が必要です。現在の長さ: ${keyStr.length}。新しいキーを生成してください。`);
    }
  ...
  function getIVBytes() {
    ...
    if (!ivStr) throw new Error(`IV を入力または生成してください (${mode} モードには ${neededHexChars} 16 進文字が必要です)`);

    // Must be valid hex
    if (!/^[0-9a-fA-F]+$/.test(ivStr)) {
      throw new Error(`IV は 16 進数 (0 ～ 9、a ～ f) である必要があります。 ${mode} モードには ${neededHexChars} 文字が必要です。`);
    }

    if (ivStr.length !== neededHexChars) {
      throw new Error(`${mode} モードでは、IV に ${neededHexChars} 16 進文字 (${neededBytes} バイト) が必要です。現在の長さ: ${ivStr.length}。新しい IV を生成してください。`);
    }
  ...
  function getInputBytes() {
    ...
    if (!text) throw new Error('処理する内容を入力してください');
    if (inputFormat === 'hex') {
  ...
  async function runEncrypt() {
    ...
    try {
      ...
      if (ks === 192 && !(await checkAes192Support())) {
        ...
        generateKey();
        updateHints(false);
        throw new Error('このブラウザは AES-192 をサポートしていないため、ツールは自動的に AES-256 に切り替わりました');
      }
      ...
      document.getElementById('outputFileLabel').textContent = baseName + '.enc - バイナリ ファイルをダウンロードする準備ができました';
      ...
      setStatus(`✓ 暗号化成功 - ${formatBytes(inputBytes.length)} -> ${formatBytes(encryptedBytes.length)} (${mode}/AES-${document.getElementById('keySize').value})`, 'success');
    } catch(e) {
  ...
  function clearInput() {
    ...
    document.getElementById('inputStat').textContent = '0バイト';
    setStatus('準備完了 - キーを設定し、「暗号化」をクリックします');
  }
  ...
  checkAes192Support().then(supported => {
    if (!supported) {
      ...
      if (opt) {
        opt.disabled = true;
        opt.textContent = '192 ビット (サポートされていません)';
      }
