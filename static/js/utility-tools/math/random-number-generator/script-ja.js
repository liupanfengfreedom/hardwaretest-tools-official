...
function updateMaxCount() {
    ...
    if (uniqueCheckbox.checked) {
        ...
        if (maxPossible < 10000) {
            ...
            countInput.title = `ユニークモードで最大の ${maxPossible} 数を生成可能`;
        } else {
            countInput.max = 10000;
            countInput.title = 'ユニークモードでは最大 10000 個の番号を生成可能';
        }
    } else {
        countInput.max = 10000;
        countInput.title = '最大 10000 個の番号を生成可能';
    }
}
...
function generate() {
    ...
    if (currentMode === 'int' || currentMode === 'dec') {
        const digits = parseInt(document.getElementById('digits').value) || 6;
        if (digits > 50) {
            warningArea.innerHTML = `\r\n                <div class="warning">\r\n                    <strong>注:</strong> 50 桁を超える非常に大きな整数を生成しています。表示や計算に時間がかかる場合があります。\r\n                </div>\r\n            `;
        }
    }
...
function renderHistory() {
    ...
    if (history.length === 0) {
        historyList.innerHTML = '<div style="color: var(--text-secondary); text-align: center; padding: 20px;">履歴記録はまだありません</div>';
        return;
    }
...
function clearHistory() {
    if (confirm("すべての履歴レコードをクリアしてもよろしいですか?")) {
        history = [];
        localStorage.setItem('randomHistory', JSON.stringify(history));
        renderHistory();
