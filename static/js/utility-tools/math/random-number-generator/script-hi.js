...
function updateMaxCount() {
    ...
    if (uniqueCheckbox.checked) {
        ...
        if (maxPossible < 10000) {
            ...
            countInput.title = `अधिकतम ${maxPossible} नंबर अद्वितीय मोड में उत्पन्न किए जा सकते हैं`;
        } else {
            countInput.max = 10000;
            countInput.title = 'यूनिक मोड में अधिकतम 10000 नंबर जेनरेट किए जा सकते हैं';
        }
    } else {
        countInput.max = 10000;
        countInput.title = 'अधिकतम 10000 नंबर जेनरेट किये जा सकते हैं';
    }
}
...
function generate() {
    ...
    if (currentMode === 'int' || currentMode === 'dec') {
        const digits = parseInt(document.getElementById('digits').value) || 6;
        if (digits > 50) {
            warningArea.innerHTML = `\r\n                <div class="warning">\r\n                    <strong>नोट:</strong> आप 50 अंकों से अधिक के बहुत बड़े पूर्णांक उत्पन्न कर रहे हैं। प्रदर्शन और गणना में अधिक समय लग सकता है.\r\n                </div>\r\n            `;
        }
    }
...
function renderHistory() {
    ...
    if (history.length === 0) {
        historyList.innerHTML = '<div style="color: var(--text-secondary); text-align: center; padding: 20px;">अभी तक कोई इतिहास रिकॉर्ड नहीं</div>';
        return;
    }
...
function clearHistory() {
    if (confirm("क्या आप वाकई सभी इतिहास रिकॉर्ड साफ़ करना चाहते हैं?")) {
        history = [];
        localStorage.setItem('randomHistory', JSON.stringify(history));
        renderHistory();
