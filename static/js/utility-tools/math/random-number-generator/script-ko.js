...
function updateMaxCount() {
    ...
    if (uniqueCheckbox.checked) {
        ...
        if (maxPossible < 10000) {
            ...
            countInput.title = `고유 모드에서 최대 ${maxPossible} 수를 생성할 수 있습니다.`;
        } else {
            countInput.max = 10000;
            countInput.title = '고유 모드에서는 최대 10000개의 숫자를 생성할 수 있습니다.';
        }
    } else {
        countInput.max = 10000;
        countInput.title = '최대 10000개의 숫자를 생성할 수 있습니다.';
    }
}
...
function generate() {
    ...
    if (currentMode === 'int' || currentMode === 'dec') {
        const digits = parseInt(document.getElementById('digits').value) || 6;
        if (digits > 50) {
            warningArea.innerHTML = `\r\n                <div class="warning">\r\n                    <strong>참고:</strong> 50자리가 넘는 매우 큰 정수를 생성하고 있습니다. 표시 및 계산에 시간이 더 걸릴 수 있습니다.\r\n                </div>\r\n            `;
        }
    }
...
function renderHistory() {
    ...
    if (history.length === 0) {
        historyList.innerHTML = '<div style="color: var(--text-secondary); text-align: center; padding: 20px;">아직 기록이 없습니다</div>';
        return;
    }
