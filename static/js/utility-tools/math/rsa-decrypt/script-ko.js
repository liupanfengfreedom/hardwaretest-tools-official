let totalBytesDecrypted = 0;
  ...
  function loadPemFile(input) {
    ...
    function validatePemText(text) {
      ...
      successMsg.textContent = `"${file.name}" - ${label}, ${bodyLines.length} Base64 라인을 로드했습니다.`;
  ...
  function updateHints() {
    ...
    document.getElementById('infoSecSub').textContent   = '비대칭';
  }
  ...
  function getCipherBytes() {
    ...
    } catch { throw new Error('잘못된 Base64 암호문입니다. 입력을 확인해주세요.'); }
  }
  ...
  async function runDecrypt() {
    ...
    try {
      ...
      if (!privPem) throw new Error('개인키를 입력해주세요');

      let privDer;
      try { privDer = pemToDer(privPem); }
      catch { throw new Error('개인 키 형식이 잘못되었습니다. PEM을 사용해 보세요.'); }
      ...
      try {
        ...
        throw new Error('개인 키 가져오기 실패 - ' + (e.message || 'invalid format, mismatched public key, or hash mismatch'));
      }
      ...
      try {
        ...
        throw new Error('복호화 실패 - 개인 키가 암호화 공개 키와 일치하는지, 해시 알고리즘과 암호문이 올바른지 확인하세요.');
      }
      ...
      setStatus(`✓ 복호화 성공 - ${formatBytes(cipherBytes.length)} -> ${formatBytes(decBytes.length)} (RSA-OAEP/${hash} ${document.getElementById('keySize').value}-bit)`, 'success');
    } catch(e) {
      document.getElementById('outputText').value = '';
  ...
  function clearInput() {
    ...
    setStatus('준비 - 개인 키와 암호문을 붙여넣은 후 복호화를 클릭하세요.');
  }
