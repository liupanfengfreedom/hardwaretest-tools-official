let totalBytesDecrypted = 0;
  ...
  function loadPemFile(input) {
    ...
    function validatePemText(text) {
      ...
      successMsg.textContent = `Đã tải các dòng "${file.name}" - ${label}, ${bodyLines.length} Base64.`;
  ...
  function updateHints() {
    ...
    document.getElementById('infoSecSub').textContent   = 'Bất đối xứng';
  }
  ...
  function getCipherBytes() {
    ...
    } catch { throw new Error('Văn bản mã hóa Base64 không hợp lệ. Vui lòng kiểm tra đầu vào.'); }
  }
  ...
  async function runDecrypt() {
    ...
    try {
      ...
      if (!privPem) throw new Error('Vui lòng nhập khóa riêng');

      let privDer;
      try { privDer = pemToDer(privPem); }
      catch { throw new Error('Định dạng khóa riêng không hợp lệ. Vui lòng sử dụng PEM.'); }
      ...
      try {
        ...
        throw new Error('Nhập khóa riêng không thành công - ' + (e.message || 'invalid format, mismatched public key, or hash mismatch'));
      }
      ...
      try {
        ...
        throw new Error('Giải mã không thành công - vui lòng xác minh khóa riêng khớp với khóa chung mã hóa và thuật toán băm cũng như văn bản mã hóa là chính xác.');
      }
      ...
      setStatus(`✓ Giải mã thành công - ${formatBytes(cipherBytes.length)} -> ${formatBytes(decBytes.length)} (RSA-OAEP/${hash} ${document.getElementById('keySize').value}-bit)`, 'success');
    } catch(e) {
      document.getElementById('outputText').value = '';
  ...
  function clearInput() {
    ...
    setStatus('Sẵn sàng - dán khóa riêng và văn bản mã hóa, sau đó nhấp vào Giải mã');
  }
