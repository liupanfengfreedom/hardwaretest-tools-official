let totalBytesDecrypted = 0;
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML =
      `16 進数キーには <span id="keyLengthHint">${keyHexChars}</span> 文字 (${ks / 8} バイト) が必要です`;
  ...
  function showFormatHint(show) {
    ...
    if (!el) {
      ...
      `;
      el.innerHTML = `<span style="font-size:13px">⚠</span> テキスト ファイルがロードされました - ツールが暗号文のエンコード方法を認識できるように、右下にある <strong>BASE64</strong> または <strong>HEX</strong> を選択してください`;
  ...
  function clearInput() {
    ...
    document.getElementById('inputText').value = '';
    document.getElementById('inputStat').textContent = '0バイト';
    setStatus('準備完了 - キー、IV、暗号文を入力し、「復号化」をクリックします。');
  }
  ...
  function hexToBytes(hex) {
    hex = hex.replace(/\s/g, '');
    if (hex.length % 2 !== 0) throw new Error('16 進文字列の長さは偶数である必要があります');
  ...
  function getKeyBytes() {
    ...
    if (!keyStr) throw new Error('キーを入力してください');
    if (!/^[0-9a-fA-F]+$/.test(keyStr))
      throw new Error('キーは、0 ～ 9 と a ～ f のみを含む 16 進文字列である必要があります。');
    if (keyStr.length !== neededHexChars)
      throw new Error(`${ks} ビット キーには ${neededHexChars} 16 進文字が必要です。現在の長さ: ${keyStr.length}`);
  ...
  function getIVBytes() {
    ...
    if (!ivStr) throw new Error(`IV を入力してください (${mode} モードには ${neededHexChars} 16 進文字が必要です)`);
    if (!/^[0-9a-fA-F]+$/.test(ivStr))
      throw new Error('IV は、0 ～ 9 と a ～ f のみを含む 16 進文字列である必要があります。');
    if (ivStr.length !== neededHexChars)
      throw new Error(`${mode} モードでは、IV に ${neededHexChars} 16 進文字が必要です。現在の長さ: ${ivStr.length}`);
  ...
  function getCipherBytes() {
    ...
    try {
      ...
      throw new Error('無効な Base64 コンテンツです。暗号文の入力を確認してください。');
    }
  }
  ...
  async function runDecrypt() {
    ...
    try {
      ...
      setStatus(`✓ 復号化成功 - ${formatBytes(cipherBytes.length)} -> ${formatBytes(decBytes.length)} (${mode}/AES-${document.getElementById('keySize').value})`, 'success');
    } catch(e) {
