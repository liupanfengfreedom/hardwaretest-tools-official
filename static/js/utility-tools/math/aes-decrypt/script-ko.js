let totalBytesDecrypted = 0;
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML =
      `16진수 키에는 <span id="keyLengthHint">${keyHexChars}</span> 문자(${ks / 8} 바이트)가 필요합니다.`;
    ...
    document.getElementById('infoMode').textContent    = mode + ' 방법';
  ...
  function showFormatHint(show) {
    ...
    if (!el) {
      ...
      `;
      el.innerHTML = `<span style="font-size:13px">⚠</span> 텍스트 파일이 로드되었습니다. 도구가 암호문이 인코딩되는 방식을 알 수 있도록 오른쪽 하단에서 <strong>BASE64</strong> 또는 <strong>HEX</strong>를 선택하세요.`;
  ...
  function clearInput() {
    ...
    document.getElementById('inputText').value = '';
    document.getElementById('inputStat').textContent = '0바이트';
    setStatus('준비 - 키, IV 및 암호문을 입력한 후 Decrypt를 클릭합니다.');
  }
  ...
  function hexToBytes(hex) {
    hex = hex.replace(/\s/g, '');
    if (hex.length % 2 !== 0) throw new Error('16진수 문자열 길이는 짝수여야 합니다.');
  ...
  function getKeyBytes() {
    ...
    if (!keyStr) throw new Error('키를 입력하세요');
    if (!/^[0-9a-fA-F]+$/.test(keyStr))
      throw new Error('키는 0~9 및 a~f만 포함하는 16진수 문자열이어야 합니다.');
    if (keyStr.length !== neededHexChars)
      throw new Error(`${ks} 비트 키에는 ${neededHexChars} 16진수 문자가 필요합니다. 현재 길이: ${keyStr.length}`);
  ...
  function getIVBytes() {
    ...
    if (!ivStr) throw new Error(`IV를 입력하십시오(${mode} 모드에는 ${neededHexChars} 16진수 문자가 필요함)`);
    if (!/^[0-9a-fA-F]+$/.test(ivStr))
      throw new Error('IV는 0-9 및 a-f만 포함하는 16진수 문자열이어야 합니다.');
    if (ivStr.length !== neededHexChars)
      throw new Error(`${mode} 모드에는 IV에 ${neededHexChars} 16진수 문자가 필요합니다. 현재 길이: ${ivStr.length}`);
  ...
  function getCipherBytes() {
    ...
    if (!text) throw new Error('암호문을 입력하세요');
    ...
    try {
      ...
      throw new Error('Base64 콘텐츠가 잘못되었습니다. 암호문 입력을 확인하세요.');
    }
  }
    ...
    try {
      ...
      try {
        ...
        document.getElementById('outputText').value =
          ...
          (result.length > 512 ? '\n...' : '');
        document.getElementById('outputFormat').textContent = '바이너리';
      ...
      if (inputFileName) {
        ...
        document.getElementById('outputFileLabel').textContent = base + ' - 복호화 완료, 다운로드 준비 완료';
      ...
      setStatus(`✓ 복호화 성공 - ${formatBytes(cipherBytes.length)} -> ${formatBytes(decBytes.length)} (${mode}/AES-${document.getElementById('keySize').value})`, 'success');
    } catch(e) {
      // Clear output and flash red border
      document.getElementById('outputText').value = '';
      document.getElementById('outputStat').textContent = '0바이트';
