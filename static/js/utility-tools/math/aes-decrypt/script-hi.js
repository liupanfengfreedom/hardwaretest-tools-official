let totalBytesDecrypted = 0;
  ...
  function setStatus(msg, type = 'idle') {
    ...
    document.getElementById('statusTime').textContent = type !== 'idle' ? new Date().toLocaleTimeString('एन-जीबी', { hour12: false }) : '';
    if (type === 'error') {
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML =
      `हेक्स कुंजी को <span id="keyLengthHint">${keyHexChars}</span> वर्णों की आवश्यकता है (${ks / 8} बाइट्स)`;
  ...
  function showFormatHint(show) {
    ...
    if (!el) {
      ...
      `;
      el.innerHTML = `<span style="font-size:13px">⚠</span> एक टेक्स्ट फ़ाइल लोड की गई थी - कृपया नीचे दाईं ओर <strong>BASE64</strong> या <strong>HEX</strong> चुनें ताकि टूल को पता चले कि सिफरटेक्स्ट कैसे एन्कोड किया गया है`;
  ...
  function clearInput() {
    ...
    document.getElementById('inputText').value = '';
    document.getElementById('inputStat').textContent = '0 बाइट्स';
    setStatus('तैयार - कुंजी, IV और सिफरटेक्स्ट दर्ज करें, फिर डिक्रिप्ट पर क्लिक करें');
  }
  ...
  function hexToBytes(hex) {
    hex = hex.replace(/\s/g, '');
    if (hex.length % 2 !== 0) throw new Error('हेक्स स्ट्रिंग की लंबाई सम होनी चाहिए');
  ...
  function getKeyBytes() {
    ...
    if (!keyStr) throw new Error('कृपया एक कुंजी दर्ज करें');
    ...
    if (keyStr.length !== neededHexChars)
      throw new Error(`${ks}-बिट कुंजियों के लिए ${neededHexChars} हेक्स वर्णों की आवश्यकता होती है। वर्तमान लंबाई: ${keyStr.length}`);
  ...
  function getIVBytes() {
    ...
    if (!ivStr) throw new Error(`कृपया एक IV दर्ज करें (${mode} मोड के लिए ${neededHexChars} हेक्स वर्ण आवश्यक हैं)`);
    if (!/^[0-9a-fA-F]+$/.test(ivStr))
      throw new Error('IV एक हेक्स स्ट्रिंग होनी चाहिए जिसमें केवल 0-9 और a-f हो');
    if (ivStr.length !== neededHexChars)
      throw new Error(`${mode} मोड को IV के लिए ${neededHexChars} हेक्स वर्णों की आवश्यकता होती है। वर्तमान लंबाई: ${ivStr.length}`);
  ...
  function getCipherBytes() {
    ...
    if (!text) throw new Error('कृपया सिफरटेक्स्ट दर्ज करें');
    ...
    try {
      ...
      throw new Error('अमान्य बेस64 सामग्री. कृपया सिफरटेक्स्ट इनपुट की जांच करें।');
    }
  }
  ...
  async function runDecrypt() {
    ...
    try {
      ...
      try {
        ...
        document.getElementById('outputFormat').textContent = 'यूटीएफ-8';
        ...
        document.getElementById('outputText').value =
          ...
          (result.length > 512 ? '\n...' : '');
        document.getElementById('outputFormat').textContent = 'बायनरी';
      ...
      if (inputFileName) {
        ...
        document.getElementById('outputFileLabel').textContent = base + ' - डिक्रिप्शन पूर्ण, डाउनलोड करने के लिए तैयार';
      ...
      setStatus(`✓ डिक्रिप्शन सफल - ${formatBytes(cipherBytes.length)} -> ${formatBytes(decBytes.length)} (${mode}/AES-${document.getElementById('keySize').value})`, 'success');
    } catch(e) {
