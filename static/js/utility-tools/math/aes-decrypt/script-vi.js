let totalBytesDecrypted = 0;
  ...
  function updateHints(clearIV = false) {
    ...
    document.getElementById('keyHint').innerHTML =
      `Khóa hex cần các ký tự <span id="keyLengthHint">${keyHexChars}</span> (byte ${ks / 8})`;
    ...
    document.getElementById('infoMode').textContent    = mode + ' Cách thức';
  ...
  function showFormatHint(show) {
    ...
    if (!el) {
      ...
      `;
      el.innerHTML = `<span style="font-size:13px">⚠</span> Một tệp văn bản đã được tải - vui lòng chọn <strong>BASE64</strong> hoặc <strong>HEX</strong> ở dưới cùng bên phải để công cụ biết văn bản mã hóa được mã hóa như thế nào`;
  ...
  function clearInput() {
    ...
    document.getElementById('inputText').value = '';
    document.getElementById('inputStat').textContent = '0 byte';
    setStatus('Sẵn sàng - nhập khóa, IV và bản mã, sau đó nhấp vào Giải mã');
  }
  ...
  function hexToBytes(hex) {
    hex = hex.replace(/\s/g, '');
    if (hex.length % 2 !== 0) throw new Error('Độ dài chuỗi hex phải chẵn');
  ...
  function getKeyBytes() {
    ...
    if (!keyStr) throw new Error('Vui lòng nhập một khóa');
    if (!/^[0-9a-fA-F]+$/.test(keyStr))
      throw new Error('Khóa phải là chuỗi hex chỉ chứa 0-9 và a-f');
    if (keyStr.length !== neededHexChars)
      throw new Error(`Khóa bit ${ks} yêu cầu ký tự thập lục phân ${neededHexChars}. Độ dài hiện tại: ${keyStr.length}`);
  ...
  function getIVBytes() {
    ...
    if (!ivStr) throw new Error(`Vui lòng nhập IV (Chế độ ${mode} yêu cầu ký tự thập lục phân ${neededHexChars})`);
    if (!/^[0-9a-fA-F]+$/.test(ivStr))
      throw new Error('IV phải là chuỗi hex chỉ chứa 0-9 và a-f');
    if (ivStr.length !== neededHexChars)
      throw new Error(`Chế độ ${mode} yêu cầu các ký tự hex ${neededHexChars} cho IV. Độ dài hiện tại: ${ivStr.length}`);
  ...
  function getCipherBytes() {
    ...
    if (!text) throw new Error('Vui lòng nhập bản mã');
    ...
    try {
      ...
      throw new Error('Nội dung Base64 không hợp lệ. Vui lòng kiểm tra đầu vào văn bản mã hóa.');
    }
  }
    ...
    try {
      ...
      try {
        ...
        document.getElementById('outputText').value =
          ...
          (result.length > 512 ? '\n...' : '');
        document.getElementById('outputFormat').textContent = 'nhị phân';
      ...
      if (inputFileName) {
        ...
        document.getElementById('outputFileLabel').textContent = base + ' - giải mã hoàn tất, sẵn sàng để tải xuống';
      ...
      setStatus(`✓ Giải mã thành công - ${formatBytes(cipherBytes.length)} -> ${formatBytes(decBytes.length)} (${mode}/AES-${document.getElementById('keySize').value})`, 'success');
    } catch(e) {
      // Clear output and flash red border
      document.getElementById('outputText').value = '';
      document.getElementById('outputStat').textContent = '0 byte';
