...
function getInputBytes() {
  ...
  if (!text) throw new Error('请输入数据或上传文件');
  if (inputEncoding === 'hex') {
    const clean = text.replace(/\s/g,'');
    if (!/^[0-9a-fA-F]*$/.test(clean)) throw new Error('Hex 格式仅允许 0-9 和 a-f');
    if (clean.length % 2 !== 0) throw new Error('Hex 字符数必须为偶数');
  ...
  if (inputEncoding === 'base64') {
    try {
      ...
      if (rem === 1) throw new Error('Base64 格式无效（单个字符不足以编码任何字节，最少需要 2 个字符）');
      if (rem > 0) b64 += '='.repeat(4 - rem);
      const raw = atob(b64);
      return new Uint8Array([...raw].map(c => c.charCodeAt(0)));
    } catch(err) {
      throw new Error(err.message.startsWith('Base64') ? err.message : 'Base64 格式无效，请检查输入');
    }
  }
...
function allFormats(val) {
  return [
    { label: 'HEX', v: '0x' + val.toString(16).toUpperCase().padStart(8,'0') },
    { label: 'DEC', v: val.toString(10) },
    { label: 'BIN', v: val.toString(2).padStart(32,'0') },
    { label: 'OCT', v: '0o' + val.toString(8).padStart(11,'0') },
  ];
}

const VARIANT_META = {
  iso:        { name: 'CRC-32 / ISO-HDLC', desc: '多项式 0xEDB88320 · 反射输入输出 · ZIP / PNG / Ethernet' },
  castagnoli: { name: 'CRC-32C / Castagnoli', desc: '多项式 0x82F63B78 · 反射输入输出 · iSCSI / NVMe / SCTP' },
  mpeg2:      { name: 'CRC-32 / MPEG-2',    desc: '多项式 0x04C11DB7 · 不反射 · MPEG-2 传输流' },
  bzip2:      { name: 'CRC-32 / BZIP2',     desc: '多项式 0x04C11DB7 · 不反射 · BZip2 / AAL5' },
};
...
function runCalculate(silent = false) {
  ...
  try {
    ...
    setStatus(`✓ 计算完成 — ${active.toUpperCase()}: ${mainVal} (${bytes.length} bytes, ${elapsed}ms)`, 'success');

    if (document.getElementById('expectedCRC').value.trim()) runVerify();

  } catch(e) {
    if (silent) return; // 打字过程中静默，不报错
    ...
    const linkHTML = encSection
      ? ` <a class="status-link" href="javascript:void(0)" onclick="scrollToCard('${encSection}',${encIdx})">查看格式说明 ↗</a>`
      : '';
...
// ─── Verify ───────────────────────────────────────────────────────────────────
function runVerify() {
  if (!lastResults) { showToast('请先计算 CRC32', 'error'); return; }
  const raw = document.getElementById('expectedCRC').value.trim();
  if (!raw) { showToast('请填写预期校验值', 'error'); return; }
  ...
  if (isNaN(expected)) { showToast('无法解析校验值，支持 0x…(hex)、十进制、二进制', 'error'); return; }
  ...
  el.innerHTML = match
    ? `✓ &nbsp; 校验匹配 — 计算值 <strong style="margin:0 6px">${formatCRC(got,'hex')}</strong> 与预期值一致`
    : `✗ &nbsp; 校验不匹配 — 计算值 <strong style="margin:0 6px">${formatCRC(got,'hex')}</strong> ≠ 预期值 <strong style="margin:0 6px">0x${expected.toString(16).toUpperCase().padStart(8,'0')}</strong>`;
}
...
function buildHexDump(bytes) {
  ...
  if (bytes.length > total) {
    lines.push(`\n… 仅预览前 ${total} 字节，文件共 ${bytes.length} 字节`);
  }
  return lines.join('\n');
}
...
function copyPrimary() {
  if (!lastResults) { showToast('暂无结果', 'error'); return; }
  const active = document.getElementById('crcVariant').value;
  const fmt = document.getElementById('outputFormat').value;
...
function copyText(text, btn) {
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    ...
    fallbackCopy(text) ? doSuccess() : showToast('复制失败，请手动复制', 'error');
  }
}
