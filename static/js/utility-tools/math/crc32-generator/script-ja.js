...
function getInputBytes() {
  ...
  if (!text) throw new Error('データを入力するか、ファイルをアップロードしてください');
  if (inputEncoding === 'hex') {
    const clean = text.replace(/\s/g,'');
    if (!/^[0-9a-fA-F]*$/.test(clean)) throw new Error('16 進入力では 0 ～ 9 および a ～ f のみが許可されます');
    if (clean.length % 2 !== 0) throw new Error('16 進入力には偶数の文字が含まれている必要があります');
  ...
  if (inputEncoding === 'base64') {
    try {
      ...
      if (rem === 1) throw new Error('無効な Base64: 1 文字ではバイトをエンコードできません。少なくとも 2 文字が必要です');
      if (rem > 0) b64 += '='.repeat(4 - rem);
      const raw = atob(b64);
      return new Uint8Array([...raw].map(c => c.charCodeAt(0)));
    } catch(err) {
      throw new Error(err.message.startsWith('Base64') ? err.message : 'Base64 形式が無効です。入力を確認してください。');
    }
  }
...
function allFormats(val) {
  return [
    { label: '16進数', v: '0x' + val.toString(16).toUpperCase().padStart(8,'0') },
    { label: '12月', v: val.toString(10) },
    { label: 'ビン', v: val.toString(2).padStart(32,'0') },
    { label: '10月', v: '0o' + val.toString(8).padStart(11,'0') },
  ];
}
...
function runCalculate(silent = false) {
  ...
  try {
    ...
    setStatus(`✓ 計算完了 — ${active.toUpperCase()}: ${mainVal} (${bytes.length} バイト、${elapsed}ms)`, 'success');
...
function runVerify() {
  ...
  el.innerHTML = match
    ? `✓ チェックサムの一致 - 計算値 <strong style="margin:0 6px">${formatCRC(got,'hex')}</strong> が期待値と一致します`
...
function clearFile() {
  ...
  ta.placeholder = 'ここにテキストを入力するか、ファイルをアップロードしてください...\\n\\nCRC32 チェックサムをリアルタイムで計算します';
...
function clearInput() {
  ...
  document.getElementById('inputStat').textContent = '0バイト';
  setStatus('準備完了 - データを入力し、「CRC32 を計算」をクリックします。');
}
...
function clearAll() {
  ...
  setStatus('準備完了 - すべてのデータがリセットされました');
}
...
function setStatus(msg, type='') {
  ...
  const now = new Date();
  time.textContent = now.toLocaleTimeString('英語版', { hour12: false });
}
