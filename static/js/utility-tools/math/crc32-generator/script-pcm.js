...
function getInputBytes() {
  ...
  if (inputEncoding === 'base64') {
    try {
      ...
      if (rem === 1) throw new Error('Invalid Base64: a single character cannot encode any byte, at least 2 characters are required');
...
function loadFile(file) {
  ...
  reader.onload = e => {
    ...
    if (isTextFile(currentFileBytes, file.name)) {
      ...
      badge.textContent = 'TEXT PREVIEW';
