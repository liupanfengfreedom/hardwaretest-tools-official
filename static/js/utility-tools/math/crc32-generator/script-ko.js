...
function getInputBytes() {
  ...
  if (!text) throw new Error('데이터를 입력하거나 파일을 업로드하세요.');
  if (inputEncoding === 'hex') {
    const clean = text.replace(/\s/g,'');
    if (!/^[0-9a-fA-F]*$/.test(clean)) throw new Error('16진수 입력은 0-9 및 a-f만 허용합니다.');
    if (clean.length % 2 !== 0) throw new Error('16진수 입력에는 짝수 개의 문자가 포함되어야 합니다.');
  ...
  if (inputEncoding === 'base64') {
    try {
      ...
      if (rem === 1) throw new Error('잘못된 Base64: 단일 문자는 어떤 바이트도 인코딩할 수 없습니다. 최소 2자가 필요합니다.');
      if (rem > 0) b64 += '='.repeat(4 - rem);
      const raw = atob(b64);
      return new Uint8Array([...raw].map(c => c.charCodeAt(0)));
    } catch(err) {
      throw new Error(err.message.startsWith('Base64') ? err.message : 'Base64 형식이 잘못되었습니다. 입력 내용을 확인하세요.');
    }
  }
...
function allFormats(val) {
  return [
    { label: '마녀', v: '0x' + val.toString(16).toUpperCase().padStart(8,'0') },
    { label: '12월', v: val.toString(10) },
    { label: '큰 상자', v: val.toString(2).padStart(32,'0') },
    { label: '10월', v: '0o' + val.toString(8).padStart(11,'0') },
  ];
}
...
function runCalculate(silent = false) {
  ...
  try {
    ...
    setStatus(`✓ 계산 완료 — ${active.toUpperCase()}: ${mainVal} (${bytes.length} 바이트, ${elapsed}ms)`, 'success');
...
function runVerify() {
  ...
  el.innerHTML = match
    ? `✓   체크섬 일치 - 계산된 값 <strong style="margin:0 6px">${formatCRC(got,'hex')}</strong>가 예상 값과 일치합니다.`
...
function clearFile() {
  ...
  ta.placeholder = '여기에 텍스트를 입력하거나 파일을 업로드하세요...\\n\\n실시간으로 CRC32 체크섬 계산';
...
function clearInput() {
  ...
  document.getElementById('inputStat').textContent = '0바이트';
  setStatus('준비 - 데이터를 입력하고 "CRC32 계산"을 클릭하세요.');
}
