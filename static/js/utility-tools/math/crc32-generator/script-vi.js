...
function getInputBytes() {
  ...
  if (!text) throw new Error('Vui lòng nhập dữ liệu hoặc tải lên một tập tin');
  if (inputEncoding === 'hex') {
    const clean = text.replace(/\s/g,'');
    if (!/^[0-9a-fA-F]*$/.test(clean)) throw new Error('Đầu vào hex chỉ cho phép 0-9 và a-f');
    if (clean.length % 2 !== 0) throw new Error('Dữ liệu nhập hex phải chứa số ký tự chẵn');
  ...
  if (inputEncoding === 'base64') {
    try {
      ...
      if (rem === 1) throw new Error('Base64 không hợp lệ: một ký tự đơn không thể mã hóa bất kỳ byte nào, bắt buộc phải có ít nhất 2 ký tự');
      if (rem > 0) b64 += '='.repeat(4 - rem);
      const raw = atob(b64);
      return new Uint8Array([...raw].map(c => c.charCodeAt(0)));
    } catch(err) {
      throw new Error(err.message.startsWith('Base64') ? err.message : 'Định dạng Base64 không hợp lệ, vui lòng kiểm tra thông tin bạn nhập');
    }
  }
...
function allFormats(val) {
  return [
    ...
    { label: 'THÁNG MƯỜI HAI', v: val.toString(10) },
    { label: 'BIN', v: val.toString(2).padStart(32,'0') },
    { label: 'Tháng 10', v: '0o' + val.toString(8).padStart(11,'0') },
  ];
}
...
function runCalculate(silent = false) {
  ...
  try {
    ...
    setStatus(`✓ Tính toán hoàn tất — ${active.toUpperCase()}: ${mainVal} (${bytes.length} byte, ${elapsed}ms)`, 'success');
...
function runVerify() {
  ...
  el.innerHTML = match
    ? `✓   Tổng kiểm tra khớp — giá trị được tính <strong style="margin:0 6px">${formatCRC(got,'hex')}</strong> khớp với giá trị mong đợi`
...
function loadFile(file) {
  ...
  reader.onload = e => {
    ...
    if (isTextFile(currentFileBytes, file.name)) {
      ...
      badge.textContent = 'XEM TRƯỚC VĂN BẢN';
...
function clearFile() {
  ...
  ta.placeholder = 'Nhập văn bản vào đây hoặc tải tệp lên...\\n\\nTính tổng kiểm CRC32 trong thời gian thực';
...
function clearInput() {
  ...
  document.getElementById('inputStat').textContent = '0 byte';
  setStatus('Sẵn sàng - nhập dữ liệu và nhấp vào "Tính CRC32"');
}
...
function clearAll() {
  ...
  setStatus('Sẵn sàng - tất cả dữ liệu đã được đặt lại');
}
