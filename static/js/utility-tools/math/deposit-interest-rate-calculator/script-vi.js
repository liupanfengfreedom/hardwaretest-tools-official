...
function updateTips(method, rate) {
    ...
    if (method === 'compound') {
        rateTag.innerText = "Điển hình hiện nay";
        rateTag.className = "text-[11px] px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full font-medium shadow-sm";
        tipsCard.innerHTML = `\r\n            <div class="flex items-center gap-3 mb-4">\r\n                <span class="w-2 h-2 bg-indigo-400 rounded-full"></span>\r\n                <span class="font-bold text-indigo-300 text-sm tracking-wider">💡 Logic phức hợp hàng ngày</span>\r\n            </div>\r\n            <p class="text-sm leading-relaxed text-slate-300">\r\n                Mặc dù <b class="text-white">tài khoản vãng lai</b> cung cấp lãi suất danh nghĩa thấp hơn (thường là 0,2% – 0,35%), nhưng họ thường áp dụng <b class="text-white"> ghép lãi hàng ngày</b>: tiền lãi nhỏ mỗi ngày được cộng vào tiền gốc, sau đó kiếm lãi - "lãi trên lãi".\r\n            </p>\r\n            <div class="mt-5 pt-4 border-t border-white/20 text-xs text-slate-400 flex justify-between">\r\n                <span>Công thức tính lãi hàng ngày</span>\r\n                <span class="font-mono text-indigo-300">Lãi suất hàng ngày = số dư × (${rate}% / 360)</span>\r\n            </div>\r\n        `;
    } else {
        rateTag.innerText = "Cố định điển hình";
        rateTag.className = "text-[11px] px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full font-medium shadow-sm";
        tipsCard.innerHTML = `\r\n            <div class="flex items-center gap-3 mb-4">\r\n                <span class="w-2 h-2 bg-emerald-400 rounded-full"></span>\r\n                <span class="font-bold text-emerald-300 text-sm tracking-wider">💎 Tiền gửi cố định Lãi suất đơn giản</span>\r\n            </div>\r\n            <p class="text-sm leading-relaxed text-slate-300">\r\n                <b class="text-white">Tiền gửi cố định</b> đưa ra lãi suất cao hơn (ví dụ: 2,75%), nhưng các ngân hàng thường sử dụng <b class="text-white">lãi suất đơn giản</b>. Lãi suất không gộp; nó tăng trưởng tuyến tính theo thời hạn.\r\n            </p>\r\n            <div class="mt-5 pt-4 border-t border-white/20 text-xs text-slate-400 flex justify-between">\r\n                <span>Công thức tính tổng lãi</span>\r\n                <span class="font-mono text-emerald-300">Tiền lãi = Tiền gốc × ${rate}% × năm</span>\r\n            </div>\r\n        `;
    }
}
