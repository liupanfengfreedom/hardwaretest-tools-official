...
function updateAddBatchBtn(){
  ...
  btn.title = fileBytes ? 'バッチ追加はファイル モードでは使用できません' : '最後にバッチラインを追加します';
}
...
function updateStatStrip(){
  ...
  document.getElementById('statMode').textContent = document.getElementById('batchMode').value==='multi' ? 'バッチ' : '標準';
}
...
function getInputStr(){
  ...
  if(enc==='base64'){
    try{ return atob(txt); } catch(e){ throw new Error('無効なBase64入力'); }
  }
...
function runHash(){
  ...
  try {
    if(fileBytes){
      ...
      setStatus(`✓ ファイルハッシュが完了しました — ${fileName} (${formatBytes(raw.length)})`, 'success');
      return;
    }

    if(!input.trim()){ outputEl.value=''; updateOutputStat(''); renderHashRows(null); setStatus('準備完了 — 自動ハッシュするテキストを入力するか、ボタンをクリックして手動で実行します'); return; }
    ...
    if(isBatch){
      ...
      setStatus(`✓ バッチ完了 - ハッシュ化された ${lines.length} レコード`, 'success');
    } else {
      ...
      setStatus(`✓ ハッシュが正常に生成されました — 128 ビット / 32 16 進数文字`, 'success');
    }
  } catch(e){
...
function renderHashRows(hex32, byteLen){
  ...
  if(!hex32){ container.innerHTML = `<div style="padding:28px 20px;text-align:center;font-family:var(--mono);font-size:12px;color:var(--text3);">— ハッシュ結果はまだありません—</div>`; return; }

  const rows = [
    { label:'MD5-32',      value: hex32 },
    { label:'MD5-16',      value: hex32.substring(8,24) },
    { label:'MD5-32 Uppercase', value: hex32.toUpperCase() },
    { label:'バイト',      value: byteLen > 0 ? formatBytes(byteLen) : '—', noCopy:true },
  ];
...
function loadFile(event){
  ...
  reader.onload = function(e){
    ...
    document.getElementById('inputText').placeholder = `📄 ${file.name} (${formatBytes(file.size)}) がロードされました`;
    updateInputStat();
    updateAddBatchBtn();
    runHash();
  };
...
function clearFile(){
  ...
  document.getElementById('inputText').placeholder='ここにテキストを入力するか、ファイルをドラッグ アンド ドロップ/アップロードします...';
  document.getElementById('outputText').value='';
  document.getElementById('inputStat').textContent='0バイト';
  renderHashRows(null);
  updateAddBatchBtn();
  setStatus('ファイルが削除されました');
}
...
inputPanel.addEventListener('drop', ev=>{
  ...
  if(file){ const reader=new FileReader(); reader.onload=function(e){ fileBytes=new Uint8Array(e.target.result); fileName=file.name; document.getElementById('fileBadge').style.display='flex'; document.getElementById('fileBadgeName').textContent=file.name; document.getElementById('inputText').value=''; document.getElementById('inputText').placeholder=`📄 ${file.name} (${formatBytes(file.size)}) がロードされました`; updateInputStat(); updateAddBatchBtn(); runHash(); }; reader.readAsArrayBuffer(file); }
});
...
function copyAll(btn){
  ...
  doCopy(lines.join('\n'), ()=>{
    const orig=btn.innerHTML;
    btn.classList.add('copied');
    btn.innerHTML='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> コピーしました';
    setTimeout(()=>{ btn.innerHTML=orig; btn.classList.remove('copied'); },1800);
...
function clearInput(){
  ...
  updateOutputStat('');
  renderHashRows(null);
  setStatus('準備完了 — 自動ハッシュするテキストを入力するか、ボタンをクリックして手動で実行します');
}
