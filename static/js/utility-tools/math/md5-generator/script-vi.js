...
function updateAddBatchBtn(){
  ...
  btn.title = fileBytes ? 'Tính năng thêm hàng loạt không khả dụng ở chế độ tệp' : 'Thêm một dòng lô vào cuối';
}
...
function updateStatStrip(){
  ...
  document.getElementById('statMode').textContent = document.getElementById('batchMode').value==='multi' ? 'BỘ' : 'TIÊU CHUẨN';
}
...
function getInputStr(){
  ...
  if(enc==='base64'){
    try{ return atob(txt); } catch(e){ throw new Error('Đầu vào Base64 không hợp lệ'); }
  }
...
function runHash(){
  ...
  try {
    if(fileBytes){
      ...
      setStatus(`✓ Hoàn tất băm tệp — ${fileName} (${formatBytes(raw.length)})`, 'success');
      return;
    }

    if(!input.trim()){ outputEl.value=''; updateOutputStat(''); renderHashRows(null); setStatus('Sẵn sàng - nhập văn bản để tự động băm hoặc nhấp vào nút để chạy thủ công'); return; }
    ...
    if(isBatch){
      ...
      setStatus(`✓ Hoàn thành hàng loạt - bản ghi ${lines.length} được băm`, 'success');
    } else {
      ...
      setStatus(`✓ Đã tạo hàm băm thành công — ký tự 128-bit / 32 hex`, 'success');
    }
  } catch(e){
...
function renderHashRows(hex32, byteLen){
  ...
  if(!hex32){ container.innerHTML = `<div style="padding:28px 20px;text-align:center;font-family:var(--mono);font-size:12px;color:var(--text3);">— Chưa có kết quả băm —</div>`; return; }

  const rows = [
    { label:'MD5-32',      value: hex32 },
    { label:'MD5-16',      value: hex32.substring(8,24) },
    { label:'Chữ hoa MD5-32', value: hex32.toUpperCase() },
    { label:'Byte',      value: byteLen > 0 ? formatBytes(byteLen) : '—', noCopy:true },
  ];
...
function loadFile(event){
  ...
  reader.onload = function(e){
    ...
    document.getElementById('inputText').placeholder = `📄 ${file.name} (${formatBytes(file.size)}) đã tải`;
    updateInputStat();
    updateAddBatchBtn();
    runHash();
  };
...
function clearFile(){
  ...
  document.getElementById('inputText').placeholder='Nhập văn bản vào đây hoặc kéo và thả/tải tệp lên...';
  document.getElementById('outputText').value='';
  document.getElementById('inputStat').textContent='0 byte';
  renderHashRows(null);
  updateAddBatchBtn();
  setStatus('Đã xóa tệp');
}
...
inputPanel.addEventListener('drop', ev=>{
  ...
  if(file){ const reader=new FileReader(); reader.onload=function(e){ fileBytes=new Uint8Array(e.target.result); fileName=file.name; document.getElementById('fileBadge').style.display='flex'; document.getElementById('fileBadgeName').textContent=file.name; document.getElementById('inputText').value=''; document.getElementById('inputText').placeholder=`📄 ${file.name} (${formatBytes(file.size)}) đã tải`; updateInputStat(); updateAddBatchBtn(); runHash(); }; reader.readAsArrayBuffer(file); }
});
...
function copyAll(btn){
  ...
  doCopy(lines.join('\n'), ()=>{
    const orig=btn.innerHTML;
    btn.classList.add('copied');
    btn.innerHTML='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> Đã sao chép';
    setTimeout(()=>{ btn.innerHTML=orig; btn.classList.remove('copied'); },1800);
...
function clearInput(){
  ...
  updateOutputStat('');
  renderHashRows(null);
  setStatus('Sẵn sàng - nhập văn bản để tự động băm hoặc nhấp vào nút để chạy thủ công');
}
