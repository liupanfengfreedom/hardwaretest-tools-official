...
function runHash(){
  ...
  try {
    if(fileBytes){
      ...
      setStatus(`✓ File hash complete — ${fileName} (${formatBytes(raw.length)})`, 'success');
      return;
    }

    if(!input.trim()){ outputEl.value=''; updateOutputStat(''); renderHashRows(null); setStatus('Ready — enter text to auto-hash, or click the button to run manually'); return; }
    ...
    if(isBatch){
      ...
      setStatus(`✓ Batch complete — hashed ${lines.length} records`, 'success');
    } else {
      ...
      setStatus(`✓ Hash generated successfully — 128-bit / 32 hex chars`, 'success');
    }
  } catch(e){
...
function renderHashRows(hex32, byteLen){
  ...
  if(!hex32){ container.innerHTML = `<div style="padding:28px 20px;text-align:center;font-family:var(--mono);font-size:12px;color:var(--text3);">— No hash results yet —</div>`; return; }

  const rows = [
    { label:'MD5-32',      value: hex32 },
    { label:'MD5-16',      value: hex32.substring(8,24) },
    { label:'MD5-32 Uppercase', value: hex32.toUpperCase() },
    { label:'Bytes',      value: byteLen > 0 ? formatBytes(byteLen) : '—', noCopy:true },
  ];
...
function loadFile(event){
  ...
  reader.onload = function(e){
    ...
    document.getElementById('inputText').placeholder = `📄 ${file.name} (${formatBytes(file.size)}) loaded`;
    updateInputStat();
    updateAddBatchBtn();
    runHash();
  };
...
function clearFile(){
  ...
  document.getElementById('inputText').placeholder='Enter text here, or drag and drop / upload a file...';
...
inputPanel.addEventListener('drop', ev=>{
  ...
  if(file){ const reader=new FileReader(); reader.onload=function(e){ fileBytes=new Uint8Array(e.target.result); fileName=file.name; document.getElementById('fileBadge').style.display='flex'; document.getElementById('fileBadgeName').textContent=file.name; document.getElementById('inputText').value=''; document.getElementById('inputText').placeholder=`📄 ${file.name} (${formatBytes(file.size)}) loaded`; updateInputStat(); updateAddBatchBtn(); runHash(); }; reader.readAsArrayBuffer(file); }
});
...
function clearInput(){
  ...
  updateOutputStat('');
  renderHashRows(null);
  setStatus('Ready — enter text to auto-hash, or click the button to run manually');
}
