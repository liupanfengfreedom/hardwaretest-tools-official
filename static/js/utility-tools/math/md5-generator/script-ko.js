...
function updateAddBatchBtn(){
  ...
  btn.title = fileBytes ? '파일 모드에서는 일괄 추가를 사용할 수 없습니다.' : '끝에 배치 라인 추가';
}
...
function updateStatStrip(){
  ...
  document.getElementById('statMode').textContent = document.getElementById('batchMode').value==='multi' ? '일괄' : '기준';
}
...
function getInputStr(){
  ...
  if(enc==='base64'){
    try{ return atob(txt); } catch(e){ throw new Error('잘못된 Base64 입력'); }
  }
...
function runHash(){
  ...
  try {
    if(fileBytes){
      ...
      setStatus(`✓ 파일 해시 완료 — ${fileName} (${formatBytes(raw.length)})`, 'success');
      return;
    }

    if(!input.trim()){ outputEl.value=''; updateOutputStat(''); renderHashRows(null); setStatus('준비 — 자동 해시할 텍스트를 입력하거나 버튼을 클릭하여 수동으로 실행하세요.'); return; }
    ...
    if(isBatch){
      ...
      setStatus(`✓ 배치 완료 — 해시된 ${lines.length} 레코드`, 'success');
    } else {
      ...
      setStatus(`✓ 해시가 성공적으로 생성되었습니다 — 128비트/32개의 16진수 문자`, 'success');
    }
  } catch(e){
...
function renderHashRows(hex32, byteLen){
  ...
  if(!hex32){ container.innerHTML = `<div style="padding:28px 20px;text-align:center;font-family:var(--mono);font-size:12px;color:var(--text3);"> — 아직 해시 결과가 없습니다 —</div>`; return; }

  const rows = [
    { label:'MD5-32',      value: hex32 },
    { label:'MD5-16',      value: hex32.substring(8,24) },
    { label:'MD5-32 대문자', value: hex32.toUpperCase() },
    { label:'바이트',      value: byteLen > 0 ? formatBytes(byteLen) : '—', noCopy:true },
  ];
...
function loadFile(event){
  ...
  reader.onload = function(e){
    ...
    document.getElementById('inputText').placeholder = `📄 ${file.name}(${formatBytes(file.size)}) 로드됨`;
    updateInputStat();
    updateAddBatchBtn();
    runHash();
  };
...
function clearFile(){
  ...
  document.getElementById('inputText').placeholder='여기에 텍스트를 입력하거나 파일을 드래그 앤 드롭/업로드하세요...';
  document.getElementById('outputText').value='';
  document.getElementById('inputStat').textContent='0바이트';
  renderHashRows(null);
  updateAddBatchBtn();
  setStatus('파일이 삭제되었습니다.');
}
...
inputPanel.addEventListener('drop', ev=>{
  ...
  if(file){ const reader=new FileReader(); reader.onload=function(e){ fileBytes=new Uint8Array(e.target.result); fileName=file.name; document.getElementById('fileBadge').style.display='flex'; document.getElementById('fileBadgeName').textContent=file.name; document.getElementById('inputText').value=''; document.getElementById('inputText').placeholder=`📄 ${file.name}(${formatBytes(file.size)}) 로드됨`; updateInputStat(); updateAddBatchBtn(); runHash(); }; reader.readAsArrayBuffer(file); }
});
...
function copyAll(btn){
  ...
  doCopy(lines.join('\n'), ()=>{
    const orig=btn.innerHTML;
    btn.classList.add('copied');
    btn.innerHTML='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> 복사됨';
    setTimeout(()=>{ btn.innerHTML=orig; btn.classList.remove('copied'); },1800);
...
function clearInput(){
  ...
  updateOutputStat('');
  renderHashRows(null);
  setStatus('준비 — 자동 해시할 텍스트를 입력하거나 버튼을 클릭하여 수동으로 실행하세요.');
}
