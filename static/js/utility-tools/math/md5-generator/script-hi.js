...
function updateAddBatchBtn(){
  ...
  btn.title = fileBytes ? 'फ़ाइल मोड में बैच ऐड अनुपलब्ध है' : 'अंत में एक बैच लाइन जोड़ें';
}
...
function updateStatStrip(){
  ...
  document.getElementById('statMode').textContent = document.getElementById('batchMode').value==='multi' ? 'बैच' : 'मानक';
}
...
function getInputStr(){
  ...
  if(enc==='base64'){
    try{ return atob(txt); } catch(e){ throw new Error('अमान्य बेस64 इनपुट'); }
  }
...
function runHash(){
  ...
  try {
    if(fileBytes){
      ...
      setStatus(`✓ फ़ाइल हैश पूर्ण - ${fileName} (${formatBytes(raw.length)})`, 'success');
      return;
    }

    if(!input.trim()){ outputEl.value=''; updateOutputStat(''); renderHashRows(null); setStatus('तैयार - ऑटो-हैश में टेक्स्ट दर्ज करें, या मैन्युअल रूप से चलाने के लिए बटन पर क्लिक करें'); return; }
    ...
    if(isBatch){
      ...
      setStatus(`✓ बैच पूर्ण - हैश किए गए ${lines.length} रिकॉर्ड`, 'success');
    } else {
      ...
      setStatus(`✓ हैश सफलतापूर्वक उत्पन्न हुआ - 128-बिट / 32 हेक्स वर्ण`, 'success');
    }
  } catch(e){
...
function renderHashRows(hex32, byteLen){
  ...
  if(!hex32){ container.innerHTML = `<div style="padding:28px 20px;text-align:center;font-family:var(--mono);font-size:12px;color:var(--text3);">—अभी तक कोई हैश परिणाम नहीं —</div>`; return; }

  const rows = [
    { label:'एमडी5-32',      value: hex32 },
    { label:'एमडी5-16',      value: hex32.substring(8,24) },
    { label:'MD5-32 अपरकेस', value: hex32.toUpperCase() },
    { label:'बाइट्स',      value: byteLen > 0 ? formatBytes(byteLen) : '—', noCopy:true },
  ];

  container.innerHTML = rows.map(r=>`\n    <div class="hash-row">\n      <span class="hash-type-tag"><span class="hash-type-dot"></span>${r.label}</span>\n      <span class="hash-value">${r.value}</span>\n      ${r.noCopy ? '<span></span>' : `<button class="copy-hash-btn" onclick="copyHashValue('${r.value}',this)" title="Copy">
...
function loadFile(event){
  ...
  reader.onload = function(e){
    ...
    document.getElementById('inputText').placeholder = `📄 ${file.name} (${formatBytes(file.size)}) लोड किया गया`;
    updateInputStat();
    updateAddBatchBtn();
    runHash();
  };
...
function clearFile(){
  ...
  document.getElementById('inputText').placeholder='यहां टेक्स्ट दर्ज करें, या फ़ाइल खींचें और छोड़ें/अपलोड करें...';
  document.getElementById('outputText').value='';
  document.getElementById('inputStat').textContent='0 बाइट्स';
  renderHashRows(null);
  updateAddBatchBtn();
  setStatus('फ़ाइल हटा दी गई');
}
...
inputPanel.addEventListener('drop', ev=>{
  ...
  if(file){ const reader=new FileReader(); reader.onload=function(e){ fileBytes=new Uint8Array(e.target.result); fileName=file.name; document.getElementById('fileBadge').style.display='flex'; document.getElementById('fileBadgeName').textContent=file.name; document.getElementById('inputText').value=''; document.getElementById('inputText').placeholder=`📄 ${file.name} (${formatBytes(file.size)}) लोड किया गया`; updateInputStat(); updateAddBatchBtn(); runHash(); }; reader.readAsArrayBuffer(file); }
});
...
function copyAll(btn){
  ...
  if(!lines.length){ showToast('No results available','error'); return; }
  doCopy(lines.join('\n'), ()=>{
    const orig=btn.innerHTML;
    btn.classList.add('copied');
    btn.innerHTML='<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> कॉपी किया गया';
    setTimeout(()=>{ btn.innerHTML=orig; btn.classList.remove('copied'); },1800);
...
function clearInput(){
  ...
  updateOutputStat('');
  renderHashRows(null);
  setStatus('तैयार - ऑटो-हैश में टेक्स्ट दर्ज करें, या मैन्युअल रूप से चलाने के लिए बटन पर क्लिक करें');
}
