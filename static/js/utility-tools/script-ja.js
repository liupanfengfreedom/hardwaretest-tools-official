...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        if (term.length > 0) {
            ...
            if (resultsHint) {
                resultsHint.textContent = `${visibleCardsCount} マッチング ツールが見つかりました`;
            }
        } else {
            const resultsHint = document.querySelector('.search-hint');
            if (resultsHint) {
                resultsHint.textContent = 'キーワードを入力すると、必要なツールがすぐに見つかります';
            }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = 'ESC を押してクリアします。 Ctrl+K を押してすばやく検索します。';
    });
