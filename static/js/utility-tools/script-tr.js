...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            if (countElement) {
                countElement.textContent = `${visibleInSection} aletler`;
            }
        ...
        if (resultsHint) {
            if (term.length > 0) {
                resultsHint.textContent = "{count} eşleştirme aracı bulundu".replace('{count}', visibleCardsCount);
            } else {
                resultsHint.textContent = "İhtiyacınız olan aracı hızlı bir şekilde bulmak için bir anahtar kelime yazın";
            }
        }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = "Temizlemek için ESC tuşuna basın • Hızlı arama yapmak için Ctrl+K";
    });
