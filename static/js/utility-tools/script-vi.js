...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        if (term.length > 0) {
            ...
            if (resultsHint) {
                resultsHint.textContent = `Đã tìm thấy công cụ so khớp ${visibleCardsCount}`;
            }
        } else {
            const resultsHint = document.querySelector('.search-hint');
            if (resultsHint) {
                resultsHint.textContent = 'Gõ từ khóa để nhanh chóng tìm thấy công cụ bạn cần';
            }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = 'Nhấn ESC để xóa • Ctrl+K để tìm kiếm nhanh';
    });
