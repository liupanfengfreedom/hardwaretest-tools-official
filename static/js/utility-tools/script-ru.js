...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            if (countElement) {
                countElement.textContent = `${visibleInSection} инструменты`;
            }
        ...
        if (resultsHint) {
            if (term.length > 0) {
                resultsHint.textContent = "Найдено инструментов сопоставления: {count}".replace('{count}', visibleCardsCount);
            } else {
                resultsHint.textContent = "Введите ключевое слово, чтобы быстро найти нужный инструмент";
            }
        }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = "Нажмите ESC, чтобы очистить • Ctrl+K для быстрого поиска.";
    });
