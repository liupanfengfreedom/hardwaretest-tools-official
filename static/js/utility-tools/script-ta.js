...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            if (countElement) {
                countElement.textContent = `${visibleInSection} கருவிகள்`;
            }
        ...
        if (resultsHint) {
            if (term.length > 0) {
                resultsHint.textContent = "{count} பொருந்தக்கூடிய கருவிகள் உள்ளன".replace('{count}', visibleCardsCount);
            } else {
                resultsHint.textContent = "உங்களுக்குத் தேவையான கருவியை விரைவாகக் கண்டுபிடிக்க ஒரு முக்கிய சொல்லைத் தட்டச்சு செய்யவும்";
            }
        }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = "அழிக்க ESC ஐ அழுத்தவும் • விரைவாக தேட Ctrl+K";
    });
