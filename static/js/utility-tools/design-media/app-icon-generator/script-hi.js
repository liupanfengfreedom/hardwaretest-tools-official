...
uploadArea.addEventListener('drop', (e) => {
    ...
    if (files.length > 0) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            showNotification('कृपया एक छवि फ़ाइल (JPG, PNG, GIF, आदि) छोड़ें', 'error');
        }
    }
});
...
function processImageFile(file) {
    if (file.size > 10 * 1024 * 1024) {
        showNotification('फ़ाइल बहुत बड़ी है. कृपया 10 एमबी से कम की छवि चुनें', 'error');
        return;
    }
    ...
    reader.onload = event => {
        ...
        showNotification('चिह्न लोड किया गया. फसल क्षेत्र को समायोजित करें', 'success');
    };
    reader.onerror = () => {
        showNotification('फ़ाइल पढ़ने में विफल. कृपया पुन: प्रयास करें', 'error');
    };
...
generateBtn.onclick = async () => {
    ...
    generateBtn.innerHTML = '<div class="loading-spinner"></div><span>जेनरेटिंग...</span>';
    ...
    try {
        ...
        showNotification(`सफलतापूर्वक ${CONFIG.ios.length + CONFIG.android.length} आइकन फ़ाइलें जेनरेट की गईं`, 'success');
        
    } catch (error) {
        console.error('Icon generation failed:', error);
        showNotification('पीढ़ी विफल रही. कृपया पुन: प्रयास करें', 'error');
    } finally {
        // Hide Progress Indicator
        progressIndicator.classList.add('hidden');
        generateBtn.disabled = false;
        generateBtn.innerHTML = '<i class="fas fa-download"></i><span> पूर्ण आइकन सेट जेनरेट करें (.zip)</span>';
    }
};
...
function updateProgress(current, total, platform) {
    ...
    if (progressText) {
        progressText.textContent = `आइकन पैक जनरेट कर रहा है... (${current}/${total}) ${platform}`;
    }
}
