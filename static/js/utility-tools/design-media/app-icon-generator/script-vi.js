...
uploadArea.addEventListener('drop', (e) => {
    ...
    if (files.length > 0) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            showNotification('Vui lòng gửi một tệp hình ảnh (JPG, PNG, GIF, v.v.)', 'error');
        }
    }
});
...
function processImageFile(file) {
    if (file.size > 10 * 1024 * 1024) {
        showNotification('Tệp quá lớn. Vui lòng chọn hình ảnh dưới 10MB', 'error');
        return;
    }
    ...
    reader.onload = event => {
        ...
        showNotification('Đã tải biểu tượng. Điều chỉnh vùng cắt', 'success');
    };
    reader.onerror = () => {
        showNotification('Không đọc được tập tin. Vui lòng thử lại', 'error');
    };
...
generateBtn.onclick = async () => {
    ...
    try {
        ...
        showNotification(`Đã tạo thành công tệp biểu tượng ${CONFIG.ios.length + CONFIG.android.length}`, 'success');
        
    } catch (error) {
        console.error('Icon generation failed:', error);
        showNotification('Thế hệ thất bại. Vui lòng thử lại', 'error');
    } finally {
        // Hide Progress Indicator
        progressIndicator.classList.add('hidden');
        generateBtn.disabled = false;
        generateBtn.innerHTML = '<i class="fas fa-download"></i><span>Tạo bộ biểu tượng đầy đủ (.zip)</span>';
    }
};
...
function updateProgress(current, total, platform) {
    ...
    if (progressText) {
        progressText.textContent = `Đang tạo gói biểu tượng... (${current}/${total}) ${platform}`;
    }
}
