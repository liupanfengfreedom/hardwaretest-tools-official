...
uploadArea.addEventListener('drop', (e) => {
    ...
    if (files.length > 0) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            showNotification('이미지 파일(JPG, PNG, GIF 등)을 드롭해 주세요.', 'error');
        }
    }
});
...
function processImageFile(file) {
    if (file.size > 10 * 1024 * 1024) {
        showNotification('파일이 너무 큽니다. 10MB 미만의 이미지를 선택하세요.', 'error');
        return;
    }
    ...
    reader.onload = event => {
        ...
        showNotification('아이콘이 로드되었습니다. 자르기 영역 조정', 'success');
    };
    reader.onerror = () => {
        showNotification('파일을 읽지 못했습니다. 다시 시도해 주세요', 'error');
    };
...
generateBtn.onclick = async () => {
    ...
    generateBtn.innerHTML = '<div class="loading-spinner"></div><span>생성 중...</span>';
    ...
    try {
        ...
        showNotification(`${CONFIG.ios.length + CONFIG.android.length} 아이콘 파일을 성공적으로 생성했습니다.`, 'success');
        
    } catch (error) {
        console.error('Icon generation failed:', error);
        showNotification('생성에 실패했습니다. 다시 시도해 주세요', 'error');
    } finally {
        // Hide Progress Indicator
        progressIndicator.classList.add('hidden');
        generateBtn.disabled = false;
        generateBtn.innerHTML = '<i class="fas fa-download"></i><span>전체 아이콘 세트 생성(.zip)</span>';
    }
};
...
function updateProgress(current, total, platform) {
    ...
    if (progressText) {
        progressText.textContent = `아이콘 팩 생성 중... (${current}/${total}) ${platform}`;
    }
}
