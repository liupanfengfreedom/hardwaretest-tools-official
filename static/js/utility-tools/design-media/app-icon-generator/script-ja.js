...
uploadArea.addEventListener('drop', (e) => {
    ...
    if (files.length > 0) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            showNotification('画像ファイル（JPG、PNG、GIFなど）をドロップしてください。', 'error');
        }
    }
});
...
function processImageFile(file) {
    if (file.size > 10 * 1024 * 1024) {
        showNotification('ファイルが大きすぎます。 10MB以下の画像を選択してください', 'error');
        return;
    }
    ...
    reader.onload = event => {
        ...
        showNotification('アイコンが読み込まれました。トリミング領域を調整する', 'success');
    };
    reader.onerror = () => {
        showNotification('ファイルの読み込みに失敗しました。もう一度試してください', 'error');
    };
...
generateBtn.onclick = async () => {
    ...
    try {
        ...
        showNotification(`${CONFIG.ios.length + CONFIG.android.length} アイコン ファイルが正常に生成されました`, 'success');
        
    } catch (error) {
        console.error('Icon generation failed:', error);
        showNotification('生成に失敗しました。もう一度試してください', 'error');
    } finally {
        // Hide Progress Indicator
        progressIndicator.classList.add('hidden');
        generateBtn.disabled = false;
        generateBtn.innerHTML = '<i class="fas fa-download"></i><span>フル アイコン セット (.zip) を生成</span>';
    }
};
...
function updateProgress(current, total, platform) {
    ...
    if (progressText) {
        progressText.textContent = `アイコン パックを生成中... (${current}/${total}) ${platform}`;
    }
}
