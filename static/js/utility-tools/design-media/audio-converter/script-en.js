...
async function startConversion() {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    try {
      ...
      convSetOp('Resampling → ' + targetSR + ' Hz', targetSR + ' Hz');
...
function makePlayBtnHtml(uid) {
  return '<button class="play-btn" id="pbtn-' + uid + '" onclick="onPlayBtn(\'' + uid + '\')" title="Preview">' +
...
function convDone() {
  convSetPct(100);
  convSetOp('Conversion Complete ✓', null);
