...
function updateFileCount() {
    ...
    if (count > 0) {
        document.querySelector('header p').innerHTML = `Xử lý cục bộ · Xem trước so sánh kích thước · Hỗ trợ định dạng đầy đủ <span class="file-count">${count}</span>`;
    } else {
        document.querySelector('header p').innerText = 'Xử lý cục bộ · Xem trước so sánh kích thước · Hỗ trợ định dạng đầy đủ';
    }
}
...
function markDirty() {
    ...
    btnRun.innerHTML = '<i class="fas fa-sync-alt"></i> Tái xử lý';
    fileQueue.forEach(item => {
        if (item.status === 'done') {
            item.status = 'dirty';
            const badge = document.getElementById(`st-${item.id}`);
            if (badge) { 
                badge.innerText = 'Cần cập nhật'; 
                badge.className = 'status-badge status-dirty';
                badge.innerHTML = '<i class="fas fa-exclamation-circle"></i> Cần cập nhật';
            }
        }
    });
}
...
async function handleFiles(files) {
    ...
    btnRun.disabled = false;
    btnDownloadMain.classList.remove('show');
    btnRun.innerHTML = '<i class="fas fa-bolt"></i> Bắt đầu xử lý';
    ...
    for (const file of files) {
        ...
        try {
            ...
            console.error(e);
            document.getElementById(`sz-${id}`).innerHTML = '<span style="color:var(--warning)"><i class="fas fa-exclamation-triangle"></i> Đọc không thành công</span>';
        }
    }
}
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            img.onerror = function() {
                URL.revokeObjectURL(url);
                reject(new Error('Tải SVG không thành công'));
            };
...
btnRun.onclick = async () => {
    ...
    btnRun.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Đang xử lý...';
    ...
    for (let i = 0; i < fileQueue.length; i++) {
        ...
        const badge = document.getElementById(`st-${item.id}`);
        badge.innerHTML = '<i class="fas fa-cog fa-spin"></i> Đang xử lý';
        ...
        try {
            ...
            badge.innerHTML = '<i class="fas fa-check-circle"></i> Sẵn sàng';
            ...
            badge.innerHTML = '<i class="fas fa-times-circle"></i> không thành công';
            badge.className = 'status-badge status-dirty';
        }
    }
    ...
    setTimeout(() => {
        if (fileQueue.length === 1) {
            btnDownloadMain.innerHTML = '<i class="fas fa-download"></i> Lưu vào máy tính';
        } else {
            btnDownloadMain.innerHTML = `<i class="fas fa-file-archive"></i> Tải xuống tất cả (hình ảnh ${fileQueue.length})`;
        }
...
btnDownloadMain.onclick = async () => {
    ...
    if (fileQueue.length === 1) {
        ...
        btnDownloadMain.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Bao bì...';
        try {
            const zip = new JSZip();
            fileQueue.forEach(item => {
                if (item.resultBlob) {
