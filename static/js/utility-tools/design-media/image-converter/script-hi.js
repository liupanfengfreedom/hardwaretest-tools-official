...
function updateFileCount() {
    ...
    if (count > 0) {
        document.querySelector('header p').innerHTML = `स्थानीय प्रसंस्करण · आकार तुलना पूर्वावलोकन · पूर्ण प्रारूप समर्थन <span class="file-count">${count}</span>`;
    } else {
        document.querySelector('header p').innerText = 'स्थानीय प्रसंस्करण · आकार तुलना पूर्वावलोकन · पूर्ण प्रारूप समर्थन';
    }
}
...
function markDirty() {
    ...
    btnRun.innerHTML = '<i class="fas fa-sync-alt"></i> पुनर्प्रक्रिया';
    fileQueue.forEach(item => {
        if (item.status === 'done') {
            item.status = 'dirty';
            const badge = document.getElementById(`st-${item.id}`);
            if (badge) { 
                badge.innerText = 'अद्यतन की आवश्यकता है'; 
                badge.className = 'status-badge status-dirty';
                badge.innerHTML = '<i class="fas fa-exclamation-circle"></i> को अद्यतन की आवश्यकता है';
            }
        }
    });
}
...
async function handleFiles(files) {
    ...
    btnRun.disabled = false;
    btnDownloadMain.classList.remove('show');
    btnRun.innerHTML = '<i class="fas fa-bolt"></i> प्रसंस्करण प्रारंभ करें';
    ...
    for (const file of files) {
        ...
        try {
            ...
            console.error(e);
            document.getElementById(`sz-${id}`).innerHTML = '<span style="color:var(--warning)"><i class="fas fa-exclamation-triangle"></i> पढ़ें विफल</span>';
        }
    }
}
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            img.onerror = function() {
                URL.revokeObjectURL(url);
                reject(new Error('एसवीजी लोडिंग विफल रही'));
            };
...
btnRun.onclick = async () => {
    ...
    btnRun.innerHTML = '<i class="fas fa-spinner fa-spin"></i> प्रसंस्करण...';
    ...
    for (let i = 0; i < fileQueue.length; i++) {
        ...
        const badge = document.getElementById(`st-${item.id}`);
        badge.innerHTML = '<i class="fas fa-cog fa-spin"></i> प्रसंस्करण';
        ...
        try {
            ...
            badge.innerHTML = '<i class="fas fa-check-circle"></i> तैयार';
            ...
            badge.innerHTML = '<i class="fas fa-times-circle"></i> विफल';
            badge.className = 'status-badge status-dirty';
        }
    }
    ...
    setTimeout(() => {
        if (fileQueue.length === 1) {
            btnDownloadMain.innerHTML = '<i class="fas fa-download"></i> कंप्यूटर में सहेजें';
        } else {
            btnDownloadMain.innerHTML = `<i class="fas fa-file-archive"></i> सभी डाउनलोड करें (${fileQueue.length} छवियां)`;
        }
...
btnDownloadMain.onclick = async () => {
    ...
    if (fileQueue.length === 1) {
        ...
        btnDownloadMain.innerHTML = '<i class="fas fa-spinner fa-spin"></i> पैकेजिंग...';
        try {
            const zip = new JSZip();
            fileQueue.forEach(item => {
                if (item.resultBlob) {
