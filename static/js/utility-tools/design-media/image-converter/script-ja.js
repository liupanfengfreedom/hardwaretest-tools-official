...
function updateFileCount() {
    ...
    if (count > 0) {
        document.querySelector('header p').innerHTML = `ローカル処理 · サイズ比較プレビュー · フルフォーマットサポート <span class="file-count">${count}</span>`;
    } else {
        document.querySelector('header p').innerText = 'ローカル処理 · サイズ比較プレビュー · フルフォーマットのサポート';
    }
}
...
function markDirty() {
    ...
    btnRun.innerHTML = '<i class="fas fa-sync-alt"></i> 再処理';
    fileQueue.forEach(item => {
        if (item.status === 'done') {
            item.status = 'dirty';
            const badge = document.getElementById(`st-${item.id}`);
            if (badge) { 
                badge.innerText = '更新が必要です'; 
                badge.className = 'status-badge status-dirty';
                badge.innerHTML = '<i class="fas fa-exclamation-circle"></i> は更新が必要です';
            }
        }
    });
}
...
async function handleFiles(files) {
    ...
    btnRun.disabled = false;
    btnDownloadMain.classList.remove('show');
    btnRun.innerHTML = '<i class="fas fa-bolt"></i> 処理開始';
    ...
    for (const file of files) {
        ...
        try {
            ...
            console.error(e);
            document.getElementById(`sz-${id}`).innerHTML = '<span style="color:var(--warning)"><i class="fas fa-exclamation-triangle"></i> 読み取りに失敗しました</span>';
        }
    }
}
...
btnRun.onclick = async () => {
    ...
    btnRun.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 処理中...';
    ...
    for (let i = 0; i < fileQueue.length; i++) {
        ...
        const badge = document.getElementById(`st-${item.id}`);
        badge.innerHTML = '<i class="fas fa-cog fa-spin"></i> 処理';
        ...
        try {
            ...
            badge.innerHTML = '<i class="fas fa-check-circle"></i> 準備完了';
            ...
            badge.innerHTML = '<i class="fas fa-times-circle"></i> 失敗しました';
            badge.className = 'status-badge status-dirty';
        }
    }
    ...
    setTimeout(() => {
        if (fileQueue.length === 1) {
            btnDownloadMain.innerHTML = '<i class="fas fa-download"></i> コンピューターに保存';
        } else {
            btnDownloadMain.innerHTML = `<i class="fas fa-file-archive"></i> すべてダウンロード (${fileQueue.length} 画像)`;
        }
...
btnDownloadMain.onclick = async () => {
    ...
    if (fileQueue.length === 1) {
        ...
        btnDownloadMain.innerHTML = '<i class="fas fa-spinner fa-spin"></i> パッケージ化...';
        try {
            const zip = new JSZip();
            fileQueue.forEach(item => {
                if (item.resultBlob) {
