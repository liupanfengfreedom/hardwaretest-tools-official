...
function updateFileCount() {
    ...
    if (count > 0) {
        document.querySelector('header p').innerHTML = `로컬 처리 · 크기 비교 미리보기 · 전체 형식 지원 <span class="file-count">${count}</span>`;
    } else {
        document.querySelector('header p').innerText = '로컬 처리 · 크기 비교 미리보기 · 전체 형식 지원';
    }
}
...
function markDirty() {
    ...
    btnRun.innerHTML = '<i class="fas fa-sync-alt"></i> 재처리';
    fileQueue.forEach(item => {
        if (item.status === 'done') {
            item.status = 'dirty';
            const badge = document.getElementById(`st-${item.id}`);
            if (badge) { 
                badge.innerText = '업데이트 필요'; 
                badge.className = 'status-badge status-dirty';
                badge.innerHTML = '<i class="fas fa-exclamation-circle"></i> 업데이트 필요';
            }
        }
    });
}
...
async function handleFiles(files) {
    ...
    btnRun.disabled = false;
    btnDownloadMain.classList.remove('show');
    btnRun.innerHTML = '<i class="fas fa-bolt"></i> 처리 시작';
    ...
    for (const file of files) {
        ...
        try {
            ...
            console.error(e);
            document.getElementById(`sz-${id}`).innerHTML = '<span style="color:var(--warning)"><i class="fas fa-exclamation-triangle"></i> 읽기 실패</span>';
        }
    }
}
...
btnRun.onclick = async () => {
    ...
    btnRun.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 처리 중...';
    ...
    for (let i = 0; i < fileQueue.length; i++) {
        ...
        const badge = document.getElementById(`st-${item.id}`);
        badge.innerHTML = '<i class="fas fa-cog fa-spin"></i> 처리';
        ...
        try {
            ...
            badge.innerHTML = '<i class="fas fa-check-circle"></i> 준비됨';
            ...
            badge.innerHTML = '<i class="fas fa-times-circle"></i> 실패';
            badge.className = 'status-badge status-dirty';
        }
    }
    ...
    setTimeout(() => {
        if (fileQueue.length === 1) {
            btnDownloadMain.innerHTML = '<i class="fas fa-download"></i> 컴퓨터에 저장';
        } else {
            btnDownloadMain.innerHTML = `<i class="fas fa-file-archive"></i> 모두 다운로드(${fileQueue.length} 이미지)`;
        }
...
btnDownloadMain.onclick = async () => {
    ...
    if (fileQueue.length === 1) {
        ...
        btnDownloadMain.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 포장...';
        try {
            const zip = new JSZip();
            fileQueue.forEach(item => {
                if (item.resultBlob) {
