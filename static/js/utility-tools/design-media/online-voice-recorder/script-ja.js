...
function showSourceIdle() {
  document.getElementById('sourceDisplay').innerHTML =
    `<div class="source-empty">オーディオ ソースが選択されていません — [REC] をクリックし、マイクを共有するか許可するかを選択します</div>`;
}

function setUI(rec) {
  const btn = document.getElementById('recBtn');
  btn.classList.toggle('recording', rec);
  document.getElementById('recBtnText').textContent = rec ? '停止' : '録音';
