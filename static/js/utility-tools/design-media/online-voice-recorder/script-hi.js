...
async function setupRecorderFromStream(stream, isMic) {
  ...
  if (!audioTracks.length) {
    ...
    throw new Error('NO_ऑडियो_ट्रैक');
  }
...
function showSourceIdle() {
  document.getElementById('sourceDisplay').innerHTML =
    `<div class="source-empty">कोई ऑडियो स्रोत चयनित नहीं - REC पर क्लिक करें और माइक्रोफ़ोन साझा करना या अनुमति देना चुनें</div>`;
}

function setUI(rec) {
  const btn = document.getElementById('recBtn');
  btn.classList.toggle('recording', rec);
  document.getElementById('recBtnText').textContent = rec ? 'रुकना' : 'आरईसी';
