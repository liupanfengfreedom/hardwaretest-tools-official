...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            if (countElement) {
                countElement.textContent = `${visibleInSection} أدوات`;
            }
        ...
        if (resultsHint) {
            if (term.length > 0) {
                resultsHint.textContent = "تم العثور على {count} من الأدوات المطابقة".replace('{count}', visibleCardsCount);
            } else {
                resultsHint.textContent = "اكتب كلمة أساسية للعثور بسرعة على الأداة التي تحتاجها";
            }
        }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = "اضغط على ESC لمسح • Ctrl+K للبحث بسرعة";
    });
