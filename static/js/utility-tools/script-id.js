...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            if (countElement) {
                countElement.textContent = `${visibleInSection} peralatan`;
            }
        ...
        if (resultsHint) {
            if (term.length > 0) {
                resultsHint.textContent = "Ditemukan {count} alat yang cocok".replace('{count}', visibleCardsCount);
            } else {
                resultsHint.textContent = "Ketikkan kata kunci untuk menemukan alat yang Anda perlukan dengan cepat";
            }
        }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = "Tekan ESC untuk menghapus • Ctrl+K untuk mencari dengan cepat";
    });
