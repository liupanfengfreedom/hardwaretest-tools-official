...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            if (countElement) {
                countElement.textContent = `${visibleInSection} Werkzeuge`;
            }
        ...
        if (resultsHint) {
            if (term.length > 0) {
                resultsHint.textContent = "{count} passende Tools gefunden".replace('{count}', visibleCardsCount);
            } else {
                resultsHint.textContent = "Geben Sie ein Schlüsselwort ein, um schnell das benötigte Werkzeug zu finden";
            }
        }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = "Drücken Sie ESC zum Löschen. • Strg+K für eine schnelle Suche";
    });
