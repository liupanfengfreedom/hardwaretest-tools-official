...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBar.addEventListener('input', (e) => {
        ...
        sections.forEach(section => {
            ...
            if (countElement) {
                countElement.textContent = `${visibleInSection} اوزار`;
            }
        ...
        if (resultsHint) {
            if (term.length > 0) {
                resultsHint.textContent = "مماثل {count} ٹولز ملے".replace('{count}', visibleCardsCount);
            } else {
                resultsHint.textContent = "مطلوبہ ٹول کو تیزی سے تلاش کرنے کے لیے کلیدی لفظ ٹائپ کریں۔";
            }
        }
    ...
    searchBar.addEventListener('focus', function() {
        searchHint.textContent = "صاف کرنے کے لیے ESC دبائیں • تیزی سے تلاش کرنے کے لیے Ctrl+K";
    });
